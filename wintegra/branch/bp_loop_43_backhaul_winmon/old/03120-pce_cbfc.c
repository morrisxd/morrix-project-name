...
WP_status WPE_DebugCallback (const WP_CHAR * file, const WP_CHAR * function, WP_S32 line, WP_S32 error_id)
{
    if (debug_error)
    {
...
#error XGI_not_defined
    ...
    terminate_on_error (status, output_string, __LINE__);
    printf ("WPE_LayerTwoSystemCreate: WPX_BoardSerdesInit(loopback) OK\n");

#else
    status = WPX_BoardSerdesInit (WP_WINPATH (DEFAULT_WPID), WP_PORT_ENET11, loopback_mode);
    sprintf (output_string, "WPX_BoardSerdesInit ENET11 (WPX_BoardSerdesSetLoopback() )");
    terminate_on_error (status, output_string, __LINE__);

    status = WPX_BoardSerdesInit (WP_WINPATH (DEFAULT_WPID), WP_PORT_ENET12, loopback_mode);
    sprintf (output_string, "WPX_BoardSerdesInit ENET12 (WPX_BoardSerdesSetLoopback() )");
    terminate_on_error (status, output_string, __LINE__);
#endif //WP_BOARD_WDS3_SL
...
#error XGI_should_be_defined
    ...
    gbe_port1 = WP_PortCreate (WP_WINPATH (DEFAULT_WPID), WP_PORT_ENET13, enet_s_fast);
    terminate_on_error (gbe_port1, "WP_PortCreate()", __LINE__);
#endif
    ...
    gbe_port2 = WP_PortCreate (WP_WINPATH (DEFAULT_WPID), WP_PORT_ENET14, enet_s_fast);
...
/**************************************************************************/
...
#if 0
    status = WP_DeviceModify (gbe_dev1, WP_DEV_MOD_ENET_TX_STATMODE | WP_DEV_MOD_ENET_RX_STATMODE | WP_DEV_MOD_ENET_RX_FLOWCONTROL | WP_DEV_MOD_ENET_TX_FLOWCONTROL, enet_dev);
    terminate_on_error (status, "WP_DeviceModify() GENET13", __LINE__);
#endif
    ...
    status = WP_DeviceModify (gbe_dev2, WP_DEV_MOD_ENET_TX_STATMODE | WP_DEV_MOD_ENET_RX_STATMODE | WP_DEV_MOD_ENET_RX_FLOWCONTROL | WP_DEV_MOD_ENET_TX_FLOWCONTROL, enet_dev);
...
/**************************************************************************/
    ...
    gbe2_rx_ch_handle = WP_ChannelCreate (ch_tag++, gbe_dev2, qniw, WP_CH_RX, WP_ENET, &gbe_ch[0]);
    terminate_on_error (gbe2_rx_ch_handle, "WP_ChannelCreate() RX_Channel ENET14", __LINE__);

    /* Create Enet2 fast ("reguler") TX channel */
    gbe2_tx_ch_handle = WP_ChannelCreate (ch_tag++, gbe_dev2, qniw, WP_CH_TX, WP_ENET, &gbe_ch[0]);
    terminate_on_error (gbe2_tx_ch_handle, "WP_ChannelCreate() TX_Channel ENET14", __LINE__);
    ...
    status = WP_IwTxBindingCreate (gbe2_tx_ch_handle, WP_IW_TX_BINDING, &tx_binding_l2pi[0]);
    terminate_on_error (status, "WP_IwTxBindingCreate() TX ENET fast device", __LINE__);
...
static void WPE_HostSendFlowCtrl ()
{
    ...
    for (temp_index = 0; temp_index < 1 /*MAX_NUM_OF_PACKETS */ ; temp_index++)
    {
