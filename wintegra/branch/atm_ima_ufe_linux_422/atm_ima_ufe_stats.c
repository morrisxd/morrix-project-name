   ...
   do
   {
      ...
      case '3':
         for (ii = 0; ii < MAX_ATM_TX_CHANNEL; ii++)
         {
            printf
               ("---------------------------------------------------\n");
            printf
               (" Flow statistics IMA h_flow_agg_trans_bridge_egress[%d] \n",
                ii);
            printf
               ("---------------------------------------------------\n");
            App_ShowFlowAggStats (h_flow_agg_trans_bridge_egress[ii],
                                  flow_stats);
         }
         printf ("----------------------------------------------------\n");
         printf (" Flow statistics IMA h_flow_agg_pwe3_atm_ingress[0] \n");
         printf ("----------------------------------------------------\n");
         App_ShowPwe3FlowAggStats (h_flow_agg_pwe3_atm_ingress[0],
                                   pwe3_flow_stats);
         if (ch == '3')
            break;
         break;
      case '4':
         ...
         for (ii = 0; ii < MAX_ATM_TX_CHANNEL; ii++)
         {
            ...
            printf ("-------------------------------\n");
            App_AtmTxChannelStats (ima_tx_ch_handle[ii],
                                   atm_tx_channel_stats);
         }
         ...
         for (ii = 0; ii < NUM_OF_GROUPS; ii++)
         {
            ...
            printf ("-------------------------------\n");
            App_AtmRxChannelStats (ima_rx_ch_handle[ii],
                                   atm_rx_channel_stats);
         }
      ...
      case '8':
         ...
         WP_ImaGroupRestart (ima_group_handle[0], WP_IMA_USE_EXISTING_GID,
                             0);
#else
                           ...
                           WP_iw_flow_stats flow_stats)
{
   ...
   status =
      WP_IwFlowStatistics (flow_agg_handle, WP_IW_FLOW_STAT, &flow_stats);
...
void App_ShowImaGroupStats (void)
{
   ...
   {
      status =
         WP_ImaGroupStatistics (ima_group_handle[ii],
                                &ima_group_statistics);
      ...
      printf ("ext_stats.rx_last_unknown_addr   : %d\n",
              (WP_U32) ima_group_statistics.ext_stats.
...
void App_ShowImaLinkStats (void)
{
   ...
   {
      status =
         WP_ImaLinkStatistics (ima_link_handle[ii], &ima_link_statistics);
      ...
      printf ("tx_icp_cells-rx_icp_cells[%d][%%%d]\n",
              ima_link_statistics.tx_icp_cells -
              ima_link_statistics.rx_icp_cells,
              100 * (ima_link_statistics.tx_icp_cells -
                     ima_link_statistics.rx_icp_cells) /
              ima_link_statistics.tx_icp_cells);
   ...
   *test_result = WP_OK;
   ...
   for (ii = 0; ii < APP_MAX_TDM_PORTS; ii++)
   {
      ...
      if (tdi_atm_stats.rx_err_hec || tdi_atm_stats.rx_err_addr_mismatch ||
          tdi_atm_stats.rx_err_overrun
          || tdi_atm_stats.trans_rx_err_overrun
          || tdi_atm_stats.trans_tx_err_underrun
          || tdi_atm_stats.ext_stats.rx_crc_err_cells
   ...
   memset (&flow_stats, 0, sizeof (flow_stats));
   status =
      WP_IwFlowStatistics (h_flow_agg_trans_bridge_egress[0],
   ...
   memset (&flow_stats, 0, sizeof (flow_stats));
   status =
      WP_IwFlowStatistics (h_flow_agg_trans_bridge_egress[1],
                           WP_IW_FLOW_STAT, &flow_stats);
   App_TerminateOnError (status, "WP_IwFlowStatistics");
   ...
   status =
      WP_IwFlowStatistics (h_flow_agg_pwe3_atm_ingress[0], WP_IW_FLOW_STAT,
                           &pwe3_flow_stats);
   App_TerminateOnError (status, "WP_IwFlowStatistics");
   ...
   if (pwe3_flow_stats.forwarded_packets != APP_MAX_PKTS_NUM)
   {
      *test_result = (-1);
      printf
         ("IMA PWE3 ingress flow agg forwarded_packets check fail(%d)!\n",
          ii);
   }

   if (pwe3_flow_stats.fbp_drop_packets || pwe3_flow_stats.mtu_drop_packets
       || pwe3_flow_stats.ttl_drop_packets
       || pwe3_flow_stats.tx_queue_drop_packets
       || pwe3_flow_stats.mpls_drop_packets
       || pwe3_flow_stats.denied_packets
   ...
   memset (&atm_tx_channel_stats, 0, sizeof (WP_stats_aal5_tx));
   status =
      WP_ChannelStatistics (ima_tx_ch_handle[0], &atm_tx_channel_stats);
   App_TerminateOnError (status, "WP_ChannelStatistics");
   ...
   memset (&atm_tx_channel_stats, 0, sizeof (WP_stats_aal5_tx));
   status =
      WP_ChannelStatistics (ima_tx_ch_handle[1], &atm_tx_channel_stats);
   App_TerminateOnError (status, "WP_ChannelStatistics");
   ...
   status =
      WP_ChannelStatistics (ima_rx_ch_handle[0], &atm_rx_channel_stats);
   App_TerminateOnError (status, "WP_ChannelStatistics");
   ...
   if (atm_rx_channel_stats.err_crc || atm_rx_channel_stats.err_size ||
       atm_rx_channel_stats.err_underflow
       || atm_rx_channel_stats.err_maxsdu
       || atm_rx_channel_stats.err_iw_underrun
       || atm_rx_channel_stats.err_iw_maxmru
       || atm_rx_channel_stats.err_cpi
       || atm_rx_channel_stats.err_ras_timeout)
   {
      *test_result = (-1);
   ...
   memset (&ima_group_statistics, 0, sizeof (WP_ima_group_statistics));
   status =
      WP_ImaGroupStatistics (ima_group_handle[0], &ima_group_statistics);
   App_TerminateOnError (status, "WP_ImaGroupStatistics");
...
*/
   if (ima_group_statistics.ext_stats.rx_crc_err_cells
       || ima_group_statistics.ext_stats.rx_unsupported_cells)
   {
      *test_result = (-1);
