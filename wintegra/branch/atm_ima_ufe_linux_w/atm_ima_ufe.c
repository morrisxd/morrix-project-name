...
void App_InitHW(void)
{
	...
	App_TerminateOnError(status, "board configure WPX_CONFIGURE_2UPI_1XGI_10SGMII_UFE4");
...
void App_IMADevicesCreate(void)
{
	...
	for(ii = 0;ii < APP_MAX_IMA_DEVICE;ii++)
	{
		h_dev_ima[ii]= WP_DeviceCreate(emphy_port,
				WP_EMPHY_PHY(0, WUFE_PHY_INDEX_GET(ufe4_app_system.phy_handle[ii])),
				/*WP_PHY(0),*/
...
extern void WPX_UFE_FRAMER_FlexmuxTerminateOnError(char * pFatalErrorMessage);
...
void App_ufeSystem(void)
{
	...
	WTI_FlexmuxInit(APP_EMPHY_PORT,   
			0,
...
void App_ImaSetup(void)
{
	...
	for(ii=0;ii<NUM_OF_GROUPS;ii++)
	{
		...
		ima_group_handle[ii] = WP_ImaGroupCreate(ima_sys_handle, &ima_group);
		...
		j = (1 << WP_IMA_EVENT_TTS) |
			...
			(1 << WP_IMA_EVENT_GSU_LASR_RX) |
			(1 << WP_IMA_EVENT_GSU_LASR_TX);
	...
	for(ii=0;ii<NUM_OF_GROUPS;ii++)
	{
		...
		ima_link_handle[2*ii+1] = WP_ImaLinkAdd(ima_group_handle[ii],
				...
				h_dev_ima[2*ii+1],
				ima_link);
		App_TerminateOnError(ima_link_handle[2*ii+1], "WP_ImaLinkAdd()");
	}
...
void App_ChannelsCreate(void)
{
	...
	h_bridge1_iwhost = WP_ChannelCreate(tag_iwhost_rx1,
			...
			WP_IW_HOST,
			&ch_iw_rx_cfg);
	App_TerminateOnError(h_bridge1_iwhost, "WP_ChannelCreate() Host Rx 1 ");
...
void App_ImaChannelsCreate(void)
{
	...
	for(ii=0;ii<MAX_ATM_TX_CHANNEL; ii++)
	{
		...
		ima_tx_ch_handle[ii] = WP_ChannelCreate(0, ima_group_handle[0], ima_qn,
				WP_CH_TX, WP_ATM_AAL5, &iw_ch_aal5_cfg[0]);
		App_TerminateOnError(ima_tx_ch_handle[ii], "WP_ChannelCreate() IMA Tx");

		/* tx binding for IMA channel */
		status =  WP_IwTxBindingCreate(ima_tx_ch_handle[ii], WP_IW_TX_BINDING, &tx_binding_cfg);
		App_TerminateOnError(status, "WP_IwTxBindingCreate()");
	}
	...
	for(ii=0;ii<NUM_OF_GROUPS;ii++)
	{
		...
		ima_rx_ch_handle[ii] = WP_ChannelCreate(0, ima_group_handle[ii],
				h_qnode_host, WP_CH_RX, WP_ATM_AAL5, &iw_ch_aal5_cfg[0]);
		App_TerminateOnError(ima_rx_ch_handle[ii], "WP_ChannelCreate() IMA AAL5 Rx");
	}
}
...
void App_EnableUFESystem(void)
{
	...
	ufe_status = WUFE_SystemEnable(ufe4_app_system.ufeid, WUFE_SYS_EMPHY, WUFE_FULL_DUPLEX);
...
void App_CreateEnetUfeIwSystem(void)
{
	...
	h_bridge1_bport_ima = WP_IwPortCreate(h_iw_sys_bridge1,
			&bridge_port_cfg);
	...
	for(ii=0;ii<MAX_ATM_TX_CHANNEL;ii++)
	{
		...
		h_flow_agg_trans_bridge_egress[ii] = WP_IwFlowAggregationCreate(
				APP_MASTER_WPID,
				WP_IW_TRANS_BRIDGING_MODE,
				&iw_agg_bridging_cfg);
		App_TerminateOnError(h_flow_agg_trans_bridge_egress[ii]," WP_IwFlowAggregationCreate() IMA ");
	}
	...
	for(ii = 0; ii<MAX_ATM_TX_CHANNEL; ii++)
	{ 
		forwarding_action.flow_aggregation = h_flow_agg_trans_bridge_egress[ii];    
		/* MPLS field for classification */
		mpls_label_value = 19+ii;  /* MPLS label 20 bits :: This value is configurable */
		sprintf(rule_string, "n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;0x%x;n",mpls_label_value);
		...
		h_bridge1_dfcflow1[ii] = WP_IwFlowAdd(h_iw_sys_bridge1, WP_FLOW_CLASS_RULE, &flow_class_rule);
		App_TerminateOnError(h_bridge1_dfcflow1[ii]," WP_IwFlowAdd() DFC Bridge1 rule1");
	}
...
void App_CreateUfeEnetIwSystem(void)
{
	...
	/* Directmap System */
	h_iw_sys_directmap = WP_IwSystemCreate(WP_WINPATH(0), WP_IW_DIRECTMAP_MODE,
			&iw_sys_directmap_cfg);
	App_TerminateOnError(h_iw_sys_directmap, "WP_IwSystemCreate() directmap");
	...
	memcpy(pwe3_atm_flow_agg_config->prefix_header, dummy_prefix_header, 18);
	h_flow_agg_pwe3_atm_ingress[0] = WP_IwFlowAggregationCreate(
			APP_MASTER_WPID,
			WP_IW_PWE3_MODE,
			pwe3_atm_flow_agg_config);
	App_TerminateOnError(h_flow_agg_pwe3_atm_ingress[0]," WP_IwFlowAggregationCreate() PWE3 ATM ");

	rx_binding_directmap_cfg.default_aggregation = h_flow_agg_pwe3_atm_ingress[0];
	for(ii=0;ii<NUM_OF_GROUPS;ii++)
	{
		status = WP_IwRxBindingCreate(ima_rx_ch_handle[ii],
				h_iw_sys_directmap,
				ima_qn,
				&rx_binding_directmap_cfg);
		App_TerminateOnError(status, "WP_IwRxBindingCreate rx_aal5_channel");
	}
	/* build the system */
