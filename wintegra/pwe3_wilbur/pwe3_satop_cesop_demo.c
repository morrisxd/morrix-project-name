...
#define WTI_TEST_RAISECOM_DCR						0/*MEF DCR*/
#define WTI_TEST_RAISECOM_DCC						0
#define WTI_DCC_LINE								4
...
#define WTI_CESOP_TO_MPLS_FA_CASCADING              0 /*CAI 1*/             /* MPLS FA cascading enable - in this mode the TDM to PSN flow aggregation points at a layer2 router
...
#else
#define WTI_ENET_MODE                               WP_ENET_NORMAL //WP_ENET_NORMAL/* defines loopback or normal operation on ENET port */
#endif // WTI_XGI_MODE
...
#define WTI_MPLS_LABEL                              0x000641ff
...
/* TDI configuration */
#if WTI_CESOP_TDI
#define WTI_ACTIVE_TDI_PORTS                        1      /* number of active TDI ports, must be defined if working in TDI mode */
...
#else
#define WTI_L3_HEADER_LEN                           0//CAI sizeof(WTI_ip_header)
//#define WTI_L3_HEADER_LEN                           0
#endif
#define WTI_SRC_IP_OFFSET                           12
#define WTI_DEST_IP_OFFSET                          16
#define WTI_L4_HEADER_LEN                           0/*sizeof(WTI_udp_header)*/
#define WTI_L4_LENGTH_OFFSET                        4
#if WTI_CLOCK_REC_MODE

#define WTI_RTP_HEADER_LEN                          sizeof(WTI_rtp_header)/*0 CAI*/

#else

#define WTI_RTP_HEADER_LEN                          0/*0 CAI*/
#endif
...
#define WTI_SYNC_ETH_ENABLE                         0/*CAI 0*/        /* 1 - diff clock from syncE clock*/
...
extern void CLI_F_UfeBSSetExtPllFreeRunModeOffset(char *StrPrm);/*CAI*/
...
extern void  WT_HostTrafficPerform();
extern void WT_DataUnitSetup();
#endif
#if WTI_CESOP_TO_MPLS_FA_CASCADING
...
const Y_MenuEntry V_UfeSetup []=
{
   ...
   {K_Leaf, TRUE, " -> UFE Deep Line Loopback Enable/Disable [client_port_id , disable/enable(0,1) SYSTEM DISABLE/ENABLE(2,3)]",         {(Y_MnuLeafP) CLI_F_UFEDeepLoopbackEnableDisable}},
...
#endif
	{K_Leaf, TRUE, " -> UFE4 PLL Freerun offset [offset][set freerun (optional if in Rx)",         {(Y_MnuLeafP) CLI_F_UfeBSSetExtPllFreeRunModeOffset}},
#endif    
...
const Y_MenuEntry V_DeviceStatistics []=
{
   ...
   {K_Leaf, TRUE, " -> HOST SEND DCC PACKET", {(Y_MnuLeafP)   WT_HostTrafficPerform}},
#endif
};
...
int CLI_T_Main(void)
{
   ...
   /* Display main menu */
	...
	CLI_F_301_TDI_E1_U_CR_NoCas_Two_Boards_N_Line_N_PW(InputBuf);

#else

#if WTI_TEST_RAISECOM_DCC

	CLI_F_SonetSdhE1UnframedNoCas_DCC(InputBuf);

#else

#if (!WTI_CESOP_CLOCK_RECOVERY_ENABLE)

	CLI_F_SonetSdhE1UnframedNoCas(InputBuf);

#else

	CLI_F_CR_101_SonetSdh_E1UnframedNoCas(InputBuf);
#endif
...
void WT_UFEBSSetExtPllFreeRunModeOffset(WP_S32 offset, WP_U32 set_freerun)
{
   if(set_freerun)
   {
      printf("Enabling Freerun mode\n");
      WUFE_BSSetExtPllFreeRunMode(the_system->ufe.ufe_id);
   }
   
   WUFE_BSSetExtPllFreeRunModeOffset(the_system->ufe.ufe_id,offset);
}

void CLI_F_UfeBSSetExtPllFreeRunModeOffset(char *StrPrm)
{
   WP_U32 res,cmd,set_freerun=0;
   WP_S32 offset =0;
    
    res = sscanf(StrPrm, "%d %d %d", &cmd, &offset,&set_freerun);
    if (res != 2 && res != 3)
    {
        WTI_TerminatePrintError("Invalid number of parameters", __LINE__);
        return;
    ...
    WT_UFEBSSetExtPllFreeRunModeOffset(offset,set_freerun);
...
#define HDLC_ADDRESS 0x0abc
struct WT_frame_config
{
      WP_U32 length;
      WP_U8  data_char;
} frame_config[1] =
...
void WT_DataUnitSetup()
{
   ...
   WP_U32 length;
      ...
      for (i=0; i<1; i++)
      {
         ...
         segment->pool_handle = the_system->qnode[WTI_QNODE_HOST].adjunct_pool;
         ...
         *(WP_U16*)segment->data = HDLC_ADDRESS;
         segment->data[2] = 0x03;
         memset(segment->data+3, frame_config[i].data_char, segment->data_size-3);

         length = frame_config[i].length;

         printf("Frame %d to be sent (%d bytes):\n",i,length);
...
void  WT_HostTrafficPerform()
{
   WP_U32      i,j,ch_index,crc_size,flag=0,counter=0;
   WP_data_unit *data_unit;
   WP_data_segment *segment;
   WP_U32 n_bds,act_bds=0;
   WP_status status;
	...
	for (j = 0; j < 1; j++)
	{
		...
		{
		 data_unit = &(the_system->tx_data_unit[i]);
		 status = WP_HostSend(the_system->tx_host_hdlc_ch_handle[ch_index], data_unit);
		 WTI_TerminateOnError(status, "WP_ChannelCreate() RX",__LINE__);
		 WP_Delay(5000);
		}
		}
	}
...
void CLI_F_EnetDeviceStatistics(char *StrPrm)
{
   ...
   printf("----------------------------------------------------\n");
	...
	/*for (i = 2; i < 4; i++)*/
	{
	WP_iw_pwe3_stats pwe3_stats = {0};
	WP_U32 t1, t2, screen_coord, temp1, temp2;
	WP_U32 cur_line;
	WP_U32 res, cmd, val, val1;
   
	WP_IwFlowStatistics(the_system->tx_dcc_fa_handle[i], 
								WP_IW_FLOW_STAT,
                                &pwe3_stats);


	printf("FLOW TX %d PWE3 AGGREGATION STATISTICS:\n", i);
	printf("\n");

	printf("--------------------------------------------------------------\n");
	printf("\n");

	temp1 = pwe3_stats.forwarded_packets;
	temp2 = pwe3_stats.forwarded_packets >> 32;
	printf("forwarded_packets:                                  fbp_drop_packets:");

	printf("%.8x%.8x\n", temp2,temp1);
	temp1 = pwe3_stats.fbp_drop_packets;
	temp2 = pwe3_stats.fbp_drop_packets >> 32;

	printf("%.8x%.8x\n", temp2,temp1);

	temp1 = pwe3_stats.mtu_drop_packets;
	temp2 = pwe3_stats.mtu_drop_packets >> 32;
	printf("mtu_drop_packets:                                   tx_queue_drop_packets:");

	printf("%.8x%.8x\n", temp2,temp1);
	temp1 = pwe3_stats.tx_queue_drop_packets;
	temp2 = pwe3_stats.tx_queue_drop_packets >> 32;

	printf("%.8x%.8x\n", temp2,temp1);


{
	WP_iw_flow_stats flow_stats[1] = {0};
	WP_U32 flow_id = 0;

	i = 3;
	WP_IwFlowStatistics(the_system->rx_dcc_fa_handle[i], 
								WP_IW_FLOW_STAT,
                                &flow_stats);

	printf("FLOW RX %d PWE3 AGGREGATION STATISTICS:\n", i);
	printf("\n");
...
{
   printf("Flow %2.2d Forward Packets        => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->forward_packet >> 32),
       (WP_U32) (flow_stats->forward_packet & 0xffffffff));
   printf("Flow %2.2d FBP Drop Packets       => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->fbp_drop_packets >> 32),
       (WP_U32) (flow_stats->fbp_drop_packets & 0xffffffff));
   printf("Flow %2.2d MTU Drop Packets       => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->mtu_drop_packets >> 32),
       (WP_U32) (flow_stats->mtu_drop_packets & 0xffffffff));
   printf("Flow %2.2d TTL Drop Packets       => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->ttl_drop_packets >> 32),
       (WP_U32) (flow_stats->ttl_drop_packets & 0xffffffff));
   printf("Flow %2.2d Tx Q Drop Packets      => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->tx_queue_drop_packets >> 32),
       (WP_U32) (flow_stats->tx_queue_drop_packets & 0xffffffff));
   printf("Flow %2.2d MPLS Drop Packets      => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->mpls_drop >> 32),
       (WP_U32) (flow_stats->mpls_drop & 0xffffffff));
   printf("Flow %2.2d Denied Packets         => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->denied_packets >> 32),
       (WP_U32) (flow_stats->denied_packets & 0xffffffff));
   printf("Flow %2.2d Group Filtered Packets => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->group_filtered_packets >> 32),
       (WP_U32) (flow_stats->group_filtered_packets & 0xffffffff));
   printf("Flow %2.2d Forwarded Bytes        => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->forwarded_bytes >> 32),
       (WP_U32) (flow_stats->forwarded_bytes & 0xffffffff));
   printf("Flow %2.2d GTP Bad Headers        => %#8.8x %8.8x\n", flow_id,
       (WP_U32) (flow_stats->gtp_bad_headers >> 32),
       (WP_U32) (flow_stats->gtp_bad_headers & 0xffffffff));
   printf("Flow %2.2d Policer NC Packets     => %#8.8x %8.8x\n\n", flow_id,
       (WP_U32) (flow_stats->policer_non_conforming_packets >> 32),
       (WP_U32) (flow_stats->policer_non_conforming_packets & 0xffffffff));
}

}

	for (i = 0; i < WTI_DCC_LINE; i++)
	{
	WP_stats_upi_hdlc upi_hdlc= {0};	

	memset(&upi_hdlc, 0, sizeof(WP_stats_upi_hdlc));

	if ((status = WP_DeviceStatistics(the_system->hdlc_dev_handle[i], &upi_hdlc)))
	{
	printf("Cannot read statistics \n");
	return;
	}

	printf("HDLC %d DEVICE STATISTICS:\n", i);
	printf("----------------------------\n");
	printf("\n");
	printf("Received frames = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_frames >> 32),
	(WP_U32)(upi_hdlc.rx_frames & 0xffff));
	printf("Received bytes = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_bytes >> 32),
	(WP_U32)(upi_hdlc.rx_bytes & 0xffff));
	printf("Received IW frames = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_iw_frames >> 32),
	(WP_U32)(upi_hdlc.rx_iw_frames & 0xffff));
	printf("Transmitted frames = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.tx_frames >> 32),
	(WP_U32)(upi_hdlc.tx_frames & 0xffff));
	printf("Transmitted bytes = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.tx_bytes >> 32),
	(WP_U32)(upi_hdlc.tx_bytes & 0xffff));
	printf("\n\n");

	printf("Received frames with CRC error = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_crc >> 32),
	(WP_U32)(upi_hdlc.rx_err_crc & 0xffff));

	printf("Received frames with ABORT error = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_phy >> 32),
	(WP_U32)(upi_hdlc.rx_err_phy & 0xffff));

	printf("Discarded frames with address mismatch = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_addr_mismatch >> 32),
	(WP_U32)(upi_hdlc.rx_err_addr_mismatch & 0xffff));

	printf("Discarded frames due to buffer overrun = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_buffer_overrun >> 32),
	(WP_U32)(upi_hdlc.rx_err_buffer_overrun & 0xffff));

	printf("Discarded frames due to max SDU exceeded = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_maxsdu >> 32),
	(WP_U32)(upi_hdlc.rx_err_maxsdu & 0xffff));

	printf("Discarded frames due to IW buffer overrun = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_iw_buffer_underrun >> 32),
	(WP_U32)(upi_hdlc.rx_err_iw_buffer_underrun & 0xffff));

	printf("Discarded frames due to IW MRU errors = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_iw_mru >> 32),
	(WP_U32)(upi_hdlc.rx_err_iw_mru & 0xffff));

	printf("Discarded frames due to overrun = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_overrun >> 32),
	(WP_U32)(upi_hdlc.rx_err_overrun & 0xffff));

	printf("Transmit underruns = %#8.8x%8.8x\n",
	(WP_U32)(upi_hdlc.rx_err_parity >> 32),
	(WP_U32)(upi_hdlc.rx_err_parity & 0xffff));
	...
	memset(&enet_stats, 0, sizeof(WP_stats_enet));
   status = WP_DeviceStatistics(the_system->enet_device_DCC, &enet_stats);
   if (status)
   {
      printf("Error Reading Hs Enet Statistics\n");
      exit(1);
...
void CLI_F_MplsFlowAggPrefixHeaderMpls(char *StrPrm)
{
   ...
   mpls_layer2_agg_cfg.prefix_header_mpls[0] = 0x00;
   mpls_layer2_agg_cfg.prefix_header_mpls[1] = 0x0e;
   mpls_layer2_agg_cfg.prefix_header_mpls[2] = 0x5e;
   mpls_layer2_agg_cfg.prefix_header_mpls[3] = 0x0f;
   mpls_layer2_agg_cfg.prefix_header_mpls[4] = 0x86;
   mpls_layer2_agg_cfg.prefix_header_mpls[5] = 0x5d;

   mpls_layer2_agg_cfg.prefix_header_mpls[6] = 0x00;
   mpls_layer2_agg_cfg.prefix_header_mpls[7] = 0x00;
   mpls_layer2_agg_cfg.prefix_header_mpls[8] = 0xbb;
   mpls_layer2_agg_cfg.prefix_header_mpls[9] = 0xbb;
   mpls_layer2_agg_cfg.prefix_header_mpls[10] = 0xbb;
   mpls_layer2_agg_cfg.prefix_header_mpls[11] = 0xbb;
 ...
 * Return Val :
 ***************************************************************/
WP_U32 g_begin_auto_switch_test = 0;
...
void CLI_F_UFEDeepLoopbackEnableDisable(char *StrPrm)
{
	...
	if(1 == enable_loopback)
        {
           WTI_FlexmuxEnableLoopback(0, -1,client_port_id , WT_FLEXMUX_DEEP_LINE_LOOPBACK);
           printf("Deep line loopback Enabled\n");
        }
	else if (0 == enable_loopback)
        {
           WTI_FlexmuxDisableLoopback(0, -1,client_port_id , WT_FLEXMUX_DEEP_LINE_LOOPBACK);
           printf("Deep line loopback disabled\n");
        }
	else if (3 == enable_loopback)
        {
           WTI_FlexmuxEnableLoopback(0, 0,client_port_id , WT_FLEXMUX_DEEP_SYSTEM_LOOPBACK);
           printf("Deep system loopback enabled\n");
        }
	else if (2 == enable_loopback)
        {
           WTI_FlexmuxDisableLoopback(0, 0,client_port_id , WT_FLEXMUX_DEEP_SYSTEM_LOOPBACK);
           printf("Deep system loopback disabled\n");
        }
	else if (4 == enable_loopback)
        {
           g_begin_auto_switch_test = 1;
           printf("g_begin_auto_switch_test = 1;\n");
        }
	else if (5 == enable_loopback)
        {
           g_begin_auto_switch_test = 0;
           printf("g_begin_auto_switch_test = 1;\n");
        }
...
static void WTI_InitDemoStructures(void)
{
   ...
   mpls_enet_header.dest_addr[0] = 0xbb;
   mpls_enet_header.dest_addr[1] = 0xbb;
   mpls_enet_header.dest_addr[2] = 0xbb;
   mpls_enet_header.dest_addr[3] = 0xbb;
   mpls_enet_header.dest_addr[4] = 0xbb;
   mpls_enet_header.dest_addr[5] = 0xbb;
   mpls_enet_header.source_addr[0] = 0xaa;
   mpls_enet_header.source_addr[1] = 0xaa;
   mpls_enet_header.source_addr[2] = 0xaa;
   mpls_enet_header.source_addr[3] = 0xaa;
   mpls_enet_header.source_addr[4] = 0xaa;
   mpls_enet_header.source_addr[5] = 0xaa;
