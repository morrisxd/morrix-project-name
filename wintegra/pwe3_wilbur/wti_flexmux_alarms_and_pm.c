...
extern WP_U32 g_begin_auto_switch_test;
...
if (g_begin_auto_switch_test)
{
	if (0 == k1)
	{
		WTI_FlexmuxForceA(0, line_port_id);
		printf("switch to protection port");
	}
		else
	{
		WTI_FlexmuxForceB(0, line_port_id);
		printf("switch to working port");
	}
}
...
void WTI_enable_alarms(int type)
{
   ...
   WPX_FRMR_RegisterSonetSdhSectionS1AnnounceCallback(&cb_s1_signal);
