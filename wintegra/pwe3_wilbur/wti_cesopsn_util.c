...
/*      status = WP_SysClockCreate(the_system->wpid,WP_BRG3,WP_BRG_SRC_BRGIN1, 0); CAI*/
   ...
   diff_ext_cfg.ref_clock = global_clock_rec_diff_ref_clock;/*25000000;*/

	printf("diff_ext_cfg.ref_clock = %d\r\n", diff_ext_cfg.ref_clock);
...
static void WTI_DfcFilterConfig_1(wp_classification_filter *cfg_ptr)
{
   WP_class_filter_user_fields_config user_config = 
   {
   	/* num_of_user_fields */1,         
   /* ref_point */WP_CLASS_REF_POINT_FRAME_START,    
   /* *user_fields */   
		{
		/* field_offset */14,               
		/* field_size */4
   ...
   memcpy(&(cfg_ptr->user_fields_config),&user_config,sizeof(WP_class_filter_user_fields_config));
   ...
   cfg_ptr->field_ids_array[0] = WP_FIELD_ID_USER_PROGRAMMABLE;
...
void WTI_IwSystemSetup(void)
{
   ...
   status = WP_IwTxBindingCreate(the_system->tx_enet_channel_DCC,
                                 WP_IW_TX_BINDING,
                                 &tx_binding_gbe);
   WTI_TerminateOnError(status, "WP_IwTxBindingCreate TDM-->PSN", __LINE__);
#endif
...
#if WTI_TEST_RAISECOM_DCR
   WTI_DfcFilterConfig_1(&filter_param);
   the_system->dfc_filter_handle = WP_IwClassFilterAdd(the_system->psn2tdm_iw_system,
                                                       &filter_param);
...

{
      ...
      bridge_port_config. unk_mac_sa_filter=WP_IW_UNK_MACSA_FILTER_DISABLE;
      bridge_port_config. unk_mc_mode=WP_IW_UNK_MC_DROP;
      bridge_port_config. bc_ht_mode = WP_IW_BC_HT_DISABLE;
...
static void WTI_DfcRuleConfigBridgeMef_1(WP_flow_class_rule *cfg_ptr, WP_U32 pw_index)
{
   WP_U32 ii;
...
/*   for (ii=0; ii<66; ii++)
   ...
   strcat(the_system->pw[pw_index].dfc_rule, "#;0x00055102;");
...
//#endif /* WT_UFE4 */
   ...
   else if(the_system->iw_system_mode == WTI_IW_SYSTEM_BRIDGE)
   {
      ...
      WTI_DfcRuleConfigBridgeMef_1(&class_cfg_bridge, pw_index);
...
*/
   ...
   *(unsigned int *)0xbd800040 = 0x87ffffff;
   ...
   status = WP_PortEnable(the_system->enet_port_SYNCE, WP_DIRECTION_DUPLEX);
   WTI_TerminateOnError(status, "WP_PortEnable() enet_port",__LINE__);

   /* Enable the Enet Device */
   status = WP_DeviceEnable(the_system->enet_device_SYNCE, WP_DIRECTION_DUPLEX);
   WTI_TerminateOnError(status, "WP_DeviceEnable() enet_device",__LINE__);
   
#endif

#if WTI_TEST_RAISECOM_DCC
   status = WP_PortEnable(the_system->enet_port_DCC, WP_DIRECTION_DUPLEX);
   WTI_TerminateOnError(status, "WP_PortEnable() enet_port",__LINE__);

   /* Enable the Enet Device */
   status = WP_DeviceEnable(the_system->enet_device_DCC, WP_DIRECTION_DUPLEX);
   WTI_TerminateOnError(status, "WP_DeviceEnable() enet_device",__LINE__);

   /* Enable the Enet Rx channel */
   status = WP_ChannelEnable(the_system->rx_enet_channel_DCC);
   WTI_TerminateOnError(status, "WP_ChannelEnable() rx_enet_channel",__LINE__);

   /* Enable the Enet Tx channel */
   status = WP_ChannelEnable(the_system->tx_enet_channel_DCC);
   WTI_TerminateOnError(status, "WP_ChannelEnable() tx_enet_channel",__LINE__);
#endif
...
void WTI_HostQnodeCreate(WTI_qnode *qnode)
{
   ...
   data_pool_cfg.size = 64;/*the_system->qnode_config[WTI_QNODE_HOST].buffer_size;*/
   data_pool_cfg.pad = 0;
...
/*I want to use ETH 14 be the slave of SyncE*/
    status = WPX_BoardSerdesInit(the_system->wpid, WP_PORT_ENET14, WP_FALSE);
    WTI_TerminateOnError(status, "WPX_BoardSerdesSetLoopback() WTI_ENET_PORT", __LINE__);

   WTI_EnetPortConfig(&enet_port_config);
   enet_port_config.interface_mode = WP_ENET_1000_BASE_X;
   the_system->enet_port_SYNCE = WP_PortCreate(the_system->wpid, WP_PORT_ENET14, &enet_port_config);
   WTI_TerminateOnError(the_system->enet_port_DCC, "WP_PortCreate() Ethernet second port",__LINE__);

   enet_device_config.extended_params = NULL;
   the_system->enet_device_SYNCE = WP_DeviceCreate(the_system->enet_port_SYNCE,
                                              0,
                                              WP_DEVICE_ENET,
                                              &enet_device_config);
   WTI_TerminateOnError(the_system->enet_device_DCC, "WP_DeviceCreate() enet second device",__LINE__);

   enet_device_config.extended_params = &enet_device_ex_config;
   status = WP_DeviceModify(the_system->enet_device_SYNCE,
   ...
   status = WPX_BoardSerdesInit(WP_WINPATH(0), WP_PORT_ENET14, WP_FALSE);
   ...
   WTI_EnetPortConfig(&enet_port_config);
   enet_port_config.interface_mode = WP_ENET_1000_BASE_X;
   the_system->enet_port_DCC = WP_PortCreate(the_system->wpid, WP_PORT_ENET14, &enet_port_config);
   WTI_TerminateOnError(the_system->enet_port_DCC, "WP_PortCreate() Ethernet second port",__LINE__);

   enet_device_config.extended_params = NULL;
   the_system->enet_device_DCC = WP_DeviceCreate(the_system->enet_port_DCC,
                                              0,
                                              WP_DEVICE_ENET,
                                              &enet_device_config);
   WTI_TerminateOnError(the_system->enet_device_DCC, "WP_DeviceCreate() enet second device",__LINE__);
   ...
   enet_device_config.extended_params = &enet_device_ex_config;
   status = WP_DeviceModify(the_system->enet_device_DCC,
                            WP_DEV_MOD_ENET_TX_STATMODE |
                            WP_DEV_MOD_ENET_RX_STATMODE,
                            &enet_device_config);
   WTI_TerminateOnError(status, "WP_DeviceModify() enet device",__LINE__);
...
void WTI_EnetChannelsCreate(void)
{
   ...
   the_system->rx_enet_channel_SYNCE = WP_ChannelCreate(0,
                                                  the_system->enet_device_SYNCE,
                                                  the_system->qnode[WTI_QNODE_IW_PSN2TDM].handle,
                                                  WP_CH_RX,
                                                  WP_ENET,
                                                  &enet_rx_ch_config);
   WTI_TerminateOnError(the_system->rx_enet_channel_DCC, "WP_ChannelCreate() Rx enet channel",__LINE__);

   the_system->tx_enet_channel_SYNCE = WP_ChannelCreate(0,
                                                  the_system->enet_device_SYNCE,
                                                  the_system->qnode[WTI_QNODE_IW_TDM2PSN].handle,
                                                  WP_CH_TX,
                                                  WP_ENET,
                                                  &enet_tx_ch_config);
   WTI_TerminateOnError(the_system->tx_enet_channel_DCC, "WP_ChannelCreate() Tx enet channel",__LINE__);
   ...
   the_system->rx_enet_channel_DCC = WP_ChannelCreate(0,
                                                  the_system->enet_device_DCC,
                                                  the_system->qnode[WTI_QNODE_IW_PSN2TDM].handle,
                                                  WP_CH_RX,
                                                  WP_ENET,
                                                  &enet_rx_ch_config);
   ...
   the_system->tx_enet_channel_DCC = WP_ChannelCreate(0,
                                                  the_system->enet_device_DCC,
                                                  the_system->qnode[WTI_QNODE_IW_TDM2PSN].handle,
                                                  WP_CH_TX,
                                                  WP_ENET,
...
/*Note: Change in bus width requires change in pinmux (WPX_BoardConfigure)*/
   ...
   cfg_ptr->emphy_ext = emphy_config;
	...
	memset(&packet_ext, 0, sizeof(WP_port_upi_packet_ext));
	memset(&pkt_limits, 0, sizeof(WP_port_pkt_limits));

	packet_ext.pkt_limits = &pkt_limits;
	pkt_limits.emphy_devices = WTI_DCC_LINE;
	pkt_limits.max_rx_channels = WTI_DCC_LINE*2;
	pkt_limits.max_tx_channels = WTI_DCC_LINE*2;
	
   cfg_ptr->packet_ext = &packet_ext;
#endif
...

{
...
#if 0/*!WTI_CESOP_TO_MPLS_FA_CASCADING CAI*/
   ...
   cfg_ptr->extraction_length = (WTI_ETHERNET_HEADER_SIZE +
                             WTI_MPLS_LABEL_SIZE +
                             WTI_CONTROL_WORD_LEN +
                             WTI_RTP_HEADER_LEN);
...
/* in case of MPLS FA CASCADING, the L2 is added by the layer2 router MPLS FA */
   ...
   mpls_label = WTI_MPLS_LABEL + ((pw_index)<< 12);
	...
	mpls_enet_header.type = (WP_U16)(0x88d8);

	mpls_label = (0x00055102);
...
void WTI_PceFilterConfig(WP_pce_filter_classification *cfg_ptr)
{
   ...
   cfg_ptr->filter_fields[0].field_id = WP_PCE_FIELD_ID_MPLS_HEADER1;
   cfg_ptr->filter_fields[0].field_mode = WP_PCE_FIELD_MODE_COMPARE_EXACT_MATCH;
   cfg_ptr->filter_fields[0].mask_mode = WP_PCE_FIELD_MASK_NOT_USED;
   cfg_ptr->filter_fields[1].field_id = WP_PCE_FIELD_ID_LAST;
   cfg_ptr->no_match_result[0].result_type = WP_PCE_RESULT_LAST;
...
#if (WTI_CESOP_MEF8_ENCAP | WTI_TEST_RAISECOM_DCR)
...
void WTI_SyncEthConfig(WP_sync_eth_config *cfg_ptr)
{
   ...
   cfg_ptr->serdes = WP_SYNC_ENET_SERDED4_LANE0/*WP_SYNC_ENET_SERDED4_LANE1*/;       /* TBD - need to map the winnet to serdes lane */
...
#endif
   cfg_ptr->divider = 15625 /*CAI*/;   /* 125MHz/15625 = 8KHz */
...
void WTI_BoardPinConfig(WP_U32 configuration, WP_U32 board_id, WP_U32 pll_clock_out)
{
   ...
   if (configuration == WTI_DEFAULT_PIN_CONFIG)
   {
      ...
      wp_cr_pin_io.clk_rec_pps2_output = WPX_FPGA_NA;/*WPX_FPGA_CR_PPS2_OUT_SMA2; CAI for syncE*/
   ...
   wp_cr_pin_io.sync_eth_clk0_output = WPX_FPGA_SYNC_ETH0_OUT_SMA2;/*CAI*/ /* out through ext. pll. sources muxes */
...
void WTI_PceRuleConfig(WP_pce_rule_classification *cfg_ptr, WP_U32 pw_index)
{
   ...
   cfg_ptr->enabled = WP_ENABLE;
   cfg_ptr->filter_handle = the_system->pce_filter_handle;
   cfg_ptr->rule_fields[0].field_id = WP_PCE_FIELD_ID_MPLS_HEADER1;
