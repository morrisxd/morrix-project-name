...
#define WTI_CRT_HOST_ADDRESS 0xbec67500/*0xbeccb100 */
#define WTI_CRT_B254_ADDRESS 0xbc0fa000 + 0x500/*0xbc249500 */
...
void CLI_F_ClockRecDebugPrint (char *StrPrm)
{
   ...
   while(ii<iterations)
      ...
      if(global_cr_operation_mode == WP_IW_CLOCK_REC_ADAPTIVE_MODE)
      {
         ...
         printf("TS( %d )",*host_ptr++);

		   /* TSdev */
         host_ptr   = (WP_S32 *)(WTI_CRT_B254_ADDRESS + pw*0x40 + 0x10);
         printf("TSdev %d ",*host_ptr);
		 
         /* Stats */
         status = WP_ClockRecStatistics(the_system->wpid, clock_rec_if_id, &stats,WP_IW_CESOP_PORT_UFE);
         printf("DevEXMA( %d )",stats.network_ave_noise);

         host_ptr  = (WP_S32 *)(host_ptr_cr_base + pw*0x80 + 0x1C);
         printf("AVG %d ",*host_ptr);
      }
      else
      {
         /* DCR */
...
#ifdef __WT_UFE412__
{
			...
			WUFE_clk_rec_info_ram_line clk_rec_info_ram_line = {0};
			...
			printf("dGAP %d ",clk_rec_info_ram_line.delta_gap );

			status = WP_ChannelQDepth(the_system->pw[pw].trans_tx, &qdepth);
			printf("%d ",qdepth);
}
