 ...
 *  File Name   : mlppp_ppp_tdm_util.c
 *
 *****************************************************************************/
...
void App_DataSend(WP_handle h_tx, WP_handle h_pool)
{
   ...
   WP_U8 e_type[2] =  {0x08,0x00};  // IP
   WP_U32 l2_header_len = sizeof(dst_mac) + sizeof(src_mac) + sizeof(vlan_1) + sizeof(e_type);
   WP_U32 ip_header_len = 20;
   WP_U32 payload_len;
   WP_U32 offset;

   /* When MLPPP is selected the DEST MAC header is {0x00,0x00,0x00,0x00,0x00,0x0a} which is classified for MLPPP    *
    * in the EnetHdlc bridge. If it is selected as PPP then the DEST MAC header is {0x00,0x00,0x00,0x00,0x00,0x0b}  *
    * which is classifed in the EnetHdlc bridge                                                                      */
   ...
   memset(km_data_ptr+offset,0x0,payload_len);
...
void app_perform_action(app_task *task)
{
   ...
   switch(task->event_type)
   {
      case WP_EVENT_RX_INDICATE:
      {
         ...
         else if(tag == tag_agg_ppp_lcp)
         {
            printf("receiving on PPP LCP aggregation\n");
            App_DuReceive(h_bridge2_iwhost, WP_DATA_IW);
         }
         else if(tag == tag_agg_mlppp_control)
         {
            printf("receiving on MLPPP Control Packet Aggregation\n");
            App_DuReceive(h_bridge2_iwhost, WP_DATA_IW);
         }
 ...
 * Description  :   not in use now... added for future use
 *
 * Input  params: none
 * Output params: 
 * Return val   : none
 ...
 * Description  :   Common routine to prepare IP Header
 *
 * Input  params: none
 * Output params: 
 * Return val   : none
