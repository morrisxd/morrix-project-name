...
/*------------------------------------------------------------------*\
   We create IPv6 flowAgg now.
   We can only do TC remarking on IPv6 flowAgg
\*------------------------------------------------------------------*/
      ...
      App_TerminateOnError (gbe[i].agg_enet_ipv6,
                            "WP_IwFlowAggregationCreate()", __LINE__);
#if 1
      //////////////////////////////////////////////////////////
      ...
      mc_group_config->qnode = gbe[i].qn_mc;
      gbe[i].mc_group_handle = WP_IwMcGroupCreate(WP_WINPATH(DEFAULT_WPID),
                                                    mc_group_config);
      App_TerminateOnError (gbe[i].mc_group_handle, "WP_IwMcGroupCreate()",__LINE__);

      mc_agg_config->mc_group = gbe[i].mc_group_handle;
 
      gbe[i].agg_multicast =
         WP_IwFlowAggregationCreate (WP_WINPATH (DEFAULT_WPID),
                                     WP_IW_GENERIC_MC_MODE, &mc_agg_config);
      App_TerminateOnError (gbe[i].agg_multicast,
                            "WP_IwFlowAggregationCreate()", __LINE__);
#endif
   }
}
...
/*-------------------------------------------------*\
   ...
   PCE_filter[FILTER_SET_PASSTHROUGH] = 
\*-------------------------------------------------*/
void WPE_CreatePceFilters (void)
{
   ...
   WP_U8 mac_addr_mask[6]        = {0x00, 0x00, 0x00, 0x00, 0x00, 0xff};
   WP_U8 i = 0;
   ...
   filter_class.filter_fields[2].field_mode = WP_PCE_FIELD_MODE_COMPARE_EXACT_MATCH;
   filter_class.filter_fields[2].mask_mode = WP_PCE_FIELD_MASK_USED;
   filter_class.filter_fields[2].mask.mac_addr[0] = 0xff;
   filter_class.filter_fields[2].mask.mac_addr[1] = 0xff;
   ...
   PCE_filter[FILTER_SET_PASSTHROUGH] =
      WP_PceFilterCreate (WP_WINPATH (DEFAULT_WPID),
                          WP_PCE_FILTER_CLASSIFICATION, &filter_class);
   App_TerminateOnError (PCE_filter[FILTER_SET_PASSTHROUGH],
                         "WP_PceFilterCreate", __LINE__);
...
#define RX_CHANNEL_FILTER_TAG          0x2200
   ...
   filter_class.filter_fields[1].mask_mode = WP_PCE_FIELD_MASK_USED;
   filter_class.filter_fields[1].field_mode = WP_PCE_FIELD_MODE_COMPARE_EXACT_MATCH;
      filter_class.filter_fields[1].mask.mac_addr[0] = 0xff;
      filter_class.filter_fields[1].mask.mac_addr[1] = 0xff;
      filter_class.filter_fields[1].mask.mac_addr[2] = 0xff;
      filter_class.filter_fields[1].mask.mac_addr[3] = 0xff;
      filter_class.filter_fields[1].mask.mac_addr[4] = 0xff;
      filter_class.filter_fields[1].mask.mac_addr[5] = 0x00;


   filter_class.filter_fields[2].field_id = WP_PCE_FIELD_ID_MAC_DA;
   filter_class.filter_fields[2].field_mode = WP_PCE_FIELD_MODE_COMPARE_RANGE_LOW;
   filter_class.filter_fields[2].mask_mode = WP_PCE_FIELD_MASK_USED;
   // filter_class.filter_fields[2].field_mode = WP_PCE_FIELD_MODE_COMPARE_EXACT_MATCH;
   // filter_class.filter_fields[2].mask_mode = WP_PCE_FIELD_MASK_NOT_USED;
   for (i = 0; i < 6; i ++)
      filter_class.filter_fields[2].mask.mac_addr[i] = mac_addr_mask[i];

   filter_class.filter_fields[3].field_id = WP_PCE_FIELD_ID_MAC_DA;
   filter_class.filter_fields[3].field_mode = WP_PCE_FIELD_MODE_COMPARE_RANGE_HIGH;
   filter_class.filter_fields[3].mask_mode = WP_PCE_FIELD_MASK_USED;
   for (i = 0; i < 6; i ++)
      filter_class.filter_fields[3].mask.mac_addr[i] = mac_addr_mask[i];
...
////////////////////////////////////////////////////////////////////////////////
   ...
   filter_class.filter_fields[1].field_mode = WP_PCE_FIELD_MODE_COMPARE_EXACT_MATCH;
   filter_class.filter_fields[1].mask_mode = WP_PCE_FIELD_MASK_NOT_USED;
   filter_class.filter_fields[1].field_id = WP_PCE_FIELD_ID_IPV6_NEXT_HEADER;
   filter_class.filter_fields[2].field_id = WP_PCE_FIELD_ID_LAST;

   PCE_filter[FILTER_SET_ICMPV6] =
      WP_PceFilterCreate (WP_WINPATH (DEFAULT_WPID),
                          WP_PCE_FILTER_CLASSIFICATION/*WP_PCE_FILTER_FORWARDING*/, &filter_class);
   ...
   PCE_filter[FILTER_SET_IGMPV6] =
      WP_PceFilterCreate (WP_WINPATH (DEFAULT_WPID),
                         WP_PCE_FILTER_CLASSIFICATION/*WP_PCE_FILTER_FORWARDING*/, &fwd_filter_cfg);
...
void WPE_CreateIPV6GroupBroadcastMatchPceRule (WP_U8 portid, WP_U8 * ipv6)
{
   WP_pce_rule_classification rule_cfg = { 0 };
   WP_handle port_handle = 0, agg_handle = 0, h_PCE_rule = 0;
...
#error USE_IPV6_FLOWAGG must be defined
   ...
   rule_cfg.filter_handle = PCE_filter[FILTER_SET_IPV6_L3_BROADCAST_CLASSIFICATION];
...
void WPE_CreatePassThroughPceRule (WP_U8 portid)
{
   WP_pce_rule_classification rule_cfg = { 0 };
   WP_handle port_handle = 0, agg_handle = 0, h_PCE_rule = 0;
   ...
   rule_cfg.filter_handle = PCE_filter[FILTER_SET_PASSTHROUGH];
...
void WPE_CreateICMPv6PceRule (WP_U8 portid, WP_U16 next_header, SEND_TO_WHERE flag)
{
   ...
   rule_cfg.rule_fields[1].value.ipv6_next_header  = next_header;
...
void WPE_CreateReservedMacPceRule (WP_U8 portid, WP_U8 * mac, WP_U8 *mac_high)
{
   WP_pce_rule_classification rule_cfg = { 0 };
   WP_handle port_handle = 0, agg_handle = 0, h_PCE_rule = 0;
   WP_U8 i = 0;
   ...
   case 1:
      port_handle = gbe[portid].bport_enet;
#if USE_DIFFERENT_FLOW_AGG   
#error USE_DIFFERENT_FLOW_AGG should NOT be defined
...
#error USE_IPV6_FLOW_AGG should be defined
   ...
   for (i = 0; i < 6; i ++)
      rule_cfg.rule_fields[1].value.mac_addr[i] = mac[i];



   rule_cfg.rule_fields[2].field_id = WP_PCE_FIELD_ID_MAC_DA;
   for (i = 0; i < 6; i ++)
      rule_cfg.rule_fields[2].value.mac_addr[i] = mac[i];

   rule_cfg.rule_fields[3].field_id = WP_PCE_FIELD_ID_MAC_DA;
   for (i = 0; i < 6; i ++)
      rule_cfg.rule_fields[3].value.mac_addr[i] = mac_high[i];
...
/*-------------------------------------------------------------------*\
   we redirect reserved MAC packet to agg_reserved1
\*-------------------------------------------------------------------*/
   rule_cfg.match_result[0].result_type = WP_PCE_RESULT_FLOW_AGG;
   rule_cfg.match_result[0].param.flow_agg.flow_aggregation =
      /*default_agg_host*/agg_handle;
...
void WT_PCERulesAdd (void)
{
   ...
   WP_U8 ipv6_2[16] = { 0xff, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
      0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
   };
...
{   
   ...
   WPE_CreateReservedMacPceRule (0, mac_addr_low_3, mac_addr_high_3);
#endif
}
...
void WPE_TrafficRemarkingEnableIPv6 (char *StrPrm,  
                                      WP_handle agg_enet_ipv6,
                                       WP_U8 tc_remark)
{
   WP_handle status;

   rout_agg_ipv6->traffic_class_remarking_mode = WP_ENABLE;
   rout_agg_ipv6->traffic_class_remarking_value = tc_remark;
   printf ("WPE_TrafficRemarkingEnableIPv6: we got value tc_remark(%d)\n", tc_remark); 
   status = WP_IwFlowAggregationModify(agg_enet_ipv6, WP_IW_FLOW_AGG_R_IPV6_MOD_TC, &rout_agg_ipv6);
...
void WPE_AddL3GroupMember (WP_handle agg, WP_handle bport)
{
   // WP_U8 i = 0;
   static WP_U8 index = 0;

      mc_member_config->txfunc = agg/*default_agg_host*/;  // flowAgg
      mc_member_config->iw_port = bport/*gbe[i].bport_enet*/;

      gbe[0].mc_member_handle[index] = WP_IwMcMemberAdd(gbe[1].mc_group_handle,
                                                                mc_member_config);
      App_TerminateOnError (gbe[0].mc_member_handle[index], "WP_IwMcMemberAdd()",__LINE__);
      index ++;
}
