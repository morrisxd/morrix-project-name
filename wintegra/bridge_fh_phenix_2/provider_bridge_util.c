...
WPE_vlan_edit_options vlan_options = 
{
   /* egress rule */
   { 
   ...
   WPE_gpe_brouter_pecs_flow_info brouter_pecs_flow_info[] = {
      ...
      /*  */
      {
       ...
       {0x11, 0x22, 0x33, 0x44, 0x55, 0x00}, /* mac_da */
       {0x66, 0x77, 0x88, 0x99, 0xaa, 0x00}, /* mac_sa */
       &vlan_options,                       /* vlan_edit_options */
...
/*BC filter*/
   ...
   App_TerminateOnError (PCE_filter[FILTER_SET_BC],
                         "WP_PceFilterCreate", __LINE__);
}

void WPE_CreateMacBindingFilter(void)
{
   WP_pce_filter_classification filter_class = { 0 };
...
/*BC filter*/
   ...
   PCE_filter[FILTER_SET_MAC_BINDING] =
      WP_PceFilterCreate (WP_WINPATH (DEFAULT_WPID),
                          WP_PCE_FILTER_CLASSIFICATION, &filter_class);
   App_TerminateOnError (PCE_filter[FILTER_SET_MAC_BINDING],
                         "WP_PceFilterCreate", __LINE__);
}

void WPE_CreateVlanCosFilter(void)
{
   WP_pce_filter_classification filter_class = { 0 };
...
/*BC filter*/
   ...
   PCE_filter[FILTER_SET_VLAN_COS] =
      WP_PceFilterCreate (WP_WINPATH (DEFAULT_WPID),
                          WP_PCE_FILTER_CLASSIFICATION, &filter_class);
   App_TerminateOnError (PCE_filter[FILTER_SET_VLAN_COS],
                         "WP_PceFilterCreate", __LINE__);
}
/*-------------------------------------------------*\
...
void WPE_CreatePceFilterSets (void)
{
   ...
   fs_level.filters[8] = PCE_filter[FILTER_SET_VLAN_COS];//Phenix
   fs_level.filters[9] = PCE_filter[FILTER_SET_MAC_BINDING];//Phenix
   fs_level.filters[10] = PCE_filter[FILTER_SET_FORWARDING];
   ...
   \*------------------------------------------------------------*/
    fs_level.filters[7] = PCE_filter[FILTER_SET_VLAN_COS];//Phenix
    fs_level.filters[8] = PCE_filter[FILTER_SET_MAC_BINDING];//Phenix
   fs_level.filters[9] = PCE_filter[FILTER_SET_FORWARDING];
   ...
   fs_level.filter_set_level = 0;
   fs_level.next_filter_set = WP_UNUSED;
   fs_level.filters[0] = PCE_filter[FILTER_SET_VLAN_COS];//Phenix
   fs_level.filters[1] = WP_UNUSED;
   filter_set_vlancos =
   WP_PceFilterSetCreate (WP_WINPATH (DEFAULT_WPID), &fs_level);
   App_TerminateOnError (filter_set_vlancos, "WP_PceFilterSetCreate",
                         __LINE__);

         /* filter set without learning */
   fs_level.filter_set_level = 0;
   fs_level.next_filter_set = WP_UNUSED;
   fs_level.filters[0] = PCE_filter[FILTER_SET_MAC_BINDING];//Phenix
   fs_level.filters[1] = WP_UNUSED;
   filter_set_macbinging =
   WP_PceFilterSetCreate (WP_WINPATH (DEFAULT_WPID), &fs_level);
...
static void WPE_CreateUnknownUnicastPceRulesWithPolicer (WP_U16 vlan,
                                              WPE_vlan_group * groupInfo)
{
   ...
   if (!WP_ERROR_P (h_PCE_rule))
   {
      groupInfo->PCE_rule_handle_policer = h_PCE_rule;
   }
   else if (WP_ERROR (h_PCE_rule) == WP_ERR_PCE_RULE_ALREADY_EXISTS)
   {
                                              ...
                                              WP_U16 out_vlan_tag)
{
   ...
   rule_cfg.rule_fields[0].value.vlan_tag = vlan_tag;//0xE000;
...
void WPE_CreateMacBindingPceRule (WP_U8 portid, WP_U8 * mac)
{
   WP_pce_rule_classification rule_cfg = { 0 };
...
void WPE_CreateUnknownUnicastGroupWithPolicer (WP_U16 vlan, WP_U32 port)
{
   WP_U32 i = 0;
   ...
   WPE_CreateUnknownUnicastPceRulesWithPolicer(vlan, &vlan_groups[i]);//Phenix
