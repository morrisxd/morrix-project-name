...
APP_gbe_enet_t gbe[NR_GBE] = { {WP_PORT_ENET8,}
, {WP_PORT_ENET7,}
};
...
WP_pce_learned_forwarding_rule
   learned_rules_list[WP_PCE_LEARNING_QUEUE_MAX_SIZE];
...
WP_int_queue_table int_queue_tables = { {{65500, 1}
                                         , {10, 1}
                                         , {10, 1}
                                         , {10, 1}
                                         }
 ...
 * Return val   : None
 *****************************************************************************/
WP_enet_filters rx_filters[1] ={{   WP_ENET_FILTER_PROCESS,   WP_ENET_FILTER_PROCESS,   WP_ENET_FILTER_PROCESS}};
...
void App_PortsDevicesCreate (void)
{
   ...
   {
      gbe[ii].port_enet =
         WP_PortCreate (WP_WINPATH (0), gbe[ii].hw_port,
                        &enet_port_config);
      App_TerminateOnError (gbe[ii].port_enet, "WP_PortCreate() Fast ENET",
                            __LINE__);

   enet_dev_config.extended_params = NULL;
      gbe[ii].dev_enet =
         WP_DeviceCreate (gbe[ii].port_enet, WP_PHY (0), WP_DEVICE_ENET,
                          &enet_dev_config);
      App_TerminateOnError (gbe[ii].dev_enet, "WP_DeviceCreate() Enet",
                            __LINE__);

   enet_dev_config.extended_params = enet_dev_ex;
   status = WP_DeviceModify(gbe[ii].dev_enet,
                            WP_DEV_MOD_ENET_PHYSTATMODE |
                            WP_DEV_MOD_ENET_TX_STATMODE |
                            WP_DEV_MOD_ENET_RX_STATMODE,
                            &enet_dev_config);
   App_TerminateOnError(status, "WP_DeviceModify() ENET",__LINE__);
...
iw_pool = pool_2048;
   ...
   qniw =
      WP_QNodeCreate (WP_WINPATH (DEFAULT_WPID),
                      WP_QNODE_IWQ | WP_QNODE_OPT_FMU |
                      WP_QNODE_OPT_DEDICATED_RX_HWQ, qn_iw);
   ...
   WP_qnode_iw_group group_highp[1] = {
      {
       ...
       /*bd_pool_weight *//*{NULL,NULL} */ {&weight_params[0],
                                            &weight_params[0]}
       }
   };
   ...
   pool_2048_2 =
      WP_PoolCreate (WP_WINPATH (DEFAULT_WPID), WP_pool_iwbuffer,
                     buffer_data_2048);
   App_TerminateOnError (pool_2048_2, "WP_PoolCreate() pool_2048",
                         __LINE__);
   ...
   pool_2048_3 =
      WP_PoolCreate (WP_WINPATH (DEFAULT_WPID), WP_pool_iwbuffer,
                     buffer_data_2048);
   App_TerminateOnError (pool_2048_3, "WP_PoolCreate() pool_2048_2",
                         __LINE__);

   bd_conf->buffer_pool = pool_2048_2;
   bd_conf->bd_bus = WP_BUS_INTERNAL;
   bd_pool[0] =
      WP_PoolCreate (WP_WINPATH (DEFAULT_WPID), WP_POOL_IW_BD, bd_conf);
   ...
   qniw =
      WP_QNodeCreate (WP_WINPATH (DEFAULT_WPID), WP_QNODE_IW_HIERARCHICAL,
                      iw_hierq);
...
void App_ModulesInit (void)
{
   ...
   status =
      WP_ModuleInit (WP_SYSHANDLE (DEFAULT_WPID), WP_WDDI_MODULE_PCE,
                     &pce_init);
   App_TerminateOnError (status, "WPI_PceModuleInit()", __LINE__);

   /* GPE module initialization */
   status =
      WP_ModuleInit (WP_SYSHANDLE (DEFAULT_WPID), WP_WDDI_MODULE_GPE,
                     &gpe_init);
   App_TerminateOnError (status, "WPI_GpeModuleInit()", __LINE__);

   /* PCE policer module initialiaztion */
   status =
      WP_ModuleInit (WP_SYSHANDLE (DEFAULT_WPID), WP_WDDI_MODULE_POLICER,
                     policer_module_config);
...
void App_EventsInit (void)
{
   ...
   status =
      WP_ControlRegister (WP_EVENT_IWGP_LEARNING_QUEUE,
                          &F_MyAppIndicateEventIwgpLearning);
   App_TerminateOnError (status, "WP_ControlRegister()", __LINE__);

   /* Host RX interrupt */
   status =
      WP_ControlRegister (WP_EVENT_RX_INDICATE, &WPE_Receive_HostData_IRQ);
   App_TerminateOnError (status, "WP_ControlRegister()", __LINE__);
...
void App_ChannelsCreate (void)
{
   ...
   {
      gbe[ii].rx_chan_enet =
         WP_ChannelCreate (0, gbe[ii].dev_enet, qniw, WP_CH_RX, WP_ENET,
                           &enet_ch_config);
      App_TerminateOnError (gbe[ii].rx_chan_enet,
                            "WP_Channel_Create() Fast RX", __LINE__);

      gbe[ii].tx_chan_enet =
         WP_ChannelCreate (0, gbe[ii].dev_enet, qniw, WP_CH_TX, WP_ENET,
                           &enet_ch_config);
      App_TerminateOnError (gbe[ii].tx_chan_enet,
                            "WP_Channel_Create() Fast TX", __LINE__);

      status =
         WP_IwTxBindingCreate (gbe[ii].tx_chan_enet, WP_IW_TX_BINDING,
                               &tx_binding_enet_config);
      App_TerminateOnError (status, "WP_IwTxBindingCreate Fast TX",
                            __LINE__);
   }
...
void App_IwSystemInit (void)
{
   ...
   dl_general_iwsys_bridge =
      WP_IwSystemCreate (WP_WINPATH (DEFAULT_WPID),
                         ...
                         &iw_sys_bridging[0]);
   App_TerminateOnError (dl_general_iwsys_bridge, "WP_IwSystemCreate",
                         __LINE__);
...
void App_PceInterfaceCreate (void)
{
   ...
   //Create BC  filter
  WPE_CreateBCFilter();//Phenix
  
   // Create all filters
   WPE_CreatePceFilters ();

 //create VLAN COS filter
  WPE_CreateVlanCosFilter();//Phenix
  WPE_CreateMacBindingFilter();
...
void *App_LearningThread (void *arg)
{
   ...
   while (1)
   {
      ...
      status = WP_PceLearningQueueService (WP_WINPATH (DEFAULT_WPID),
                                           ...
                                           size, &rules_returned);
      App_TerminateOnError (status, "WP_PceLearningQueueService",
                            __LINE__);
      ...
      {
         memcpy (&rule_fwd, &(learned_rules_list[ii].fwd_rule),
                 sizeof (WP_pce_rule_forwarding));

         for (i = 0; i < NR_GBE; i++)
         {
            if (gbe[i].bport_enet ==
                rule_fwd.match_result[0].param.iw_port.iw_port_handle)
            {
               break;
            }
         }
         ...
         if ((i < NR_GBE)
             && (gbe[i].cur_learned_mac < gbe[i].max_learned_mac))
            gbe[i].cur_learned_mac++;
         else
         {
            printf ("-->PORT %d full, max %d\n", i,
                    gbe[i].max_learned_mac);
            continue;
         }
         ...
         {
            if (WP_ERROR (PCE_rule_handle) ==
                WP_ERR_PCE_RULE_ALREADY_EXISTS)
            {
               continue;
            }
            else
            {
               App_TerminateOnError (PCE_rule_handle, "WP_PceRuleCreate",
                                     __LINE__);
            }
         }
      ...
      if (PMTest_enable)
      {
         ...
         {
            printf
               ("Performance Test: learnt %d rules, in Time delta (in micro sec) %d\n",
...
void *App_AgingFunc (void *arg)
{
   ...
   {
      aging_iterator_handle =
         WP_PceIteratorCreate (WP_WINPATH (DEFAULT_WPID),
                               WP_PCE_ITERATOR_TYPE_AGED_RULE,
                               (void *) NULL);

      App_TerminateOnError (aging_iterator_handle, "WP_PceIteratorCreate",
                            __LINE__);
   }
   ...
   if (!WP_ERROR_P (rule_handle))
   {
      ...
      printf
         ("Aging : Already aging out %d rules, in Time delta (in micro sec) %d\n",
          aged_rules, delta);
   }
   else
   {
