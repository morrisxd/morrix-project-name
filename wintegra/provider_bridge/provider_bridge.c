...
#include "fiberhome.h"
char *port2index[] = {"ENET13",	 "ENET14", "ENET15",  "ENET16",  "ENET3", "ENET5",  "ENET4",  "ENET6",  "ENET8",  "ENET7",  "XGI1"};
...
int main(int argc, char *argv[])
{
	...
	WPE_CreateUnknownUnicastGroup(101,8);
	WPE_CreateLearningFlowAggPceRule(8,101);
	WPE_CreateUnknownUnicastGroup(101,9);
	WPE_CreateLearningFlowAggPceRule(9,101);
	printf ("auto build vlan & port finished\n");
...
void App_PortsDevicesCreate(void)
{
        ...
        WP_port_enet enet_port[1] =
                {
                        {
                                /* pkt_limits */     {MAX_NUM_OF_CHANNELS, MAX_NUM_OF_CHANNELS, 0 ,0 },
        ...
        for (ii = 0; ii < NR_GBE; ii++) 
        {
#if MORRIS_CHANGE_INTERFACE_TYPE
	   if (8 == ii)
	   {
                // create GBE physical port
                enet_port_config.interface_mode = WP_ENET_1000_BASE_X_AN;
		printf ("create ENET(%3d) as (%16s)\n", ii, "1000_BASE_X_AN");
...
#if MORRIS_DISABLE_PORT_MODIFY
#else
                // change to packet base mode
                enet_device_extended_config->tx_force_pb = WP_ENET_TX_USE_PB_FORCE;
                enet_dev_config.max_tx_channels = 1;
