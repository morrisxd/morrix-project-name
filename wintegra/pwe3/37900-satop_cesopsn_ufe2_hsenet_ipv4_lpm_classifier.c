...
#define WTI_CI_TESTING_MODE_UFE4  (WT_TRIAL_MASK == 1<<4 || WT_TRIAL_MASK == 1<<5 || WT_TRIAL_MASK == 1<<1)  //DCR/ACR/                       
...
/*This is 37900ote_master */
#if DEFAULT_DEMO_MODE_REGRESSION
#define WTI_CESOP_TDI                               0                 /* use of TDM I/F */
#define WTI_CESOP_CLOCK_RECOVERY_ENABLE             0                 /* enable clock recovery module */
#define WTI_CESOP_RX_TIMING_ENABLE                  0                 /* enable Rx timing direction */
#define WTI_CESOP_REGRESSION_TEST                   1                 /* when '1' running in regression mode*/
#define A_AUTOMATION                                0                 /* Should set to 1 for automation */
#ifdef DEFAULT_DEMO_MODE
#undef DEFAULT_DEMO_MODE
#endif
#define DEFAULT_DEMO_MODE 1
#endif

/*****************************************************************************/
/*                       Internal Include                                    */
/*****************************************************************************/
#if ((WT_TRIAL_MASK == (WT_TRIAL_MASK & 1 << 10)) || (WT_TRIAL_MASK == (WT_TRIAL_MASK & 1 << 11)) || (WT_TRIAL_MASK == (WT_TRIAL_MASK & 1 << 12)))
#define OTE_AUTOMATION
#if (WT_TRIAL_MASK == (WT_TRIAL_MASK & 1 << 11)|| WT_TRIAL_MASK == (WT_TRIAL_MASK & 1 << 12))
#define WTI_FRAMER_OTE_TESTS 1
#endif
#endif
...
#if (DEFAULT_DEMO_MODE)         /* This is 37900b */
#define WTI_EMPHY_PORT                              WP_PORT_UPI1
#define SERIAL_X_CPLD_INTERRUPT_MASK   (~WPX_FPGA_INTR_SERIAL_1_CONNECTOR & 0x7f)
#elif WT_TRIAL_MASK & (1 << 1)    /* This is 37900c */
#define WTI_EMPHY_PORT                              WP_PORT_UPI3
#define SERIAL_X_CPLD_INTERRUPT_MASK   (~WPX_FPGA_INTR_SERIAL_3_CONNECTOR & 0x7f)
#else                             /* This is 37900d */
...
#endif // WTI_XGI_MODE
#else // !DEFAULT_DEMO_MODE
#define WTI_ENET_MODE                               WP_ENET_LOOPBACK // Loopback for all automatic modes (cjeckins)
#endif
...
#elif defined WP_HW_WINPATH4
#define WTI_ENET_PORT                               WP_PORT_ENET3      /* ENET port */
...
#define WTI_TDM_MODE                                WP_TDM_LOOPBACK_NO_EXT_CLK //WP_TDM_NORMAL
#else
#define WTI_TDM_MODE                                WP_TDM_NORMAL //WP_TDM_LOOPBACK_NO_EXT_CLK
#endif
...
#define WTI_TDI_TS_CLOCK_SOURCE                     WP_CLK_SRC_INT_SYS /* WP4 tdi timestamp source     */
#else
...
#ifdef WP_HW_WINPATH4
//#undef WTI_WG_TS2_CLOCK_SOURCE
//#define WTI_WG_TS2_CLOCK_SOURCE WP_CLK_SRC_INT_SYS
#endif
...
#define N_IW_PSN2TDM_QUEUE_BUFFER_SIZE              600 /* regular tests need only 512. 800 is for 30/24
                                                           slots tests in CAS mode */
#define N_IW_PSN2TDM_QUEUE_BUFFER_SIZE_CEP          1100 /* regular tests need only 512. 800 is for 30/24
                                                           slots tests in CAS mode */
#if WTI_8K_CH_SETUP
...
#define N_IW_TDM2PSN_QUEUE_BUFFER_SIZE              600 /* regular tests need only 512. 800 is for 30/24
                                                           slots tests in CAS mode */
#define N_IW_TDM2PSN_QUEUE_BUFFER_SIZE_CEP          1100 /* regular tests need only 512. 800 is for 30/24
                                                           slots tests in CAS mode */
...
extern void CLI_F_EnetDeviceEnableDisable                  (char *StrPrm);
extern void CLI_F_SetRandomUdPattern                       (char *StrPrm); 
...
extern void CLI_F_LAisTowardsLine                          (char *StrPrm);
extern void CLI_F_HoAisTowardsLine                         (char *StrPrm);
extern void CLI_F_VC3LoAisTowardsLine                      (char *StrPrm);
extern void CLI_F_LoAisTowardsLine                         (char *StrPrm);
...
void CLI_F_FlexmuxSetFraming                               (char *StrPrm);

/* Framer omnispy debug functions */
extern void CLI_F_UfeFramerDebugEnable                     (char *StrPrm);
extern void CLI_F_UfeFramerDebugDisable                    (char *StrPrm);
extern void CLI_F_UfeFramerDebugShowScreen                 (char *StrPrm);
extern void CLI_F_UfeFramerDebugSetScreen                  (char *StrPrm);
extern void CLI_F_UfeFramerDebugNextPage                   (char *StrPrm);
extern void CLI_F_UfeFramerDebugPrevPage                   (char *StrPrm);
extern void CLI_F_UfeFramerDebugSetAddress                 (char *StrPrm);
extern void CLI_F_UfeFramerDebugSetHighOrder               (char *StrPrm);
extern void CLI_F_UfeFramerDebugSetLowOrder                (char *StrPrm);
extern void CLI_F_UfeFramerDebugVariableAddressGet         (char *StrPrm);
...
extern void CLI_F_UFETerminateRxTrafficAndCheck            (char *StrPrm);

extern void CLI_F_UfeLineCasGetValue                        (char *StrPrm);
...
extern void CLI_F_SonetSdhT1AlternateNoCas                 (char *StrPrm);
extern void CLI_F_SonetSdhE1AlternateNoCas                 (char *StrPrm);
extern void CLI_F_SdhVC3_HI_Cep                            (char *StrPrm);
extern void CLI_F_SonetSTS1_SPE_Cep                        (char *StrPrm);
extern void CLI_F_SdhVc3_HI_Cep_E3_Combined                (char *StrPrm);
extern void VC3C_CEP_To_E1_CES                             (char *StrPrm);
extern void CLI_F_VC4_CEP_T1_T3_VC4C                       (char *StrPrm);

extern void CLI_F_SonetSdhE1FramedWithCasEnableDisable     (char *StrPrm);
extern void CLI_F_SonetSdhT1FramedWithCasEnableDisable     (char *StrPrm);
extern void CLI_F_SonetSdhE1FramedWithCasAndE1Unframed     (char *StrPrm);
extern void CLI_F_SonetSdhT1FramedWithCasAndT1Unframed     (char *StrPrm);
extern void CLI_F_SonetSdhE1FramedWithCasCreateDelete      (char *StrPrm);
extern void CLI_F_SonetSdhT1FramedWithCasCreateDelete      (char *StrPrm);
extern void CLI_F_SonetSdhE1FramedWithCasAlternateFramedE1 (char *StrPrm);
extern void CLI_F_SonetSdhT1FramedWithCasAlternateFramedT1 (char *StrPrm);
extern void CLI_F_SonetSdhE1T1CombinedWithCas              (char *StrPrm);
extern void CLI_F_SonetSdh_E3_E1_On_Same_PortWithCas       (char *StrPrm);
extern void CLI_F_SonetSdh_DS3_T1_On_The_Same_PortWithCas  (char *StrPrm);
...
#else
Y_MenuEntry V_UfeFramerDebugFeature []=
{
   {10 ,TRUE, "Debug Framer",                                                               {(Y_MenuEntry*) V_UfeSetup}},
   {K_Leaf, TRUE, " -> Enable Debug Feature[flexmux_id]",                                              {(Y_MnuLeafP) CLI_F_UfeFramerDebugEnable}},
   {K_Leaf, TRUE, " -> Disable Debug Feature[flexmux_id]",                                             {(Y_MnuLeafP) CLI_F_UfeFramerDebugDisable}},
   {K_Leaf, TRUE, " -> Show the screen[flexmux_id]",                                                   {(Y_MnuLeafP) CLI_F_UfeFramerDebugShowScreen}},
   {K_Leaf, TRUE, " -> Screen Type: [flexmux_id][1-Section, 2-HO Overhaed, 3-LO Overhead,4-PRBS,5-POINTERS,[6]-MEMORY,7-metadata,8-Mailbox,9-Performance]", {(Y_MnuLeafP) CLI_F_UfeFramerDebugSetScreen}},
   {K_Leaf, TRUE, " -> Go to next page[flexmux_id]",                                                   {(Y_MnuLeafP) CLI_F_UfeFramerDebugNextPage}},
   {K_Leaf, TRUE, " -> Go to prev page[flexmux_id]",                                                   {(Y_MnuLeafP) CLI_F_UfeFramerDebugPrevPage}},
   {K_Leaf, TRUE, " -> Set Address[flexmux_id]",                                                   {(Y_MnuLeafP) CLI_F_UfeFramerDebugSetAddress}},
   {K_Leaf, TRUE, " -> Set High Order[flexmux_id][stm4][stm1][stm0][HO type- (0-VC3,1-VC4)]",{(Y_MnuLeafP) CLI_F_UfeFramerDebugSetHighOrder}},
   {K_Leaf, TRUE, " -> Set Low Order[flexmux_id][stm4][stm1][stm0][tug2][tu][HO type- (0-VC12,1-VC11)]",{(Y_MnuLeafP) CLI_F_UfeFramerDebugSetLowOrder}},
   {K_Leaf, TRUE, " -> Variable Address Get[flexmux_id]",{(Y_MnuLeafP) CLI_F_UfeFramerDebugVariableAddressGet}},
...
const Y_MenuEntry V_UfeSetup []=
{
   ...
   {K_Leaf, TRUE, " -> UFE framer poke (write)  [flexmux_id][Address,value]",         {(Y_MnuLeafP)CLI_F_FlexmuxPoke }}, /* should be the 15 entry for DS3/E3 automation */
   ...
   {K_Leaf, TRUE, " -> UFE Rx traffic termination and check[AUG-1]",   {(Y_MnuLeafP)CLI_F_UFETerminateRxTrafficAndCheck}},
   ...
   {K_Leaf, TRUE, " -> CLI_F_GetAlarmState",   {(Y_MnuLeafP)CLI_F_GetAlarmState}},
#if WTI_FLEXMUX_ENABLE_PERFORMANCE_MONITORING
   {K_Leaf, TRUE, " -> CLI_F_Enable_PM_Print[0-Port,1-HOP,2-LOP,3-PDH][pm point][0-disable,1-enable]",   {(Y_MnuLeafP)CLI_F_Enable_PM_Print}},
#endif
   {K_Leaf, TRUE, " -> TDM loopback mode [0-NO LPBK,1-SYSTEM LPBK,2-LINE LPBK][line_index]", {(Y_MnuLeafP) CLI_F_UfeTdmLoopbackMode}},
   {K_Leaf, TRUE, " -> CLI_F_Alarm_status_check [1 -print data]",   {(Y_MnuLeafP)CLI_F_AlarmsStatusCheck}},
   {K_Leaf, TRUE, " -> CLI_F_FlexmuxSetFraming[flexmux id][1-SF_D4/PCM31, 2-ESF/PCM31CRC][number of lines (1-336)]", {(Y_MnuLeafP) CLI_F_FlexmuxSetFraming}},      /* should be the 31 entry for OTE automation purposes */
   {K_Menu, TRUE, " -> Ufe Framer Debug",   {(Y_MenuEntry*) V_UfeFramerDebugFeature}},
#endif
   {K_Leaf, TRUE, " -> CLI_F_UfeLineCasGetValue [0-RX value,1-Tx Value][line_index]", {(Y_MnuLeafP) CLI_F_UfeLineCasGetValue}},  /*Shold be the 33 entry for OTE automation */
...
{
   {11 ,TRUE, "PW modify",                                                                   {(Y_MenuEntry*) V_MainMenu}},
   ...
   {K_Leaf, TRUE, " -> Enable/Disable Enet Device[0-disable,1-enable]", {(Y_MnuLeafP) CLI_F_EnetDeviceEnableDisable}},
   {K_Leaf, TRUE, " -> Set Random UDPattern", {(Y_MnuLeafP) CLI_F_SetRandomUdPattern}},
};
...
const Y_MenuEntry V_PreDefinedSystemConfigurations []=
{
...
#if WTI_CESOP_MPLS_IW
#if WTI_CESOP_CLOCK_RECOVERY_ENABLE
   { 80, TRUE, "System configurations: UFE4 Scenarios",{(Y_MenuEntry*) V_MainMenu}},
#else
   { 70, TRUE, "System configurations: UFE4 Scenarios",{(Y_MenuEntry*) V_MainMenu}},
#endif /* WTI_CESOP_CLOCK_RECOVERY_ENABLE */
   ...
   {K_Leaf, TRUE, " -> Clear Channel, Sonet/Sdh. [0 - SDH/VC44C, 1- SDH/VC4, 2- SONET/STS12, 3- SONET/STS3]",              {(Y_MnuLeafP) CLI_F_SonetSdhClearChannel}},/* 10*/
   ...
   {K_Leaf, TRUE, " -> E1, Sonet/Sdh, Phy Enable/Disable. [0- SDH, 1- SONET][0-load lines,1-disable,2-enable,3-enable & disable in loop][0-Cas Disable, 1-Cas Enable]",                            {(Y_MnuLeafP) CLI_F_SonetSdhE1PhyEnableDisable}},
   ...
   {K_Leaf, TRUE, " -> E1, Sonet/Sdh, error_4_e1_on_1_tug2. [0- SDH, 1- SONET]",                          {(Y_MnuLeafP) CLI_F_error_4_e1_on_1_tug2}},/* 20*/
   ...
   {K_Leaf, TRUE, " -> DS3, Sonet/Sdh, Create/Delete. [0 - SDH, 1- SONET][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",              {(Y_MnuLeafP) CLI_F_SonetSdhDS3CreateDelete}},/*30*/
   ...
   {K_Leaf, TRUE, " -> DS3VFPT1 Combined, Sonet/Sdh. [0 - SDH, 1- SONET][physical conf: 0-oc12,1-4oc3][in SDH:0-AU3,1-AU4][T1 CAS: 0-Disable, 1-Enable]",  {(Y_MnuLeafP) CLI_F_SonetSdh3DS31VFP3DS328T1}},
   ...
   {K_Leaf, TRUE, " -> E3 Protection Switching, Sonet/Sdh, [0 - SDH, 1- SONET][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",              {(Y_MnuLeafP) CLI_F_SonetSdhE3ProtectionSwitch}},/* 40*/
   ...
   {K_Leaf, TRUE, " -> CR 103, T1, Framed, No Cas. Multiple PWs per line [NUM_OF_LINES][0-SDH, 1-SONET, 2-CAD][#PWs/line][recovered PW]",   {(Y_MnuLeafP) CLI_F_CR_103_SonetSdh_T1FramedNoCas_MultiplePwPerLine}},/*50*/
   ...
   {K_Leaf, TRUE, " -> E1, Sonet/Sdh/CAD, Alternate, Without Cas. [0- SDH, 1- SONET, 2- CAD][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhE1AlternateNoCas}},/* this entry should be entry 54 for OTE purposes */
   {K_Leaf, TRUE, " -> T1, Sonet/Sdh/CAD, Alternate, Without Cas. [0- SDH, 1- SONET, 2- CAD][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhT1AlternateNoCas}},
   {K_Leaf, TRUE, " -> VC3 HI, Sdh,[physical conf: 0-oc12,1-4oc3,2-2oc3][NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SdhVC3_HI_Cep}},   
   {K_Leaf, TRUE, " -> STS1 SPE, SONET,[physical conf: 0-oc12,1-4oc3,2-2oc3][NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSTS1_SPE_Cep}},   
   {K_Leaf, TRUE, " -> VC3 HI CEP + E3 combined ,SDH,[physical conf: 0-oc12,1-4oc3][NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SdhVc3_HI_Cep_E3_Combined}},   
   {K_Leaf, TRUE, " -> T1, Sonet/Sdh, EnableDisable, With Cas. [0- SDH, 1- SONET, 2- CAD][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhT1FramedWithCasEnableDisable}},
   {K_Leaf, TRUE, " -> E1, Sonet/Sdh, CAS framed alternate with UNFRAMED. [0- SDH, 1- SONET][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhE1FramedWithCasAndE1Unframed}},/*60 (without CR) */
   {K_Leaf, TRUE, " -> T1, Sonet/Sdh, CAS framed alternate with UNFRAMED. [0- SDH, 1- SONET][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhT1FramedWithCasAndT1Unframed}},
   {K_Leaf, TRUE, " -> E1, Sonet/Sdh, CreateDelete, With Cas. [0- SDH, 1- SONET, 2- CAD][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES][0-Finish in CAS,1-Finish in Framed]",               {(Y_MnuLeafP) CLI_F_SonetSdhE1FramedWithCasCreateDelete}},
   {K_Leaf, TRUE, " -> T1, Sonet/Sdh, CreateDelete, With Cas. [0- SDH, 1- SONET, 2- CAD][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhT1FramedWithCasCreateDelete}},
   {K_Leaf, TRUE, " -> E1, Sonet/Sdh, CAS framed alternate with No-CAS framed. [0- SDH, 1- SONET, 2- CAD][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhE1FramedWithCasAlternateFramedE1}},
   {K_Leaf, TRUE, " -> T1, Sonet/Sdh, CAS framed alternate with No-CAS framed. [0- SDH, 1- SONET, 2- CAD][physical conf: 0-oc12,1-4oc3,2-2oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhT1FramedWithCasAlternateFramedT1}},
   {K_Leaf, TRUE, " -> E1T1 Combined, Sonet/Sdh, With CAS. [0- SDH, 1- SONET, 2- CAD] [NUM_OF_LINES]",               {(Y_MnuLeafP) CLI_F_SonetSdhE1T1CombinedWithCas}},
   {K_Leaf, TRUE, " -> E3 & E1 On The Same Port, Sonet/Sdh. [0 - SDH, 1- SONET][physical conf: 0-oc12,1-4oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",    {(Y_MnuLeafP) CLI_F_SonetSdh_E3_E1_On_Same_PortWithCas}},
   {K_Leaf, TRUE, " -> DS3 & T1 On The Same Port, Sonet/Sdh. [0 - SDH, 1- SONET][physical conf: 0-oc12,1-4oc3][in SDH:0-AU3,1-AU4] [NUM_OF_LINES]",    {(Y_MnuLeafP) CLI_F_SonetSdh_DS3_T1_On_The_Same_PortWithCas}},
   {K_Leaf, TRUE, " -> VC3c/STS1_SPE CEP to VC12/VT2 CES [0- SDH, 1- SONET],[physical conf: 0-oc12,1-4oc3]",         {(Y_MnuLeafP)VC3C_CEP_To_E1_CES }},           
   {K_Leaf, TRUE, " -> VC4 CEP+84 T1's+3 T3+VC4C [0- SDH, 1- SONET],[physical conf: 0-oc12,1-4oc3]",               {(Y_MnuLeafP) CLI_F_VC4_CEP_T1_T3_VC4C }},   
...
#else
   {K_Leaf, TRUE, " -> PSN->TDM IW port statistics [0-show stats, 1-reset stats]",                                                                    {(Y_MnuLeafP)   CLI_F_IwPortStatistics}},
#if WTI_CESOP_TDI
#if (WTI_4_ENET_DEVICES)
...
const Y_MenuEntry V_Host []=
{
   ...
   {K_Leaf, TRUE, " -> Enable LAIS towards line on [device] [port_num(0-3) [enable ==> 1, disable ==> 0]", {(Y_MnuLeafP) CLI_F_LAisTowardsLine}},
   {K_Leaf, TRUE, " -> Enable HO AIS towards line on [device] ho_type(0-STS1,1-VC3,2-VC4,3-VC44C, 4-OC12c) [ho(stm4_sts12, stm1_sts3, stm0_sts1) [enable ==> 1, disable ==> 0]", {(Y_MnuLeafP) CLI_F_HoAisTowardsLine}},
   {K_Leaf, TRUE, " -> Enable VC3 LO AIS towards line on [device] [VC3 LO(stm4_sts12, stm1_sts3, stm0_sts1) [enable ==> 1, disable ==> 0]", {(Y_MnuLeafP) CLI_F_VC3LoAisTowardsLine}},
   {K_Leaf, TRUE, " -> Enable LO AIS towards line on [device] lo_type(0-VT15,1-VT2,2-VC11,3-VC12)[lo(stm4_sts12, stm1_sts3, stm0_sts1,tug2_vtg,tu_vt) [enable ==> 1, disable ==> 0]", {(Y_MnuLeafP) CLI_F_LoAisTowardsLine}}
#endif
#endif
};
 ...
 * Return Val :
 ***************************************************************/
void CLI_F_UfeLineCasGetValue(char *StrPrm)
{
   WP_U32 res, cmd, val,first_line_index, line_index,cas_line_index,range;
   WP_status status;
   WUFE_UFE412_line_cas_value cas_value;
   res = sscanf(StrPrm, "%d %d %d %d", &cmd, &val, &line_index,&range);
   ...
   if (val != 0 && val != 1 && val != 2)
   {
      WTI_TerminatePrintError("CLI_F_UfeLineCasGetRx - invalid type", __LINE__);
   }

   if (line_index >= 336)
   {
      WTI_TerminatePrintError("CLI_F_UfeLineCasGetRx - invalid line_index", __LINE__);
      return;
   }
   ...
   first_line_index=line_index;
   cas_line_index = line_index;
   while(cas_line_index < first_line_index + range)
   {
       if((the_system->ufe.line_params[line_index].transfer_type == WUFE_SONET_TYPE_E1 ||
          the_system->ufe.line_params[line_index].transfer_type == WUFE_SDH_TYPE_E1 ||
           the_system->ufe.line_params[line_index].transfer_type == 0) &&
       ...
       else
       {
           if (val == 0)
           {
               status = WUFE_LineCasGetRx(the_system->ufe.line_handle[line_index],&cas_value );
               WTI_TerminateOnError(status , "WUFE_LineCasGetRx",__LINE__);
           }
           
           else if ( val == 1)
           {
               status = WUFE_LineCasGetTx(the_system->ufe.line_handle[line_index], &cas_value);
               WTI_TerminateOnError(status , "WUFE_LineCasGetTx",__LINE__);
           }
           printf("line_index %d cas 00: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_0_3);
           printf("line_index %d cas 01: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_4_7);
           printf("line_index %d cas 02: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_8_11);
           printf("line_index %d cas 03: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_12_15);
           printf("line_index %d cas 04: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_16_19);
           printf("line_index %d cas 05: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_20_23);
           printf("line_index %d cas 06: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_24_27);
           printf("line_index %d cas 07: 0x%04x\n",line_index,cas_value.ufe_line_cas_slot_28_31);
           line_index++;
           cas_line_index++;
       }
   }
...
void CLI_F_Pwe3ChannelShowInfo(char *StrPrm)
{
   ...
   printf("cep_mode: ", trans_ch_config.cep_mode);
   if (WP_ENABLE == trans_ch_config.cep_mode)
      printf("WP_ENABLE\n");
   else
      printf("WP_DISABLE\n");      
...
{
   WP_U32 res, cmd, val1, e1t1_index, pw_index, num_slots=0, i, rx_dataunit_size, tx_dataunit_size, cep_mode;
   ...
   if (num_of_params == 6)
   {
      res = sscanf(StrPrm, "%d %d %d %d %d %d",
                   &cmd, &e1t1_index, &pw_index, &rx_dataunit_size, &tx_dataunit_size, &cep_mode);
      if (res != 6)
      {
         WTI_TerminatePrintError("Invalid number of parameters", __LINE__);
         return;
            ...
            val1 == WUFE_SONET_TYPE_E1 || val1 == WUFE_SDH_TYPE_E1)/* E1 */
   {
      num_slots = WTI_E1_SLOTS;
            ...
            val1 == WUFE_SONET_TYPE_DS3 || val1 == WUFE_SDH_TYPE_DS3 ||
            val1 == WUFE_SONET_TYPE_E3 || val1 == WUFE_SDH_TYPE_E3 ||
            val1 == WUFE_SONET_TYPE_STS1_SPE || val1 == WUFE_SDH_TYPE_VC3 || val1 == WUFE_SDH_TYPE_TU3)
   {
      num_slots = WTI_DS3_SLOTS;
   }
   ...
   {
      if (WUFE_ENABLE == cep_mode)
         pw_config.mode = WP_EMPHY_TRANS_CEP;
      else
         pw_config.mode = WP_EMPHY_TRANS_UNFRAMED | WP_EMPHY_TRANS_E3;

   }
   else if(val1 == WUFE_SDH_TYPE_VC44C || val1 == WUFE_SONET_TYPE_STS12)
   {
      if (WUFE_ENABLE == cep_mode)
         pw_config.mode = WP_EMPHY_TRANS_CEP;
      else 
   ...
   else if( val1 == WUFE_SDH_TYPE_VC3 || val1 == WUFE_SONET_TYPE_STS1_SPE || val1 == WUFE_SDH_TYPE_TU3)
   {
      if (WUFE_ENABLE == cep_mode)
         pw_config.mode = WP_EMPHY_TRANS_CEP;
      else
   ...
   if (num_of_params == 4)
   {
      pw_config.cep_mode = cep_mode = WUFE_DISABLE;
      pw_config.phy_index = total_number_of_pw;
   }   
   else if (num_of_params == 5)
   {   
...
void CLI_F_UFETerminateRxTrafficAndCheck(char *StrPrm)
{
   WP_U32 res, cmd, aug1, min_q=2500,max_q,threshold,status,i,FBP_Size,iteration=0;

   res = sscanf(StrPrm, "%d %d %d", &cmd, &aug1);
   if(res != 2)
   {
      WTI_TerminatePrintError("Invalid number of parameters", __LINE__);
      return;
   }
   threshold = 1482;//2500 buffers - 1008 PHYS - 10 (gap)
   while(1)
   {
      if (WTI_SampleSerial())
         break;
      WT_UfeSystemRxTrafficMask(&the_system->ufe, aug1, 1);
      WP_Delay(100000);
      WT_UfeSystemRxTrafficMask(&the_system->ufe, aug1, 0);
      WP_Delay(100000);
      for(i=0;i<100;i++)
      {
         WP_Delay(1000);
         ...
         if(FBP_Size<min_q)
         {
            ...
            if(min_q < threshold)
            {
               printf("min buffers is less then max buffers - num of phys - 10 \n");
               printf("num of buffers: %d\n",min_q);
               return;
      ...
      printf("min_q is: %d, iteration number: %d!!!\n",min_q,iteration);
      iteration++;
...
void CLI_F_Tdm2PsnFlowAggShowInfo(char *StrPrm)
{
   ...
   printf("cep_mode: ");
   if (WP_ENABLE == tdm2psn_agg_param.cep_mode)
      printf("WP_ENABLE\n");
   else
      printf("WP_DISABLE\n");

   printf("cep_bandwidth: ");
   if (WP_IW_UFE4_CEP_VC3_LOW_BANDWIDTH == tdm2psn_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC3_LOW_BANDWIDTH\n");
   else if (WP_IW_UFE4_CEP_VC3_HIGH_STS1C_BANDWIDTH == tdm2psn_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC3_HIGH_STS1C_BANDWIDTH\n");
   else if (WP_IW_UFE4_CEP_VC4_STS3C_BANDWIDTH == tdm2psn_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC4_STS3C_BANDWIDTH\n");
   else if (WP_IW_UFE4_CEP_VC44C_STS12C_BANDWIDTH == tdm2psn_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC44C_STS12C_BANDWIDTH\n");
...
void CLI_F_Psn2TdmFlowAggShowInfo(char *StrPrm)
{
   ...
   printf("cep_mode: ");
   if (WP_ENABLE == psn2tdm_agg_param.cep_mode)
      printf("WP_ENABLE\n");
   else
      printf("WP_DISABLE\n");
   printf("cep_bandwidth: ");
   if (WP_IW_UFE4_CEP_VC3_LOW_BANDWIDTH == psn2tdm_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC3_LOW_BANDWIDTH\n");
   else if (WP_IW_UFE4_CEP_VC3_HIGH_STS1C_BANDWIDTH == psn2tdm_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC3_HIGH_STS1C_BANDWIDTH\n");
   else if (WP_IW_UFE4_CEP_VC4_STS3C_BANDWIDTH == psn2tdm_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC4_STS3C_BANDWIDTH\n");
   else if (WP_IW_UFE4_CEP_VC44C_STS12C_BANDWIDTH == psn2tdm_agg_param.cep_bandwidth)
      printf("WP_IW_UFE4_CEP_VC44C_STS12C_BANDWIDTH\n");
...
void CLI_F_Pwe3TransTxChannelModify(char *StrPrm)
{
   ...
   WP_U8 pattern[WTI_E1_SLOTS];
   WP_U32 i;
   WP_U32 start_pw_index, end_pw_index, pw_index;

   res = sscanf(StrPrm, "%d %d %d %s", &cmd, &pw_index, &mod, val);
   ...
   {
      start_pw_index = 0;
      end_pw_index = total_number_of_pw;            
   }
   else
   {
      start_pw_index = pw_index;
      end_pw_index = pw_index + 1;            
   }
   ...
   switch (mod)
   {
      ...
      {
         command = WP_CH_MOD_TRANS_STATMODE;
         for (pw_index=start_pw_index;pw_index<end_pw_index;pw_index++)
            pw_info_base[pw_index].trans_tx_ch_config.statmode = atoi(val);
...
#endif
         for (pw_index=start_pw_index;pw_index<end_pw_index;pw_index++)
            pw_info_base[pw_index].trans_tx_ch_config.tx_udcas = atoi(val);
      ...
      case 3:
      {
         ...
         for (pw_index=start_pw_index;pw_index<end_pw_index;pw_index++)
            pw_info_base[pw_index].trans_tx_ch_config.tx_udp_dummy_mode = atoi(val);
      ...
      case 4:
      {
         ...
         /* if the user configured one slot with the sign '-', we set this value for all the active slots */
         if (1 == i)
         {            
            if (strstr(val, "-"))
            {               
               for (i=1;i<WTI_E1_SLOTS;i++)
                  pattern[i] = pattern[0];               
            }            
         }
         
         for (pw_index=start_pw_index;pw_index<end_pw_index;pw_index++)
            memcpy(pw_info_base[pw_index].trans_tx_ch_config.tx_udpattern, pattern, WTI_E1_SLOTS);
      ...
      case 5:
      {
         ...
         for (pw_index=start_pw_index;pw_index<end_pw_index;pw_index++)
            memcpy(pw_info_base[pw_index].trans_tx_ch_config.tx_udp_dummy_pattern, pattern, WTI_E1_SLOTS);
         break;
      }
   ...
   for (pw_index=start_pw_index;pw_index<end_pw_index;pw_index++)
   {      
      status = WP_ChannelModify(the_system->pw[pw_index].trans_tx,
                                command,
                                &pw_info_base[pw_index].trans_tx_ch_config);
      WTI_TerminateOnError(status, "WP_ChannelModify",__LINE__);
   }
...
void CLI_F_EnetDeviceEnableDisable(char * StrPrm)
{
   WP_status status;
   WP_U32 res, cmd, enable;
   ...
   {
      WTI_TerminatePrintError("Invalid parameters", __LINE__);
      return;
   }
   ...
   {
      case 0:
   /* Disable the Enet Device */
         status = WP_DeviceDisable(the_system->enet_device, WP_DIRECTION_DUPLEX);
         WTI_TerminateOnError(status, "WP_DeviceDisable() enet_device",__LINE__);
         break;
      case 1:
...
void CLI_F_SetRandomUdPattern(char * StrPrm)
{
   WP_CHAR cmd_temp_buf[WTI_MAX_STRING_SIZE];
   WP_CHAR udpattern_temp_buf[WTI_MAX_STRING_SIZE];
   WP_CHAR short_temp_buf[16];
   WP_U32 i, data_unit_value, num_of_slots;
   
   short_temp_buf[0] = '\0';
   cmd_temp_buf[0] = '\0';
   udpattern_temp_buf[0] = '\0';

   srand (0);

   num_of_slots = WTI_E1_SLOTS;
   
   for (i=0;i<num_of_slots;i++)
   {
      data_unit_value = rand() % 256;
      if ((num_of_slots - 1) != i)
         sprintf(short_temp_buf, "%u,", data_unit_value);
      else
         sprintf(short_temp_buf, "%u", data_unit_value);
      strncat(udpattern_temp_buf, short_temp_buf, strlen(short_temp_buf));
      
   }
   
   printf("%s\n", udpattern_temp_buf);
   
   sprintf(cmd_temp_buf, "0 %d 4 %s", total_number_of_pw, udpattern_temp_buf);
   printf("%s\n", cmd_temp_buf);
   CLI_F_Pwe3TransTxChannelModify(cmd_temp_buf);
...
void CLI_F_EnetDeviceStatistics(char *StrPrm)
{
   ...
   printf("rx_err_fcs:           %.8x%.8x\n",
          ((WTI_StatField)xgi_stats.rx_err_fcs).part.high,
          ((WTI_StatField)xgi_stats.rx_err_fcs).part.low);
...
void WTI_F_Iw_PortStatisticsDisplay(void *stats)
{
   WP_route_port_stats *iw_stats = NULL;
   WP_bport_stats *bport_stats = NULL;
   WP_U32 screen_coord, hor_coord, temp1, temp2;
   WP_U32 i, j=0;
   ...
   {
      iw_stats = (WP_route_port_stats *) stats;
   }
   else if (the_system->iw_system_mode == WTI_IW_SYSTEM_BRIDGE)
   {
      bport_stats = (WP_bport_stats *) stats;
   }

   if (the_system->iw_system_mode == WTI_IW_SYSTEM_BRIDGE)
   {
      ...
      Vt100GotoXy(hor_coord+24,screen_coord+18);
      ...
      Vt100GotoXy(hor_coord+24,screen_coord+19);
      ...
      Vt100GotoXy(hor_coord+24,screen_coord+20);
      ...
      for(i=0; i<WP_IW_DFC_MAX_NUM_OF_FILTERS; i++)
      {
         ...
         {
            temp1 = bport_stats->discard_dfc_filter[i];
            temp2 = bport_stats->discard_dfc_filter[i] >> 32;
            Vt100GotoXy(hor_coord+24,(screen_coord+21+i));
      ...
      for(i=0; i<WP_PCE_MAX_NUM_OF_FILTERS; i++)
      {
         ...
         {
            temp1 = bport_stats->discard_pce_filter[i];
            temp2 = bport_stats->discard_pce_filter[i] >> 32;
            Vt100GotoXy(hor_coord+24,screen_coord+21+j+i);
      ...
      temp1 = bport_stats->ingress_policer_non_conforming_packets;
      temp2 = bport_stats->ingress_policer_non_conforming_packets >> 32;
      Vt100GotoXy(hor_coord+24,screen_coord+22+j);
      printf("ingress_policer_non_conforming_packets: %.8x%.8x\n", temp2,temp1);
   ...
   else if (the_system->iw_system_mode == WTI_IW_SYSTEM_ROUTING)
   {
      ...
      Vt100GotoXy(hor_coord+24,screen_coord+14+j);
      printf("ingress_policer_non_conforming_packets: %.8x%.8x\n", temp2,temp1);
...
void CLI_F_IwPortStatistics(char *StrPrm)
{
   ...
   WP_U32 t1=0, t2=0, screen_coord, hor_coord;
   WP_U32 res, cmd, val;
   WP_route_port_stats iw_stats;
   WP_bport_stats bport_stats;

   memset(&iw_stats, 0, sizeof(WP_route_port_stats));
   ...
   if (val > 1)
   {
      WTI_TerminatePrintError("CLI_F_IwPortStatistics", __LINE__);
      return;
   }
   ...
   if (val)
   {
      if(the_system->iw_system_mode == WTI_IW_SYSTEM_ROUTING)
      {
         printf("Resetting stats for PSN->TDM IW port (rx_binding.input_port):\n");
         status =  WP_IwPortStatisticsReset(the_system->iw_port, &iw_stats);
         memset(&iw_stats, 0, sizeof(WP_route_port_stats));
      }
      else if(the_system->iw_system_mode == WTI_IW_SYSTEM_BRIDGE)
      {
         printf("Resetting stats for PSN->TDM bport (rx_binding_gbe.input_port):\n");
         status =  WP_IwPortStatisticsReset(the_system->bridge_iw_port, &bport_stats);
         memset(&bport_stats, 0, sizeof(WP_bport_stats));
      }
      if (status)
      {
         printf("WP_IwPortStatisticsReset error\n");
         return;
      }
      else
      {
      ...
      while (1)
      {
         ...
         while (1)
         {
            ...
            if(WP_TimeDelta(t2,t1) <= FACTOR)
            {
               ...
               {
                  status =  WP_IwPortStatistics(the_system->bridge_iw_port, &bport_stats);
               }
               ...
               if (the_system->iw_system_mode == WTI_IW_SYSTEM_ROUTING)
               {
                  WTI_F_Iw_PortStatisticsDisplay(&iw_stats);
               }
               else if (the_system->iw_system_mode == WTI_IW_SYSTEM_BRIDGE)
               {
                  WTI_F_Iw_PortStatisticsDisplay(&bport_stats);            
               }
            }
#ifdef AUTOMATION_MODE
      return;
...
void CLI_F_FlexmuxSetFraming(char *StrPrm)
{
   WP_U32 res, cmd, flexmux_id, framing_type, n_max_link, client_port_id,client_port,num_of_lines;
   WT_ufe_line_params *line_params;
   res = sscanf(StrPrm, "%d %d %d %d", &cmd, &flexmux_id, &framing_type, &n_max_link);
   if (res != 4)
   {
   ...
   if ((1 != framing_type) && (2 != framing_type))
   {
      printf("CLI_F_FlexmuxSetFraming: invalid  parameters\n");
      return;
   }
   ...
   {
      if (1 == framing_type)
         framing_type = WPX_UFE_FRAMER_CLIENT_PCM31;
      else if (2 == framing_type)
         framing_type = WPX_UFE_FRAMER_CLIENT_PCM31CRC;      
   }   
   else if (WPX_UFE_FRAMER_WUFE_SDH_TYPE_VC11 == lo_transfer_type_in_use)
   {
      if (1 == framing_type)
         framing_type = WPX_UFE_FRAMER_CLIENT_FRAMING_SF_D4;
      else if (2 == framing_type)
         framing_type = WPX_UFE_FRAMER_CLIENT_FRAMING_ESF; 
   }
   else
   {
      printf("currently support only E1/T1\n");
      
   }
   
   
   for (num_of_lines = client_port_id=0;num_of_lines<n_max_link;client_port_id++)
   {
      if (WPX_UFE_FRAMER_WUFE_SDH_TYPE_VC12 == lo_transfer_type_in_use)
      {
         if ((client_port_id % 28) > 20)
         {
            continue;
         }
      }
      
      line_params = &(the_system->ufe.line_params[client_port_id]);
      client_port = WTI_FlexmuxClientPortIndexGet(client_port_id, line_params->transfer_type);
      if(line_params->framed == WUFE_FRAMED)
         WTI_FlexmuxSetframing(flexmux_id, client_port, framing_type);
      num_of_lines++;
...
static WP_U8 WTI_SampleSerial(void)
{
   ...
   WP_U32 data;
   /* sample serial port */
   uart_line_status_reg = WPX_AppWpIntRegBaseGet(0) + WPX_UART_LINE_STATUS_REG_OFFSET ;
