...
#if 0
      printf("Peek number %#x: Value is 0x%08x\n",Address,Value);
#endif
...
#define WTI_COMMON_LOCK_KEY \
       WPL_LOCK_KEY_CREATE(WPL_HW_LOCK, WPL_SHARED_LOCK, WPL_TA_SEMAPHORE_RESERVED_ALARMS, 0)
...
void WTI_flexmux_take_framer_lock(void)
{
   ...
   while (1)
   {
      WPL_Lock(WTI_COMMON_LOCK_KEY, NULL);

      rc = wpv_swsem_acquire((int *) framer_lock, NO_WAIT);

      WPL_Unlock(WTI_COMMON_LOCK_KEY, NULL);
...
static WP_U32 locks[2];
void initialize_locks(void)
{
   WPL_LockKeyInit(WTI_COMMON_LOCK_KEY, NULL);
...
void WTI_flexmux_take_framer_lock(void)
{
   /* Read from non-cached location to allow for MIPS thread-switch */
   WP_TimeRead();
}
void WTI_flexmux_release_framer_lock(void) {}
void WTI_flexmux_take_server_lock(void)
...
void WTI_Flexmux_Squelch_RecClkGet(WP_U8 flexmux_id, WP_U8 output_selector)
{
   ...
   WP_U8 squelch_mode;
#if WTI_FLEXMUX_DBG_MODE
   printf("output_selector %d\n", output_selector);
#endif   
   status = WPX_FRMR_DEV_Squelch_RecClkGet(flexmux_id, output_selector, &squelch_mode);
   WTI_FlexmuxCheckStatus("WPX_FRMR_DEV_Squelch_RecClkGet", status, __LINE__);
   if(squelch_mode == WPX_UFE_FRAMER_RECOVERED_CLOCK_SQUELCH_MODE_DISABLE)
       printf("squelch mode is: WPX_UFE_FRAMER_RECOVERED_CLOCK_SQUELCH_MODE_DISABLE\n");
   else if(squelch_mode == WPX_UFE_FRAMER_RECOVERED_CLOCK_SQUELCH_MODE_ENABLE)
       printf("squelch mode is: WPX_UFE_FRAMER_RECOVERED_CLOCK_SQUELCH_MODE_ENABLE\n");
   else
       printf("squelch mode is: WPX_UFE_FRAMER_RECOVERED_CLOCK_SQUELCH_MODE_MAX (out of range)\n");    
}
...
void WTI_Flexmux_Sdh_Ho_Rdi_Set_Mode(U8 flexmux_id, U8 stm4,U8 stm1,U8 stm0,U8 ho_type, U8 type)
{
  ...
  {
  case 0:
      status= WPX_FRMR_SONET_SDH_HO_PATH_SetRdiMode(0,flexmux_id, &line_end_point, WPX_UFE_FRAMER_RDI_MODE_ENABLE);
      WTI_FlexmuxCheckStatus("WPX_FRMR_SONET_SDH_HO_PATH_SetRdiMode", status, __LINE__);  
      break;
  case 1:
      status= WPX_FRMR_SONET_SDH_HO_PATH_SetRdiMode(0,flexmux_id, &line_end_point, WPX_UFE_FRAMER_ERDI_MODE_ENABLE);
      WTI_FlexmuxCheckStatus("WPX_FRMR_SONET_SDH_HO_PATH_SetRdiMode", status, __LINE__);  
      break;
  default:
      return;
...
void WTI_Flexmux_Sdh_Ho_SS_Bits_Set_Tx(U8 flexmux_id, U8 stm4,U8 stm1,U8 stm0,U8 ho_type, U8 Value)
{
  ...
  status= WPX_FRMR_SONET_SDH_HO_PATH_SS_BITS_Set_Tx(0,flexmux_id, &line_end_point, Value);
  WTI_FlexmuxCheckStatus("WPX_FRMR_SONET_SDH_HO_PATH_SetRdiMode", status, __LINE__);  
...
void WTI_Flexmux_Sdh_Ho_SS_Bits_Get_Rx(U8 flexmux_id, U8 stm4,U8 stm1,U8 stm0,U8 ho_type)
{
  ...
  status= WPX_FRMR_SONET_SDH_HO_PATH_SS_BITS_Get_Rx(0,flexmux_id, &line_end_point);
  WTI_FlexmuxCheckStatus("WPX_FRMR_SONET_SDH_HO_PATH_SetRdiMode", status, __LINE__);  
...
void WTI_APS_Time_Measure(void)
{
   ...
   /* Print the time of each APS */
   item = enter_int;
   
   exit_los_service_cnt = 0;   
   while(item != last_exit_los_service)
   {
      if (23 == item->type)
         printf("APS %u: %u[uSec]\n", exit_los_service_cnt++, WP_TimeDelta(item->timestamp, enter_int->timestamp));
   ...
   /* print the time delta of each step */
   while(item != last_exit_los_service)
   {
      next_item = item + 1;      
      while ((16 == next_item->type) ||
             (43 == next_item->type) ||
             (17 == next_item->type))
         next_item++;      

      printf("%s - %s: %u[uSec]\n", type_name[next_item->type], type_name[item->type], WP_TimeDelta(next_item->timestamp, item->timestamp));  
      item = next_item;      
   }
