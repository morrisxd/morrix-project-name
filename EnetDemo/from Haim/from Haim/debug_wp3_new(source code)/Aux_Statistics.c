...
{   
   ...
   if ( (status = WP_DeviceStatistics(dev_enet, &stats_enet)) ) {
      printf("GetStatistics error Enet : %d\n",dev_enet);
      return;
   }
...
{   
   ...
   if ( (status = WP_DeviceStatistics(dev_pos, &stats_upi)) ) {
      printf("GetStatistics error pos \n");
      return;
   }
...
{   
   ...
   if ( (status = WP_DeviceStatistics(dev_atm, &stats_upi)) ) {
      printf("GetStatistics error atm \n");
      return;
   }
...
void WPE_DisplayIWPortStats(WP_handle iw_port_handle)
{
   ...
   for (ii=0 ; ii<32 ; ii++)
   {
      ...
      {
         printf ("discard_dfc_filter %02d:            %08x%08x\n",
                 ii,
                 ((WPE_StatField)bport_stats.discard_dfc_filter[ii]).part.high,
                 ((WPE_StatField)bport_stats.discard_dfc_filter[ii]).part.low);
      }
   }
