 ...
 *  File Name   : ppp_hdlc_tdm_stats.c
 *
 *****************************************************************************/
...
void App_ShowDeviceStats(void)
{
   ...
   printf(" g999_rx_length_error      : %08d\n", (WP_U32)s_hs_enet.g999_rx_length_error);
	...
	for(ii = 0;ii < APP_MAX_DEV_TDM	;ii++)
	{
	   ...
	   printf("-----------------------------------------------------------\n");
	   printf(" Device statistics TDM [%d] \n",ii);
	   printf("-----------------------------------------------------------\n");
	   memset(&tdi_hdlc_stats,0,sizeof(WP_stats_tdi_hdlc));
	   status = WP_DeviceStatistics(h_dev_tdm[ii],&tdi_hdlc_stats);
	   App_TerminateOnError(status, "WP_Devicestatistics");

	   printf("   %-25s : %d\n",  "rx_frames", (WP_U32)tdi_hdlc_stats.rx_frames);
	   printf("   %-25s : %d\n",  "tx_frames",(WP_U32)tdi_hdlc_stats.tx_frames);
	   printf("   %-25s : %d\n",  "rx_err_crc",(WP_U32)tdi_hdlc_stats.rx_err_crc);
	   printf("   %-25s : %d\n",  "rx_err_abort", (WP_U32)tdi_hdlc_stats.rx_err_abort);
	   printf("   %-25s : %d\n",  "rx_err_addr_mismatch",(WP_U32)tdi_hdlc_stats.rx_err_addr_mismatch);
	   printf("   %-25s : %d\n",  "rx_err_buffer_overrun", (WP_U32)tdi_hdlc_stats.rx_err_buffer_overrun);
	   printf("   %-25s : %d\n",  "rx_err_overrun", (WP_U32)tdi_hdlc_stats.rx_err_overrun);
	   printf("   %-25s : %d\n",  "rx_err_iw_buffer_underrun", (WP_U32)tdi_hdlc_stats.rx_err_iw_buffer_underrun);
	   printf("   %-25s : %d\n",  "rx_err_iw_mru", (WP_U32)tdi_hdlc_stats.rx_err_iw_mru);
	   printf("   %-25s : %d\n",  "tx_err_underrun", (WP_U32)tdi_hdlc_stats.tx_err_underrun);
	   printf("   %-25s : %d\n",  "rx_err_maxsdu", (WP_U32)tdi_hdlc_stats.rx_err_maxsdu);
	   printf("   %-25s : %d\n",  "rx_iw_frames", (WP_U32)tdi_hdlc_stats.rx_iw_frames);
	   printf("   %-25s : %d\n",  "rx_err_non_octet", (WP_U32)tdi_hdlc_stats.rx_err_non_octet);
	   printf("   %-25s : %d\n",  "rx_bytes", (WP_U32)tdi_hdlc_stats.rx_bytes);
	   printf("   %-25s : %d\n",  "rx_err_bytes", (WP_U32)tdi_hdlc_stats.rx_err_bytes);
	   printf("   %-25s : %d\n",  "tx_bytes", (WP_U32)tdi_hdlc_stats.tx_bytes);
	   printf("   %-25s : %d\n",  "rx_err_iw_l2_parse", (WP_U32)tdi_hdlc_stats.rx_err_iw_l2_parse);
...
void App_ResetStats(void)
{
    ...
    for(ii = 0; ii < APP_MAX_DEV_TDM; ii++)
    {
        WP_DeviceStatisticsReset(h_dev_tdm[ii], &tdi_hdlc_stats);
...
void App_CheckTrafficDataStats(WP_status *test_result)
{
    ...
    WP_stats_tdi_hdlc tdi_hdlc_stats[APP_MAX_DEV_TDM];
   ...
   *test_result = WP_OK;
   ...
   {
       memset(&tdi_hdlc_stats[0], 0, sizeof(tdi_hdlc_stats[0]));
       status = WP_DeviceStatistics(h_dev_tdm[0], &tdi_hdlc_stats[0]);
       App_TerminateOnError(status, "WP_Devicestatistics");


       /*Check tdm errors*/
        if (tdi_hdlc_stats[0].rx_err_abort || tdi_hdlc_stats[0].rx_err_addr_mismatch || 
            tdi_hdlc_stats[0].rx_err_buffer_overrun || tdi_hdlc_stats[0].rx_err_crc || 
            tdi_hdlc_stats[0].rx_err_overrun || tdi_hdlc_stats[0].rx_err_bytes 
            )
        {
          *test_result = (-1);
   ...
   if ((tdi_hdlc_stats[0].tx_frames != tdi_hdlc_stats[0].rx_iw_frames)
       || (APP_MAX_PKTS_NUM != tdi_hdlc_stats[0].tx_frames))
   {
       *test_result = (-1);
   ...
   memset(&bport_stats, 0, sizeof(bport_stats));
   status = WP_IwPortStatistics(h_bridge1_bport_tdm[0], &bport_stats);
   App_TerminateOnError(status, "WP_BportStatistics");
   ...
   memset(&bport_stats, 0, sizeof(bport_stats));
   status = WP_IwPortStatistics(h_bridge2_bport_tdm[0], &bport_stats);
   App_TerminateOnError(status, "WP_BportStatistics");
   ...
   memset(&flow_stats, 0, sizeof(flow_stats));
   status = WP_IwFlowStatistics(h_flow_agg_tdm[0], WP_IW_FLOW_STAT, &flow_stats);
   App_TerminateOnError(status, "WP_IwFlowStatistics");
