                      ...
                      U8 IsAsserted)
{
    ...
    if (initial_cfg->stru_stm1_cfg_info[0].physicalLayer == NPU_PHYSICAL_SDH 
     && initial_cfg->stru_stm1_cfg_info[1].physicalLayer == NPU_PHYSICAL_SDH)
	{
        WP_U8 stm4 = pLineEndpoint->u.SDH.stm4;
        ...
        printf("PATH    : device = %d, stm4 = %d, stm1 = %d, stm0 = %d, tug2 = %d,tu = %d, category = %d %s, isAsserted = %d\n", iDevice, stm4, stm1, tug3, tug2, tug12, AlarmCategory, lineAlrmStr[AlarmCategory], IsAsserted);
        ...
        np_set_ufe4_E1T1_alarm_info(stm1, lineIndex, AlarmCategory, IsAsserted);
	}
    else if (initial_cfg->stru_stm1_cfg_info[0].physicalLayer == NPU_PHYSICAL_SONET 
          || initial_cfg->stru_stm1_cfg_info[1].physicalLayer == NPU_PHYSICAL_SONET)
	{
        WP_U8 sts12     = pLineEndpoint->u.SONET.sts12;
        WP_U8 sts3      = pLineEndpoint->u.SONET.sts3;
        WP_U8 sts1      = pLineEndpoint->u.SONET.sts1;
        WP_U8 vt_group  = pLineEndpoint->u.SONET.vt_group;
        WP_U8 vt        = pLineEndpoint->u.SONET.vt;

        printf("PATH    : device = %d, sts12 = %d, sts3 = %d, sts1 = %d, vt_group = %d, vt = %d, category = %d %s, isAsserted = %d\n", iDevice, sts12, sts3, sts1, vt_group, vt, AlarmCategory, lineAlrmStr[AlarmCategory], IsAsserted);

        if(sts3 > (SPP_MAX_STM1_PORT_NUM-1))
        {
            Fprintf("sts3 %d is not valid \n", sts3);
            return;
        }
		if(sts1 > 3)return;
		if(vt_group > 7)return;
		if(vt > 4)return;
		lineIndex = vt + 4 * vt_group + 28 * sts1;
		//Fprintf("stm1 = %d tug3 = %d tug2 =%d tug12 = %d lineIndex = %d \n",stm1,tug3,tug2,tug12,lineIndex);
		/*Lucent TU12 + TU2 *4 +  TU3*28*/

        np_set_ufe4_E1T1_alarm_info(sts3, lineIndex, AlarmCategory, IsAsserted);
	}
    return;
}
...
/* in CR when using the ADM for testing */
        ...
        for(AlarmCategory = WPX_UFE_FRAMER_SONET_LOS;AlarmCategory <= WPX_UFE_FRAMER_SONET_LOP_P;AlarmCategory++)
        {
