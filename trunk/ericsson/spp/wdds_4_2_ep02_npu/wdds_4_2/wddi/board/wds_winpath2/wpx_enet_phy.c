...
WP_status WPU_EnetPhyInit(WP_U32 port, WP_U32 mode)
{
   ...
   WP_U32 status, lock_status = WP_OK;
   WP_U32 os_enet_usage;
   ...
   {
      os_enet_usage = WPI_RegistryGlobalLockOsEnetUsage();
      if ((os_enet_usage == WPL_os_enet_keepalive) &&
          (port == WPI_SED_PORT))
      {
         WPI_WDDI_DETACH();
   ...
   hw_winnet = WPI_WINNET(res_reg_bases->reg_bases_ptr, 0);
