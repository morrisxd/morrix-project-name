...
WP_status WPI_CapabilityPlatform(WP_U32 wpid)
{
   ...
   /************* Clock recovery *************/
   for (i = 0; i < 16; i++) {
       WPI_CAP_CALL(WPI_CapabilitySetIndexed(wpid, WPI_CAP_CLK_REC_AVAILABLE, i, 1));
   }
   ...
   for (i = 0; i < 2; i++) {
      ...
      for (phy = 0; phy < 191; phy++)
          WPI_CAP_CALL(WPI_CapabilitySetDoublyIndexed(wpid, WPI_CAP_UPI_PHY_AVAILABLE,
                                                            i, phy, 1));
      for (phy = 0; phy < 96; phy++)
          WPI_CAP_CALL(WPI_CapabilitySetDoublyIndexed(wpid, WPI_CAP_UPI_PHY_AVAILABLE_MC,
                                                            i, phy, 1));
   }
   return WP_OK;
}
