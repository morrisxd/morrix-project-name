...
"Card Status:        xru Display NPU UP time  (ex:   hour(s),   minute(s),    seconds.)\n"
...
"Clear Statistics    cd Clear Driver Statistics\n"
...
int npu_pcap_file_record(FILE *fp ,char *buf,unsigned int len)
{
  ...
  sprintf(timestr,"%d:%d:%d.%04ld",p->tm_hour,p->tm_min,p->tm_sec,tv.tv_usec/1000);
  /*  printf("time is %s \n",timestr);*/
  fprintf(fp,"%s \n",timestr);
  fprintf(fp,"%s ","0000");
  for(i = 0;i< len;i++)
...
int main(int argc, char *argv[])
{
	...
	for (c = 0; c != -1;) 
	{
		...
		case OPT_PCAP_FILE_NAME:
			    sprintf(info.PcapFileName,"%s",optarg);
				filenameFlag = 1;
				break;
		case OPT_DLCI_VALUE:
    ...
    {
		sprintf(info.PcapFileName,"%s",DEFAULT_FILE_NAME);
	}

    char pcaptxtfileName[50];
	sprintf(pcaptxtfileName,"%s.txt",info.PcapFileName);
    ...
    /*printf("cmd = %d \n",info.cmd);*/
	sprintf(npumon_linx_name,"%s%d", NPUMON_NAME,getpid());
                     ...
                     case NPU_MON_GET_CAPTURE_FILE:
					 {
						switch(info->cmd)
						{
						 ...
						 case NPU_MON_CMD_CD:
						 {
						 	 ...
						 	 len = sig->npu_get_request_ack.pdata[0]<<8 | sig->npu_get_request_ack.pdata[1];
							 if(len != (u8)sizeof(get_upistatics))/*maybe send error by linx*/
							 {
...
int np_save2pcap_file(STRU_NPUMON_INFO *info,FILE *fp)
{
	...
	if(info->ifType == VP_TYPE_FR)
	{
		...
		sprintf(cmdstr,"text2pcap -q -l 107 -t \"%%H:%%M:%%S.\" %s.txt %s.pcap",info->PcapFileName,info->PcapFileName);
	
	}
	else
	{
		sprintf(cmdstr,"text2pcap -q -t \"%%H:%%M:%%S.\" %s.txt %s.pcap",info->PcapFileName,info->PcapFileName);
	...
	sprintf(cmdstr,"rm -f /var/log/%s.txt",info->PcapFileName);
