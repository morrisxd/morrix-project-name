...
static void linx_data_ready(struct sock *sk, int len)
{
	...
	if (likely(linx_match_filters(sk))) {
		...
		if (likely(sk->sk_sleep && waitqueue_active(sk->sk_sleep))) {
...
linx_poll(struct file *file, struct socket *sock, poll_table * wait)
{
	...
	/* The poll_wait function is called to add the sk->sk_sleep wait queue
	 ...
	 * sk->sk_sleep. If the linx_poll function return a mask indicating an
		    ...
		    int frag)
{
	...
	if (unlikely(payload_size > 65536)) {
		return -EMSGSIZE;
	}
			...
			struct msghdr *msg, size_t size, int flags)
{
	...
	if (likely(msg->msg_controllen != 0)) {
		if (unlikely(0 != copy_from_user(cmsg,
						 msg->msg_control,
						 msg->msg_controllen))) {
			ret = -EFAULT;
			goto out;
...
static int linx_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	case LINX_IOCTL_VERSION:
		return put_user(linx_version(), (int __user *)arg);
		break;
...
int af_linx_init(void)
{
	...
	BUG_ON(sizeof(struct linx_skb_cb) > (sizeof((struct sk_buff *) 0)->cb));
