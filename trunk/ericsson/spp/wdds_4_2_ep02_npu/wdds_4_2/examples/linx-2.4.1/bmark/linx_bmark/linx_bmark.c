...
static void test_without_linxlib(LINX_SPID client, LINX * linx)
{
	...
	union LINX_SIGNAL *sig = (void *)buf;
...
static void test_slave_proc(LINX_SPID client, int use_linx_api, int slave_no)
{
	char proc_name[24];
	LINX *linx;
	union LINX_SIGNAL *sig;

	sprintf(proc_name, "linx_bmark_%08u", slave_no);
	...
	if (use_linx_api)
		test_with_linxlib(client, linx);
	else
		test_without_linxlib(client, linx);
}
	...
	for (;;) {
		...
		if (sig->sigNo == CONN_EST) {
			...
			if (!pid) {
				...
				if (!pid) {
					test_slave_proc(client,
							use_linx_api, slave_no);
					exit(0);
				}
				exit(0);
			}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "S?p:ac:n:m:b:i:s:e:Alt:q")) != -1) {
		switch (c) {
		case 'S':
			/* Start server */
