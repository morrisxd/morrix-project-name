...
// XALYO endianess
#if 1
extern void __xsWP_MEM_SET(volatile WP_U32 *dest, WP_U32 source);
extern void __xsWP_MEM_SHORT_SET(volatile WP_U16 *dest, WP_U16 source);
extern void __xsWP_MEM_BYTE_SET(volatile WP_U8 *dest, WP_U8 source);
extern void __xsWP_MEM_GET(WP_U32 *dest, volatile WP_U32 *source);
extern void __xsWP_MEM_SHORT_GET(WP_U16 *dest, volatile WP_U16 *source);
extern void __xsWP_MEM_BYTE_GET(WP_U8 *dest, volatile WP_U8 *source);
extern void *__xsmemcpy(void *dest, void *src, WP_S32 n);
extern void *__xsmemset(void *, WP_S32, WP_S32);

#define WP_MEM_SET(dest, source) do { __xsWP_MEM_SET((volatile WP_U32 *)&dest, (WP_U32)source); } while(0)

#define WP_MEM_SHORT_SET(dest, source) do { __xsWP_MEM_SHORT_SET((volatile WP_U16 *)&dest, (WP_U16)source) ; } while(0)

#define WP_MEM_BYTE_SET(dest, source) do { __xsWP_MEM_BYTE_SET((volatile WP_U8 *)&dest, (WP_U8)source) ; } while(0)

#define WP_MEM_GET(dest, source) do { __xsWP_MEM_GET((WP_U32 *)&dest, (volatile WP_U32 *)&source) ; } while(0)

#define WP_MEM_SHORT_GET(dest, source) do { __xsWP_MEM_SHORT_GET((WP_U16 *)&dest, (volatile WP_U16 *)&source); } while(0)

#define WP_MEM_BYTE_GET(dest, source) do { __xsWP_MEM_BYTE_GET((WP_U8 *)&dest, (volatile WP_U8 *)&source); } while(0)

#define WP_MEM_BYTES_SET(dest, source, length) __xsmemcpy(dest, source, length)

#define WP_MEM_BYTES_GET(dest, source, length) __xsmemcpy(dest, source, length)

#define WP_MEM_BYTES_FILL(dest, value, length) __xsmemset(dest, value, length)
