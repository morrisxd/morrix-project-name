...
/*
 * There are 2 fields in the registry root for our use ..
 */
...
extern wpi_veneer_memory wpl_bus_mappings[WPI_BUS_LAST];
...
void WPLI_pool_pgfree_free(wpl_heapinfo *h, wpl_pgfree *p);
 ...
 * Called by:
 *
 *     WPLI_HeapInit  (common/wpi_bsp.c)
 *
 ****************************************************************************/
 ...
 *
 * Called by:
 *     WPLI_HeapInit (wpi_heap.c)
 *     free_pages (wpi_heap.c)
 *
 ****************************************************************************/
...
/*****************************************************************************
 *
 * Function: WPLI_pool_pgfree_free
 *
 * Purpose: Return page to pool_pgfree list
 *
 * Description:
 *
 * Inputs:
...
void WPLI_pool_pgfree_free(wpl_heapinfo *h, wpl_pgfree *p)
{
   p->next = h->pool_pgfree_head;
   h->pool_pgfree_head = p;
}
