...
/*
 * init UFE4 right here, call WTI_FlexmuxInit () here.
 * 	- WUFE_UfeInit ()
 * 	- WUFE_DeviceTypeSet ()
 * 	- WUFE_SystemConfig ()
 * 	- WUFE_SystemEnable ()
 *	- WPL_IntConnect() , WPL_IntEnable ()
 * 	- WUFE_UfeFramerHwInterruptEnable ()
 * 	- WUFE_UfeCoreHwInterruptEnable ()
 * 	- WUFE_SystemInterruptEnable ()
 * 	- WUFE_UfeCoreHwInterruptDisable ()
 * 	- WTI_FlexmuxInit ()
 * 	- WTI_FlexmuxForceB ()
 */
static void WTUFE_config_system( WP_U32 ufe_lb_mode,WP_U32 ufe_port)
        ...
        if(NPU_VALID == initial_cfg->stru_stm1_cfg_info[i].stm1Valid)
        {
            ...
            for(j = line_index_base; j < line_index_counts; j ++)
            {
                ...
                else // for these un-configured E1s, we need create the defaut object to clear the LOP alarms on the peer transmission device;
                {
                    ...
                    Fprintf("Create UFE phy for un-configured E1:  UFE phy on stm1 %d trunk %d ,group %d is Created slot_selection = 0x%x \n",i,j+1,1,0xfffffffe);
                }
            }
        }
	}
