 ...
 * bouth sides use only one Phy.
      Test Scenarios (will influence Hierarchical Enet Shaping):
      1 - Hierarchical Enet uses HW channels,  L2 Shaping is CIR_EIR, L3 Shaping is STRICT
      2 - Hierarchical Enet uses HW channels,  L2 Shaping is WRR, L3 Shaping is CIR_EIR with STRICT flag
      3 - Hierarchical Enet uses HW channels,  L2&L3 organized into ShapingBlock
      4 - Hierarchical Enet uses SW channels,  L2 Shaping is CIR_EIR, L3 Shaping is STRICT
 * +------------+--------------------+------------------------------------------
 * |            |                    |
 * +------------+--------------------+------------------------------------------
 *
...
#include "Aux_Statistics.c"

//#define DEBUG_PRINT
#define SCENARIO 1

#define USE_SHAPING_BLOCK_L1 1 // Need to Create L1 Shaping block always, untill the errata will be fixed.
 ...
 ***                                 DEFINES                                  ***
 *******************************************************************************/
#if (SCENARIO == 1) // simplest - all WFQ
#define L1_WFQ  1
#define L2_WFQ  1
#define L3_WFQ  1
#elif   (SCENARIO == 2)
...
#define L1_WFQ     1
#define L2_CIR_EIR 1
#define L3_CIR_EIR_STRICT 1
#elif (SCENARIO == 4) // L2 ShapingBlock
#define L1_CIR_EIR 1
#define L2_STRICT  1
#define L3_STRICT  1
#define USE_SHAPING_BLOCK_L2 1
#elif (SCENARIO == 5) // SW channels
#define L1_WFQ     1
#define L2_CIR_EIR 1
#define L3_SW_CHANNELS  1
#endif
...
#define NUM_OF_L2_GROUP 2

// Channels
#define NUM_OF_ALL_L2HOST_CHANNELS    2 // 2 Enet
#define NUM_OF_FAST_ENET_TX_CHANNELS  1 // FIXME to be 8 after FAST_ENET bug fixed
#define NUM_OF_FAST_ENET_RX_CHANNELS  1 // Must be always 1 per PHY
#define NUM_OF_HIER_ENET_TX_CHANNELS_PER_L2 4 // 8 is maximum number of channels for HW channels mode
#define NUM_OF_HIER_ENET_TX_CHANNELS  NUM_OF_HIER_ENET_TX_CHANNELS_PER_L2*NUM_OF_L2_GROUP*NUM_OF_L1_GROUP
...
#if USE_SHAPING_BLOCK_L2==1
#define NUM_OF_HIER_ENET_FAS_PER_L2 1 // Because in shaping block one FA points to the whole 8 FIFOs (see diagram in QoS Doc)
#else
#define NUM_OF_HIER_ENET_FAS_PER_L2 NUM_OF_HIER_ENET_TX_CHANNELS_PER_L2 // each FA point to one channel
#endif


#define NUM_OF_HIER_ENET_FAS     (NUM_OF_HIER_ENET_FAS_PER_L2*NUM_OF_L2_GROUP*NUM_OF_L1_GROUP)
#define NUM_OF_FAST_ENET_FAS     8 // FIXME to be NUM_OF_FAST_ENET_TX_CHANNELS 
...
#define MAX_IW_FLOWS             (MAX_VLAN_IDS*NUM_OF_IW_BPORTS+NUM_OF_FAST_ENET_FAS)  /* Max combinations of VLAN+BP = Max Entries in Memberset table = */ 
...
WP_handle  port_enet, dev_enet, bport_enet, agg_enet_change_mac, agg_enet[NUM_OF_FAST_ENET_FAS], rx_gbe_channel, tx_gbe_channel[NUM_OF_FAST_ENET_TX_CHANNELS];

WP_handle  port_hier_enet, dev_hier_enet, rx_gbe_hier_channel;
WP_handle  agg_hier_enet[NUM_OF_L1_GROUP][NUM_OF_L2_GROUP][NUM_OF_HIER_ENET_FAS_PER_L2], bport_hier_enet;
WP_handle  tx_gbe_hier_channel[NUM_OF_L1_GROUP][NUM_OF_L2_GROUP][NUM_OF_HIER_ENET_TX_CHANNELS_PER_L2];

WP_handle  mc_group_handle[NUM_OF_MC_GROUPS], mc_agg_handle[NUM_OF_MC_GROUPS], mc_dummy_bport;
...
WP_U8 enet_dst_mac[6]               = {0x00,0x00,0x00,0x0A,0x00,0x00};
WP_U8 enet_change_dst_mac[6]        = {0x00,0x00,0x00,0x0A,0x01,0x00};

WP_U8 hier_enet_dst_mac[6]         = {0x00,0x00,0x00,0x0B,0x00,0x00};
WP_U8 hier_enet_change_dst_mac[6]  = {0x00,0x00,0x00,0x0B,0x01,0x00};
WP_U8 hier_enet_mcast_dst_mac[6]   = {0x10,0x00,0x00,0x0B,0x00,0x00};

WP_U8 host_dst_mac[6]               = {0x00,0x00,0x00,0x0C,0x00,0x00};
...
void    WPE_Send_HostData(WP_handle tx_channel, WP_data_type data_type, WP_U32 ifc_type, WP_U8 dst_mac[]);
...
//Debug:
void WPE_CreateBusError1(void);
void WPE_CreateBusError2(void);
void WPE_EnableDebugBaseLog(void);
...
//-------- Optional Functionality ---------//
   ...
   status = WP_ControlRegister(WP_FORWARD_RULE_FIND_ALL, &WPE_DisplayForwardingRule);
   terminate_on_error(status, "WP_ControlRegister WP_FORWARD_RULE_FIND_ALL ");
...
/* Interrupt queue configuration used with WP_SysInit */
   ...
   WP_iw_global  max_iw =
      {
         /* max_iw_aggregations */          MAX_IW_FLOWS, // MAX_IW_AGG is always < MAX_IW_FLOWS since hierarchicalple flows can point to same FA
   ...
   WP_context context =
      {
         ...
         /* the first four assignments are mandatory bus assignments */
         {
            ...
            {/* FBP DATA                 */ WP_BUS_PACKET,   APP_BANK_PACKET},
            {/* L2_BKGND                 */ WP_BUS_PACKET,   APP_BANK_PACKET},
            ...
            {/* HASH_IW_LPM_EMC          */ WP_BUS_PACKET,   APP_BANK_PACKET},
            ...
            {/* MFC_INTERNAL             */ WP_BUS_PACKET,   APP_BANK_PACKET},
...
void    WPE_InitHWCards()
{
   ...
   status = WPX_BoardSerdesInit(0, WP_PORT_ENET7, WPX_SERDES_LOOPBACK); // Loopback on Fast Enet
...
void    WPE_CreateIWQnode()
{
   ...
   WP_pool_buffer_data adjunct_buffer_config =
      {
         /* n_buffers */     2, // Number of host threads that do Host Receive (1 BD per thread)
         /* offset */        64,
         /* size */          2048,
         /* pad */           0,
   ...
   WP_qnode_iwq qn_iw_config =
      {
         ...
         /* num_buffers */     MAXT_SIZE *(NUM_OF_HIER_ENET_TX_CHANNELS+1+1) + (NUM_OF_HIER_ENET_RX_CHANNELS+1+1),
         /* adjunct_pool */    0
      };
   ...
   iw_qnode = WP_QNodeCreate(0,WP_QNODE_IWQ | WP_QNODE_OPT_DEDICATED_RX_HWQ | WP_QNODE_OPT_FMU, &qn_iw_config);
...
void    WPE_CreateHostQnode()
{
   ...
   WP_pool_buffer_data buffers_host_config =
      {   /* n_buffers */ NUM_OF_ALL_L2HOST_CHANNELS*RING_LENGTH + NUM_OF_ALL_L2HOST_CHANNELS, // n_rings * ring_length + (number of max HostReceive threads that can be done simultaneously each taking one buffer for swap)
          /* offset */    0, // Must be zero to prevent NpoolAlloc Errors!
          /* size */      2048,
          /* pad */       0,
          /* bus */       WP_BUS_HOST,
          /* bank */      APP_BANK_HOST
   ...
   WP_pool_ring_data ring_host_config =
      {
         /* n_rings */     NUM_OF_ALL_L2HOST_CHANNELS, // Host Termination part of All RX channels + Non IW Host TX channels
         /* ring_length */ RING_LENGTH,                        // Max Numuber of BDs in each channel
         /* bus */         WP_BUS_HOST,
         /* bank */        APP_BANK_HOST
      };
...
void    WPE_CreateMultiCastQnode()
{
   WP_qnode_iw_mcq iw_mc_qn_config =
      {
         /* interruptqueue*/ 0,
         /* num_buffers */   2*NUM_OF_MC_GROUPS*NUM_OF_MC_MEMBERS_PER_GROUP,
         /* offset */        64,
         /* size */          2048,
         /* pad */           0
      };
...
//   for(i=0;i < NUM_OF_FAST_ENET_TX_CHANNELS; i++) // FIXME
   {
      status = WP_ChannelEnable(tx_gbe_channel[0]);
   ...
   for(i = 0; i < NUM_OF_L1_GROUP; i ++)
   {
      ...
      {
         for(k=0;k < NUM_OF_HIER_ENET_TX_CHANNELS_PER_L2; k++)
         {
   ...
   terminate_on_error(status, "WP_PortEnable FastEnet");

   status = WP_DeviceEnable(dev_enet, WP_DIRECTION_DUPLEX);
   terminate_on_error(status, "WP_DeviceEnable FastEnet");


   status = WP_PortEnable(port_hier_enet, WP_DIRECTION_DUPLEX);
   terminate_on_error(status, "WP_PortEnable HierEnet");

   status = WP_DeviceEnable(dev_hier_enet, WP_DIRECTION_DUPLEX);
   terminate_on_error(status, "WP_DeviceEnable HierEnet");
...
void    WPE_CreateHostIwRxChannel()
{
   ...
   terminate_on_error(rx_host_channel, "WP_ChannelCreate RX Host channel");
}
...
void    WPE_CreateHostIwTxChannel()
{
   ...
   tx_host_channel = WP_ChannelCreate(0, dev_host, iw_qnode, WP_CH_TX, WP_IW_HOST, &ch_iw_tx_config);
   terminate_on_error(tx_host_channel, "WP_ChannelCreate TX Host channel");
   ...
   WP_iw_agg_bridging agg_host_config =
      {
         ...
         /*encap_vlan_tag_mode*/ WP_IW_VLAN_TAG_DISABLE, //  Enable = Untagged - AddDefaultVLAN, Tagged - Do not Strip VLAN
         /*interruptqueue;*/     WP_IW_IRQT1,
         /*error_pkt_mode*/      WP_IW_ERRPKT_DISCARD,
         /*intmode;*/            WP_IW_INT_ENABLE, // To allow receiving packets to host using interrupts
         ...
         /*mtu;*/                MTU_SIZE,
         /*prefix_length */      0,
         /*prefix_header[32]*/   {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
         ...
         /*external_vlan*/       {WP_IW_EXTERN_VLAN_NONE,0},
         /*bc_mc_priority*/      {0,0,0,0},
         ...
         /*new_dst_mac[6];*/     {0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
         /*new_src_mac[6];*/     {0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
         /*hier_cong_prof;*/    WP_DISABLE,
         /*extraction_length;*/  0,
         /*policer_handle;*/     0,
         /*vpmt_accessing_mode*/ WP_IW_VPMT_ACCESSING_EGRESS_EXTERNAL,
         /*prefix_rmrk_fields*/  0,
         /*prefix_rmrk_offst*/   {0x00, 0x00, 0x00},
         /*forw_dscp_remark*/    WP_DISABLE,
         /*dscp_remark_offset;*/ 0
      };

   agg_host_config.txfunc = rx_host_channel;
   agg_host = WP_IwFlowAggregationCreate(WP_WINPATH(0), WP_IW_TRANS_BRIDGING_MODE, &agg_host_config);
...
void    WPE_CreateIwBportHost()
{
   ...
   WP_iw_agg_bridging agg_host_config = {0};
   ...
   status = WP_IwFlowAggregationModify (agg_host, WP_IW_FLOW_AGG_B_MOD_BPORT, &agg_host_config);
...
void    WPE_CreateFastEnetPortDevice()
{
   ...
   WP_port_enet enet_port_config =
      {
         /* pkt_limits */
         {
            /* max_tx_channels*/ NUM_OF_FAST_ENET_TX_CHANNELS,
            /* max_rx_channels*/ NUM_OF_FAST_ENET_RX_CHANNELS,
            /* pq_block_size*/   0,
            /* n_pq_blocks*/     0,
   ...
   port_enet = WP_PortCreate(WP_WINPATH(0), WP_PORT_ENET7, &enet_port_config);
   terminate_on_error(port_enet, "WP_PortCreate() Fast ENET");

   dev_enet = WP_DeviceCreate(port_enet, WP_PHY(0), WP_DEVICE_ENET, &enet_dev_config);
...
void    WPE_CreateHierEnetPortDevice()
{
   ...
   WP_port_enet enet_port_config =
      {
         /* pkt_limits */
         {
            /* max_tx_channels*/ NUM_OF_HIER_ENET_TX_CHANNELS,
            /* max_rx_channels*/ NUM_OF_HIER_ENET_RX_CHANNELS,
            /* pq_block_size*/   0,
            /* n_pq_blocks*/     0,
            /* emphy_devices*/   WP_UNUSED
         },
   ...
   WP_device_enet_ex enet_dev_ext_params_config =
      {
         ...
         /* rx_timestamp;*/     0,
         /*timestamp_type*/     0,
         /*max_tx_slow_channels*/  NUM_OF_HIER_ENET_TX_CHANNELS,      
      };
#endif
   ...
   port_hier_enet = WP_PortCreate(WP_WINPATH(0), WP_PORT_ENET8, &enet_port_config);
   terminate_on_error(port_hier_enet, "WP_PortCreate() Hierarchical ENET");

   dev_hier_enet = WP_DeviceCreate(port_hier_enet, WP_PHY(0), WP_DEVICE_ENET, &enet_dev_config);
   terminate_on_error(dev_hier_enet, "WP_DeviceCreate() Hierarchical Enet");


#if L3_SW_CHANNELS==1
         enet_dev_config.extended_params = &enet_dev_ext_params_config;
         status = WP_DeviceModify(dev_hier_enet,
                                  WP_DEV_MOD_ENET_TX_STATMODE | 
                                  WP_DEV_MOD_ENET_RX_STATMODE |
                                  WP_DEV_MOD_ENET_MAX_TX_SLOW_CHANNELS,
                                  &enet_dev_config);
         terminate_on_error(status, "WP_DeviceModify() Hierarchical Enet");
#endif
}
...
void    WPE_CreateHierEnetIwBport()
{
   ...
   terminate_on_error(bport_hier_enet," WPE_CreateIwBportEnet Hierarchical");
...
void    WPE_CreateFastEnetFlowAgg()
{
   ...
   WP_policer_slb_config slb_config =
      {
         /* leaky_bucket_size */    1000 // in milliseconds
      };
   ...
   WP_policer_v1 policer_config =
      {
         ...
         /* committed_info_rate */  20000000,   // in bits per second
   ...
   WP_iw_agg_bridging agg_enet_config =
      {
         ...
         /*rfcs*/                WP_IW_RFCS_DISABLE,
         /*l2_hdr_insert_mode*/  WP_IW_L2H_INSERT_DISABLE,
         /*encap_vlan_tag_mode*/ WP_IW_VLAN_TAG_ENABLE, // Enable = Untagged - AddDefaultVLAN, Tagged - Do not Strip VLAN
         /*interruptqueue;*/     WP_IW_IRQT1,
         /*error_pkt_mode*/      WP_IW_ERRPKT_DISCARD,
   ...
   for(i=0;i < NUM_OF_FAST_ENET_FAS; i++)
   {
      agg_enet_config.txfunc = tx_gbe_channel[0]; // FIXME ONLY ONE CHANNEL for all FAs untill the Fast L3 fifo bug will be fixed
      //agg_enet_config.policer_enable = WP_IW_POLICER_ENABLE;
      //agg_enet_config.policer_config = &policer_config;
      agg_enet[i] = WP_IwFlowAggregationCreate(WP_WINPATH(0), WP_IW_VLAN_AWARE_BRIDGING_MODE, &agg_enet_config);
      terminate_on_error(agg_enet[i], "WPE_CreateEnetIwFlowAgg() Fast Enet");
      agg_enet_config.new_dst_mac[5]++;
   ...
   agg_enet_config.l2_header_insert_mode = WP_IW_L2H_INSERT_ENABLE;
   memcpy(agg_enet_config.new_dst_mac, host_dst_mac, 6);
   agg_enet_change_mac = WP_IwFlowAggregationCreate(WP_WINPATH(0), WP_IW_VLAN_AWARE_BRIDGING_MODE, &agg_enet_config);
   terminate_on_error(agg_enet_change_mac, "WPE_CreateEnetIwFlowAgg Replace MAC()");
}
...
void    WPE_CreateHierEnetFlowAgg()
{
   ...
   WP_priority_vpmt_entry vlan_priority_map[8] = 
      {
         {
            /* priority */ 0, 
            /* deny_mode*/ WP_PRIORITY_ENTRY_PASS
         },
         {
            /* priority */ 1, 
            /* deny_mode*/ WP_PRIORITY_ENTRY_PASS  
         },
         {  2, WP_PRIORITY_ENTRY_PASS
         },
         {
            3, WP_PRIORITY_ENTRY_PASS
         },
         {
            4, WP_PRIORITY_ENTRY_PASS
         },
         {
            5, WP_PRIORITY_ENTRY_PASS
         },
         {
            6, WP_PRIORITY_ENTRY_PASS
         },
         {
            7, WP_PRIORITY_ENTRY_PASS
         }
      };

   vpmt_handle = WP_IwVlanPriorityMapCreate (0x0, WP_WINPATH(0), vlan_priority_map);
   terminate_on_error(vpmt_handle, "WP_IwVlanPriorityMapCreate");
   agg_enet_config.vpmt_handle = vpmt_handle;
   agg_enet_config.txfunc = l2_shaping_block;
   agg_enet_config.iw_port = bport_hier_enet;
   
   // Only one FA is needed to point to shaping block with many TX channels
   agg_hier_enet[0][0][0] = WP_IwFlowAggregationCreate(WP_WINPATH(0), WP_IW_VLAN_AWARE_BRIDGING_MODE, &agg_enet_config);
   terminate_on_error(agg_hier_enet[0][0][0], "WPE_CreateEnetIwFlowAgg Hierarchical()");

#else // No Shaping Block
   WP_U32 i,j,k;
   ...
   for(i = 0; i < NUM_OF_L1_GROUP; i ++)
   {
      for(j=0;j < NUM_OF_L2_GROUP; j++)
      {
         for(k=0;k < NUM_OF_HIER_ENET_FAS_PER_L2; k++)
         {
            ...
            terminate_on_error(agg_hier_enet[i][j][k], "WPE_CreateEnetIwFlowAgg Hierarchical()");
         }
      }
   }
#endif
...
//   WP_U32 i;
   ...
   // Fast Enet
   status = WP_IwRxBindingCreate(rx_gbe_channel, iw_sys, iw_qnode, &rx_binding_enet_config);
   terminate_on_error(status, "WPE_CreateIwSysEnetRxBinding() rx_binding_enet Fast");

//   for(i=0;i < NUM_OF_FAST_ENET_TX_CHANNELS; i++) // FIXME
   {
      status = WP_IwTxBindingCreate(tx_gbe_channel[0], WP_IW_TX_BINDING, &tx_binding_enet_config);
...
{  
   ...
   status = WP_IwRxBindingCreate(rx_gbe_hier_channel, iw_sys, iw_qnode, &rx_binding_enet_config);
   terminate_on_error(status, "WPE_CreateIwSysEnetRxBinding() rx_binding_enet Hier");
   for(i = 0; i < NUM_OF_L1_GROUP; i ++)
   {
      for(j=0;j < NUM_OF_L2_GROUP; j++)
      {
...
//   WP_U32 i;
   ...
   WP_fmu_shaping_wfq fmu_wfq_shaping_params =
      {
         /* weight */ 10000,
         /* flags */ 0,
      };
   ...
   WP_ch_enet enet_ch_config =
      {
         ...
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_WFQ,//WP_FMU_SHAPING_TYPE_STRICT,
         /* tx_shaping_params*/&fmu_wfq_shaping_params,//NULL,
         /* rx_maxsdu */       SDU_SIZE,
   ...
   rx_gbe_channel = WP_ChannelCreate(0x22, dev_enet, iw_qnode, WP_CH_RX, WP_ENET, &enet_ch_config);
   terminate_on_error(rx_gbe_channel, "WP_Channel_Create() Fast RX");

//    for(i=0;i < NUM_OF_FAST_ENET_TX_CHANNELS; i++) //FIXME
    {
//       enet_ch_config.tx_tq = i;
      tx_gbe_channel[0] = WP_ChannelCreate(0x11, dev_enet, iw_qnode, WP_CH_TX, WP_ENET, &enet_ch_config);
      terminate_on_error(tx_gbe_channel[0], "WP_Channel_Create() Fast TX");
   }
...
void    WPE_CreateHierHWEnetRxTxChannel()
{
...
#if L3_WFQ==1
   WP_fmu_shaping_wfq l3_wfq_shaping_params =
      {
         /* weight */ 10000,  /* weight in bytes */
         /* flags */  0,
      };
#elif L3_WRR==1
   WP_fmu_shaping_wrr l3_wrr_shaping_params =
      {
         /* weight */ 1,  /* weight in data unit size (cells / packets ) */
         /* flags */  0,  /* contains data unit (cells / packets) size */
      };
#elif L3_CIR_EIR_STRICT==1
   WP_fmu_shaping_cir_eir l3_cir_eir_shaping_params = 
      {
         /* cir;*/     3000000,  /* bits/second, 3Mbps */
         /* cbs;*/     1000,     /* Committed Burst Size in bits */
         /* eir;*/     1000000,  /* bits/second */
         /* ebs;*/     1000,     /* Committed Burst Size in bits */
         /* flags */   WP_FMU_SHAPING_CIR_EIR_SELECT_MODE_STRICT,
      };
#elif L3_SW_CHANNELS==1
   WP_pkt_shaping_cir cir_shaping_param = 
      {
         /* cir;*/     10000000,  /* bits/second */
         /* polling */ WP_SHAPING_POLL_AUTOD,
         /* cbs;*/     24288     /* Committed Burst Size in bits */
      };
#endif
   ...
   WP_ch_enet enet_ch_config =
      {
         ...
         /* tx_shaping_params*/&l3_wfq_shaping_params,
#elif L3_WRR==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_WRR,
         /* tx_shaping_params*/&l3_wrr_shaping_params,
#elif L3_CIR_EIR_STRICT==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_CIR_EIR,
         /* tx_shaping_params*/&l3_cir_eir_shaping_params,
#elif L3_SW_CHANNELS==1
         /* tx_shaping_type*/  WP_PKT_SHAPING_CIR,
         /* tx_shaping_params*/&cir_shaping_param,
#endif
         /* rx_maxsdu */       SDU_SIZE,
         /* tx_cwid */         WP_CW_ID_A, // Unused in HW mode
         /* tx_tq */           0,
   ...
   rx_gbe_hier_channel = WP_ChannelCreate(0x22, dev_hier_enet, iw_qnode, WP_CH_RX, WP_ENET, &enet_ch_config);
   terminate_on_error(rx_gbe_hier_channel, "WP_Channel_Create() Gige Hierarchical RX");
   ...
   for(i = 0; i < NUM_OF_L1_GROUP; i ++)
   {
      for(j=0;j < NUM_OF_L2_GROUP; j++)
      {
         for(k=0;k < NUM_OF_HIER_ENET_TX_CHANNELS_PER_L2; k++)
         {
            ...
            tx_gbe_hier_channel[i][j][k] = WP_ChannelCreate(0x11, l2_group_hier[i][j], iw_qnode, WP_CH_TX, WP_ENET, &enet_ch_config);
            terminate_on_error(tx_gbe_hier_channel[i][j][k], "WP_Channel_Create() Gige Hierarchical TX");
         }
      }
   }
}
...
void    WPE_CreateL1FMUGroups(void)
{
...
#if   L1_WFQ==1
   WP_fmu_shaping_wfq l1_wfq_shaping_params =
      {
         /* weight */ 10000,  /* weight in bytes */
         /* flags */  0,
      };
#elif L1_WRR==1
   WP_fmu_shaping_wrr l1_wrr_shaping_params =
      {
         /* weight */ 1,  /* weight in data unit size (cells / packets ) */
         /* flags */  0,  /* contains data unit (cells / packets) size */
      };
   ...
   WP_fmu_shaping_cir_eir l1_cir_eir_shaping_params = 
      {
         /* cir;*/     10000000,/* bits/second, 10Mbps*/
         /* cbs;*/     1000,    /* Committed Burst Size in bits */
         /* eir;*/     0,       /* bits/second */
         /* ebs;*/     1000,    /* Committed Burst Size in bits */
...
#elif L1_WFQ==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_WFQ,
         /* tx_shaping_params*/&l1_wfq_shaping_params,
#elif L1_WRR==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_WRR,
         /* tx_shaping_params*/&l1_wrr_shaping_params,
#elif L1_CIR_EIR==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_CIR_EIR,
...
#if (USE_SHAPING_BLOCK_L1 == 1)
   l1_shaping_block = WP_ShapingBlockCreate(WP_SYSHANDLE(WP_WINPATH(0)), WP_L1_GROUP, NUM_OF_L1_GROUP, WP_NUM_FIFOS_8);
   terminate_on_error(l1_shaping_block, "WP_ShapingBlockCreate");
   enet_group_l1_config.block_handle = l1_shaping_block;
#endif

   for(entry = 0; entry < NUM_OF_L1_GROUP; entry ++)
   {
      enet_group_l1_config.block_level = entry;
      l1_group_hier[entry] = WP_ShapingGroupCreate(dev_hier_enet, WP_SHAPING_GROUP_TYPE_PACKET, &enet_group_l1_config);
      terminate_on_error(l1_group_hier[entry] , "WP_ShapingGroupCreate l1_hier");
...
void    WPE_CreateL2FMUGroups(void)
{
...
#if   L2_WFQ==1
   WP_fmu_shaping_wfq l2_wfq_shaping_params =
      {
         /* weight */ 10000,  /* weight in bytes */
         /* flags */  0,
      };
#elif L2_WRR==1
   WP_fmu_shaping_wrr l2_wrr_shaping_params =
      {
         /* weight */ 1,  /* weight in data unit size (cells / packets ) */
         /* flags */  0,  /* contains data unit (cells / packets) size */
      };
   ...
   WP_fmu_shaping_cir_eir l2_cir_eir_shaping_params = 
      {
         /* cir;*/     10000000,  /* bits/second, 10Mpbs */
         /* cbs;*/     1000,      /* Committed Burst Size in bits */
         /* eir;*/     0,         /* bits/second */
         /* ebs;*/     1000,      /* Committed Burst Size in bits */
         /* flags */   WP_FMU_SHAPING_CIR_EIR_SELECT_MODE_RR,
...
#elif L2_WFQ==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_WFQ,
         /* tx_shaping_params*/&l2_wfq_shaping_params,
#elif L2_WRR==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_WRR,
         /* tx_shaping_params*/&l2_wrr_shaping_params,
#elif L2_CIR_EIR==1
         /* tx_shaping_type*/  WP_FMU_SHAPING_TYPE_CIR_EIR,
         /* tx_shaping_params*/&l2_cir_eir_shaping_params,
#endif
         /* num_fifos */	WP_NUM_FIFOS_8,
         /* block_handle */     0,
         /* block_level */      0, // not used without ShapingBlock
#if L3_SW_CHANNELS==1
         /* group_mode */       WP_MODE_SW,
#else
...
#if (USE_SHAPING_BLOCK_L2 == 1)
   l2_shaping_block = WP_ShapingBlockCreate(WP_SYSHANDLE(WP_WINPATH(0)), WP_L2_GROUP, NUM_OF_L2_GROUP, WP_NUM_FIFOS_8);
   terminate_on_error(l2_shaping_block, "WP_ShapingBlockCreate");
   enet_group_l2_config.block_handle = l2_shaping_block;
#endif
   ...
   for(i = 0; i < NUM_OF_L1_GROUP; i ++)
   {
      ...
      {
         enet_group_l2_config.block_level = entry; // used only if scenario with shaping block was chosen

         l2_group_hier[i][entry] = WP_ShapingGroupCreate(l1_group_hier[i], WP_SHAPING_GROUP_TYPE_PACKET, &enet_group_l2_config);
         terminate_on_error(l2_group_hier[i][entry] , "WP_ShapingGroupCreate l2_hier");
...
void    WPE_PolicerInit()
{
   ...
   memset(&atm_policer_limits, 0, 
          sizeof(WP_policer_limits_atm));
   memset(&v1_policer_limits, 0, 
          sizeof(WP_policer_limits_v1));
   memset(&policer_module_cfg, 0, 
          sizeof(WP_module_policer));
   ...
   status =  WP_ModuleInit(WP_SYSHANDLE(0),WP_WDDI_MODULE_POLICER, &policer_module_cfg);
      ...
      {
         {
            /* function */    WP_CW_PSU,
            /* cwid*/         WP_CW_ID_A,
            /* min_ch_rate */ 10000000,  // bps
            /* max_ch_rate */ 100000000  // bps
         },
...
void WPE_CreateDedicatedFlowAgg()
{
   ...
   dedicated_agg_hier_enet_config.reserved_agg_mode = WP_IW_PPPOE_DISCOVERY_TERMINATION;
   flow_agg_dedicated[i] =  WP_IwFlowAggregationCreate(0, WP_IW_VLAN_AWARE_BRIDGING_MODE, &dedicated_agg_hier_enet_config);
   terminate_on_error(flow_agg_dedicated[i], "WP_IwFlowAggregationCreate");
    
   i++;
   dedicated_agg_hier_enet_config.txfunc = rx_host_channel;
   dedicated_agg_hier_enet_config.iw_port = bport_host;
   dedicated_agg_hier_enet_config.reserved_agg_mode = WP_IW_ARP_TERMINATION;
   flow_agg_dedicated[i] =  WP_IwFlowAggregationCreate(0, WP_IW_VLAN_AWARE_BRIDGING_MODE, &dedicated_agg_hier_enet_config);
   terminate_on_error(flow_agg_dedicated[i], "WP_IwFlowAggregationCreate");
   ...
   dedicated_agg_hier_enet_config.txfunc = rx_host_channel;
   dedicated_agg_hier_enet_config.iw_port = bport_host;
   dedicated_agg_hier_enet_config.reserved_agg_mode = WP_IW_IGMP_TERMINATION;
   flow_agg_dedicated[i] =  WP_IwFlowAggregationCreate(0, WP_IW_VLAN_AWARE_BRIDGING_MODE, &dedicated_agg_hier_enet_config);
   terminate_on_error(flow_agg_dedicated[i], "WP_IwFlowAggregationCreate");
}
...
void    WPE_SetStaticForwardRules()
{
   ...
   WP_l2_forward forward_rule_config =  
      {
         /*dst mac[6];*/         {0x00,0x00,0x00,0x00,0x00,0x00},
         /*bport_tag;*/          0,
         /*vlan_id;*/            0,
         /*aggregation;*/        0,
...
// -> Enet
   ...
   for(i=0;i < NUM_OF_FAST_ENET_FAS; i++)
   {
   forward_rule_config.aggregation = agg_enet[i];
...
// -> Enet (with change of MAC)
   ...
   status = WP_IwMacAddressInsert(iw_sys, &forward_rule_config);     
   terminate_on_error(status, "WP_IwMacAddressInsert() to Enet with change of MAC"); 
...
#if USE_SHAPING_BLOCK_L2==1
   forward_rule_config.aggregation = agg_hier_enet[0][0][0];
   status = WP_IwMacAddressInsert(iw_sys, &forward_rule_config);     
   terminate_on_error(status, "WP_IwMacAddressInsert() -> HierarchicalEnet"); 
#else
 {
   WP_U32 i,j,k;
...
// -> Multicast to HierarchicalEnet
   ...
   for (i=0;i<NUM_OF_MC_GROUPS;i++) {
      ...
      status = WP_IwMacAddressInsert(iw_sys, &forward_rule_config);     
      terminate_on_error(status, "WP_IwMacAddressInsert() MC to HierarchicalEnet");  
      forward_rule_config.vlan_id++;
   }
#endif
...
/* Filled values and their default values */
   ...
   status =  WP_IwLearningQueueService(iw_sys, learning_queue, LEARNING_QUEUE_SIZE, &return_size);
...
void WPE_MemeberSetFill(void)
{
   ...
   for (i=0;i<MAX_VLAN_IDS;i++) {
      ...
      terminate_on_error(status, "WP_IwFlowAdd() WPE_MemeberSetFill Host");
   }    

   //MemeberSetFill Enet
   for (i=0;i<MAX_VLAN_IDS;i++) {
      bridge_cfg.flow_agg = agg_enet[i];
      bridge_cfg.vlan_id =  VLAN_TAG_1 + i;
      status = WP_IwFlowAdd(iw_sys, WP_FLOW_BRIDGEPATH, &bridge_cfg);
      terminate_on_error(status, "WP_IwFlowAdd() WPE_MemeberSetFill Enet");
   }
   //MemeberSetFill Hierarchical Enet

#if USE_SHAPING_BLOCK_L2==1
            bridge_cfg.flow_agg = agg_hier_enet[0][0][0];
            bridge_cfg.vlan_id =  VLAN_TAG_1;
            status = WP_IwFlowAdd(iw_sys, WP_FLOW_BRIDGEPATH, &bridge_cfg);
            terminate_on_error(status, "WP_IwFlowAdd() WPE_MemeberSetFill Hierarchical Enet");
#else
 {
   WP_U32     i,j,k;
...
void   WPE_AddBasicRules(void)
{
   ...
   terminate_on_error(flow_rule_handle[0][0], "WP_IwFlowAdd - Basic Rules");
...
//   sprintf(dfc_rule,  "n;n;n;n;n;n;n;n;0x.0.0.22.22.22;");
   ...
   flow_rule_handle[1][0] = WP_IwFlowAdd( iw_sys, WP_FLOW_CLASS_RULE, &class_cfg);
   terminate_on_error(flow_rule_handle[1][0], "WP_IwFlowAdd - Basic Rules");
...
//   sprintf(dfc_rule,  "n;n;n;n;n;n;n;n;0x.0.0.22.22.22;");
   ...
   flow_rule_handle[1][1] = WP_IwFlowAdd( iw_sys, WP_FLOW_CLASS_RULE, &class_cfg);
   terminate_on_error(flow_rule_handle[1][1], "WP_IwFlowAdd - Basic Rules");
#endif
...
//     WP_U32 i;
    ...
    if (status != WP_OK)
    {
       ...
       else terminate_on_error(status, "WP_HostReceive Error()");
    }
...
void    WPE_IWSendReceive(WP_U32 packets, WP_U8 dst_mac[])
{
   ...
   for (j=0;j<packets;j++) {
      WPE_Send_HostData(tx_host_channel, WP_DATA_IW, 0, dst_mac);
   }
...
void    WPE_L2SendReceiveEnet(WP_U32 packets, WP_U8 dst_mac[])
{
   ...
   for (j=0;j<packets;j++) {
      WPE_Send_HostData(tx_gbe_channel[0], WP_DATA_ENET, WP_ENET, dst_mac);
   }
...
void WPE_Send_HostData(WP_handle tx_channel, WP_data_type data_type, WP_U32 ifc_type, WP_U8 dst_mac[])
{
   ...
   WP_U8 hexa_buffer[PACKET_SIZE] = 
    ...
    memset (&data_unit,0, sizeof(WP_data_unit));
    memset (&segment,  0, sizeof(WP_data_segment));
    if ((data_type == WP_DATA_IW) || (ifc_type == WP_ENET))
    {
    ...
    else // L2 Data not used for Enet HS (only for ATM)
    {
   ...
   status = WP_HostSend(tx_channel, &data_unit );
    ...
    if ((data_type == WP_DATA_IW) || (ifc_type == WP_ENET))
    {
       if ( WP_ERR_WMM_FIFO_PUT == WP_ERROR(status )) {
          ...
          terminate_on_error(status, "WP_HostSend IW()");
    }
    
    else // for L2 host send only
    {
       terminate_on_error(status, "WP_HostSend L2()");

       status = WP_HostRecover(tx_channel, &data_unit);
       //WPE_DisplayHostRecoverStat(data_unit.status);
...
void WPE_ShowForwardingRules(void)
{
   ...
   printf("------------------------------------\n");
   printf("MAC\t\tVLAN\tBPORT\tFLOW\t\tAGING\tSTATIC\tDENY SA\tPROCESS\tUNK MODE\n");
...
void    WPE_CLI(void)
{
   ...
   while ((InputBuf[0] != 'q') && (InputBuf[0] != 'k') ) {
      ...
      printf("       HostTermination: 7-bPortHost,  8-FlowAggHost, q-Exit, k-Kill(noDriverRelease) \n");
      printf("       FastFMU: l-FastTxCh,  m-FastDevice \n");
      printf("       HierFMU: n-HierTxCh,  o-HierL2, p-HierL1, r-HierDevice, \n");
      printf("       z- Send Packets \n");
      printf("Debug: b-destroy L2PI TCPT base e-Destroy FIFO 14 d-Enable Debug base\n");
      gets(InputBuf);

      switch (InputBuf[0]) {
         case '1':
            printf("************************* System Stats for Enet Port ********************** \n");
            WPE_DisplayIWSystemStats(port_enet);
            printf("************************* Enet Device Stats ******************************* \n");
            WPE_DisplayEnetDeviceStat(dev_enet);
            break;
         case '2':
            printf("************************* Enet bPort Stats ******************************** \n");
            WPE_DisplayIWPortStats(bport_enet);
            break;
         case '3':
            printf("************************* HierarchicalEnet FlowAgg[0][0][0] Stats ********************** \n");
            WPE_DisplayFlowAggStats(agg_hier_enet[0][0][0]);
            printf("************************* HierarchicalEnet FlowAgg[0][0][1] Stats ********************** \n");
            WPE_DisplayFlowAggStats(agg_hier_enet[0][0][1]);
            break;
         case '4':
            printf("************************* System Stats for HierarchicalEnet Port ***************** \n");
            WPE_DisplayIWSystemStats(port_hier_enet);
            printf("************************** HierarchicalEnet Device Stats ************************* \n");
            WPE_DisplayEnetDeviceStat(dev_hier_enet);
            break;
         case '5':
            printf("************************* HierarchicalEnet bPort Stats *************************** \n");
            WPE_DisplayIWPortStats(bport_hier_enet);
            break;
         case '6':
            printf("************************* Enet FlowAgg Stats [0] ****************************** \n");
            WPE_DisplayFlowAggStats(agg_enet[0]);
            printf("************************* Enet FlowAgg Stats [1]****************************** \n");
            WPE_DisplayFlowAggStats(agg_enet[1]);
            break;
         case '7':
            printf("************************* Host bPort Stats ******************************** \n");
            WPE_DisplayIWPortStats(bport_host);
            break;
         case '8':
            printf("************************* Host FlowAgg Stats ****************************** \n");
         ...
         case 'f':
            ...
            break;
         case 's':
            printf("********************* WPE_ShowAvailableBusesMemory **********************   \n");
            WPE_ShowAvailableBusesMemory();
            break;
         case 'q':
            printf("************************* Exiting with Driver Release ********************* \n");
            WP_DriverRelease();
            break;
         case 'k':
            printf("************************* Exiting Driver Still Running! ******************* \n");
            break;
         case 'l':
            printf("************************* Fast[0] L3 channel FMU ******************* \n");
            WP_Display(0, tx_gbe_channel[0], WP_DISPLAY_FMU_ONLY, &destination);
            break;
         case 'm':
            printf("************************* FAST device FMU ******************* \n");
            WP_Display(0, dev_enet, WP_DISPLAY_FMU_ONLY, &destination);
            break;
         case 'n':
            printf("************************* Hier[0][0][0] L3 channel FMU ******************* \n");
            WP_Display(0, tx_gbe_hier_channel[0][0][0], WP_DISPLAY_FMU_ONLY, &destination);
            //WP_Display(0, tx_gbe_hier_channel[0][0][1], WP_DISPLAY_FMU_ONLY, &destination);
            break;
         case 'o':
            printf("************************* L2[0][0] group hier FMU ******************* \n");
            WP_Display(0, l2_group_hier[0][0], WP_DISPLAY_FMU_ONLY, &destination);
            break;
         case 'p':
            printf("************************* L1[0] group hier FMU ******************* \n");
            WP_Display(0, l1_group_hier[0], WP_DISPLAY_FMU_ONLY, &destination);
            break;
         case 'r':
            printf("************************* Hier device FMU ******************* \n");
            WP_Display(0, dev_hier_enet, WP_DISPLAY_FMU_ONLY, &destination);
            break;
         case 'z':
         {
            
           WP_U8 tmp_dst_mac[6];
           memcpy(tmp_dst_mac, enet_dst_mac,6);
           for (i=0;i<8;i++)
           {
              WPE_Send_HostData(tx_host_channel, WP_DATA_IW, 0, tmp_dst_mac);
              tmp_dst_mac[5]++;
         ...
         case 'b':
            ...
            printf("************************* Destroying L2PI TCPT base ******************* \n");
            WPE_CreateBusError1();
            break;
         case 'e':
            printf("************************* Creating Bus Error ******************* \n");
            printf("************************* Destroying FIFO 14 ******************* \n");
            WPE_CreateBusError2();
            break;
         case 'd':
            printf("************************* Enabling Debug base ******************* \n\n\n");
            WPE_EnableDebugBaseLog();
            break;
         default:
            printf("Wrong Key.\n\n");
            break;
...
static void WPE_LogErrCb( const WP_CHAR *file, const WP_CHAR *function, WP_S32 line, WP_U32 error_id)
{
   printf( "ERR %s:%s:%d - %s\n",  file, function, line, WP_error_name[error_id]);
}

#if 0
static void WPE_LogFunctionCb(WP_CHAR *pi_file, WP_CHAR *pi_func, WP_U32   i_line)
{
...
void WPE_RegisterLogCbFunc()
{
   ...
   status = WP_ControlRegister(WP_DEBUG_CALLBACK_FILE_LINE_ERROR, WPE_LogErrCb);
...
void WPE_CreateBusError1(void)
{
   //Destroy L2PI TCPT base- create bus error:
   // The address of b129 is 0x1ec010a5, we will fill it with garbage

//   WPU_WinutilCall("mm 0x1ec010a5 0xcafecafe");
   WPU_WinutilCall("mm 0x1e001204 0xcafecafe");
   WPU_WinutilCall("md 0x1ec010a5 :4");
}

void WPE_CreateBusError2(void)
{
   //Destroy FIFO 14- create bus error:
   
   WPU_WinutilCall("mm MAP_WM0_F14SA 0x00000000");
   WPU_WinutilCall("md MAP_WM0_F14SA :4");
}

//Works only on Winmon:
void WPE_EnableDebugBaseLog(void)
{
// Debug base is 0x1c5b1000- using 0xbc5b1000 for Winmon cached memory
   
   WP_U32 *base;
   base = (WP_U32 *)(0xbc5b1000);
   WP_MEM_SET(*base, 1);
