...
#ifdef WDS_SL
#pragma message ("Compiling for SuperLight")
#endif

#include "wt_debug_menu.h"
#include "wt_new_automation.h"
...
#if _WP_MODULE_WIMAX_ENABLE_
   {"WIMAX_UL_PHY_QUEUE", WP_SA_WIMAX_UL_PHY_QUEUE},
   {"WIMAX_UL_SDU_BUILDER", WP_SA_WIMAX_UL_SDU_BUILDER},
   {"WIMAX_DL", WP_SA_WIMAX_DL},
   {"WIMAX_UL", WP_SA_WIMAX_UL},
#endif
...
display_cmd DispCmds[] = {
#if _WP_MODULE_ABIS_ENABLE_
   {"ABIS", WP_DISPLAY_ABIS},
#endif
   {"ASU", WP_DISPLAY_ASU},
   {"BIU", WP_DISPLAY_BIU},
   {"BRG", WP_DISPLAY_BRG},
   ...
   {"CLOCK_RECOVERY", WP_DISPLAY_CLOCK_RECOVERY},
#endif
   {"CID", WP_DISPLAY_CID},
...
#if _WP_MODULE_LTE_FW_MAC_ENABLE_
   {"LTE_MAC", WP_DISPLAY_LTE_MAC},
#endif /* _WP_MODULE_LTE_FW_MAC_ENABLE_ */
#if _WP_MODULE_LTE_FW_RLC_ENABLE_
   {"LTE_RLC", WP_DISPLAY_LTE_RLC},
#endif /* _WP_MODULE_LTE_FW_RLC_ENABLE_ */
   {"MFR", WP_DISPLAY_MFR},
...
void CLI_setWPID (WP_CHAR * StrPrm)
{
   ...
   {
      WPU_DUMP
         ("Usage: <wpid>: If operating more than one Winpath, select Winpath number for API functions\n");
      return;
   }
...
void CLI_WinUtilTraceEnable_Serial (WP_CHAR * StrPrm)
{
   ...
   if (res != 1)
   {
      ...
      WPU_DUMP ("Parameters: \n");
      WPU_DUMP
         ("<serial_name>: Serial name (note: not index), See \"Trace Specific menu\" option 3, \"List Trace Serials\"\n");
      return;
   }
   CLI_WinUtilCall (strParam, StrPrm);
}
...
void CLI_WinUtilModifyTA_Table (WP_CHAR * StrPrm)
{
   ...
   res = sscanf (StrPrm, "%*d %d %d %d %d %x",
                 &baseIndex, &tableIndex, &entryIndex, &tableSize,
                 &newVal);
   if (res != 3)
   {
      WPU_DUMP ("Invalid number of arguments\n");
   ...
   F_menu_TermOnError (status,
                       "WPU_TA(0, baseIndex, WPU_TA_CMD_GET_ONE_BASE,ta)",
                       __LINE__);
   sprintf (strParam, "mm %X %X",
            ta.bases_info[baseIndex].addr + tableIndex * tableSize +
            entryIndex * (4), newVal);
...
void CLI_WinUtilDisplayTA_Table (WP_CHAR * StrPrm)
{
   ...
   res =
      sscanf (StrPrm, "%*d %d %d %d", &baseIndex, &entrySize, &entryIndex);
   if (res < 3)
   {
      WPU_DUMP ("Invalid number of arguments\n\n");
      ...
      WPU_DUMP ("Parameters:\n");
      WPU_DUMP
         ("<baseIndex>: Base Register Index, See \"Winutil Cmd: tab\" for list of registers\n\n");
   ...
   sprintf (strParam, "md %X :%d",
            (ta.bases_info[baseIndex].addr & 0xFFFFFFF0) + entryIndex * entrySize,
            entrySize);
...
void CLI_WinUtilDisplayAllTraceSerials ()
{
   ...
   wpu_serial_info *serial_map_table =
      (wpu_serial_info *) WPUI_HwSerialMapTable ();

   WPU_DUMP ("Listing Serial Names:\n");
   for (serial_index = 0;
        (serial_index < WPU_SERIAL_LAST_ID)
        && (serial_map_table[serial_index].serial_name != NULL);
        serial_index++)
   {
...
void CLI_SYSCMD_GetTRS (WP_CHAR * StrPrm)
{
   ...
   if (WP_SysCommand (wpid_context, WP_SYSCMD_READ_TRS_INFO, &params) ==
       WP_OK)
   {
      WPU_DUMP ("Serial name: %s, max TRS #: %d\n", TRSserials[index].name,
                params.entry.max_trs);
      WPU_DUMP ("TRS Masks: ");
      for (i = 0; i < WINFARM_NUMBER; i++)
      {
         WPU_DUMP ("WF%d: %04X  ", i + 1, params.entry.trs_mask[i]);
         if (i % 2 == 1)
            WPU_DUMP (" ");
      }
      WPU_DUMP ("\n");
...
void CLI_SYSCMD_SetTRS (WP_CHAR * StrPrm)
{
...
#ifdef WP_HW_WINPATH3
   res =
      sscanf (StrPrm, "%*d %d %d %X %X %X %X", &input, &max_trs,
              &trs_mask_pattern[0], &trs_mask_pattern[1],
              &trs_mask_pattern[2], &trs_mask_pattern[3]);
   params.serial_name = input;
   if (res < 6)
#endif
#ifdef WP_HW_WINPATH2
      res =
         sscanf (StrPrm, "%*d %d %d %X %X", &input, &max_trs,
                 &trs_mask_pattern[0], &trs_mask_pattern[1]);
   params.serial_name = input;
   if (res < 4)
#endif
   {
      WPU_DUMP ("Invalid number of arguments\n\n");
      WPU_DUMP
         ("Usage:<trace_serial_index> <max_trs> <trs_mask_pattern>\n\n");
      WPU_DUMP ("Parameters: \n");
      WPU_DUMP ("<max_trs>: Max TRS value to set\n\n");
      WPU_DUMP
         ("<trs_mask_pattern>: TRS Mask pattern of hexadecimal inputs,ie: AAAA BBBB CCCC DDDD\n");
      WPU_DUMP ("<trace_serial_index>: \n");
      CLI_SYSCMD_PrintTRSserials ();
      WPU_DUMP ("\n\n");
      return;
   ...
   status =
      WP_SysCommand (wpid_context, WP_SYSCMD_WRITE_TRS_INFO, &params);
   F_menu_TermOnError (status,
                       "WP_SysCommand(wpid, WP_SYSCMD_WRITE_TRS_INFO, &params)",
                       __LINE__);
   if (status == WP_OK)
   {
...
void CLI_SYSCMD_PrintTRSserials ()
{
   ...
   if (cmds % 3 == 1)
   {
      for (i = 0; i < rows; i++)
      {
         ...
         if (strlen (TRSserials[i].name) < 10)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03d) %s\t", TRSserials[i + rows + 1].index,
                   TRSserials[i + rows + 1].name);
         if (strlen (TRSserials[i + rows + 1].name) < 10)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03d) %s\n", TRSserials[i + 2 * rows + 1].index,
                   TRSserials[i + 2 * rows + 1].name);
      }
   ...
   else if (cmds % 3 == 2)
   {
      for (i = 0; i < rows; i++)
      {
         ...
         if (strlen (TRSserials[i + rows + 1].name) < 10)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03d) %s\n", TRSserials[i + 2 * rows + 2].index,
                   TRSserials[i + 2 * rows + 2].name);
      }
      WPU_DUMP ("(%03d) %s\t", TRSserials[i].index, TRSserials[i].name);
      if (strlen (TRSserials[i].name) < 10)
         WPU_DUMP ("\t");
   ...
   else
   {
      for (i = 0; i < rows; i++)
      {
         ...
         if (strlen (TRSserials[i].name) < 10)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03d) %s\t", TRSserials[i + rows].index,
                   TRSserials[i + rows].name);
         if (strlen (TRSserials[i + rows].name) < 10)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03d) %s\n", TRSserials[i + 2 * rows].index,
                   TRSserials[i + 2 * rows].name);
      }
   }
   WPU_DUMP ("\n\n");
}
...
void CLI_Bus_ECCcheck ()
{
   ...
   sprintf (strParam, "md %s :%d",
            STRINGIFY_ (MAP_SIU3_REG_ECC_DEBUG_CONF0), 4);
   WPU_WinutilCall (strParam);
   WPU_DUMP ("MAP_SIU3_REG_ECC_DEBUG_CONF1: ");
   sprintf (strParam, "md %s :%d",
            STRINGIFY_ (MAP_SIU3_REG_ECC_DEBUG_CONF1), 4);
   WPU_WinutilCall (strParam);
   WPU_DUMP ("MAP_SIU3_REG_ECC_ERROR_COUNTER: ");
   sprintf (strParam, "md %s :%d",
            STRINGIFY_ (MAP_SIU3_REG_ECC_ERROR_COUNTER), 4);
   WPU_WinutilCall (strParam);
   ...
   sprintf (strParam, "md %s :%d", STRINGIFY_ (MAP_Global_BIU_SD_Status),
            8);
   WPU_WinutilCall (strParam);
...
WP_status WTI_DebugAlloc (const WP_CHAR * file, WP_U32 line, WP_U32 bus,
                          WP_U32 size, WP_U32 alignment,
                          const WP_CHAR * purpose)
{
   WPU_DUMP ("file = %s , line = %d  bus = %d  size = %d , %s \n", file,
             line, bus, size, purpose);
   return WP_OK;
}
...
void CLI_WDDI_Allocation (void)
{
   ...
   status =
      WP_ControlRegister (WP_DEBUG_CALLBACK_WDDI_ALLOCATION,
                          &WTI_DebugAlloc);
   if (status == WP_OK)
...
void CLI_APILevel_StartLog ()
{
   ...
   status =
      WP_ControlRegister (WP_DEBUG_CALLBACK_WDDI_CALL,
                          &WT_CallTraceCallback);
   if (status == WP_OK)
...
void CLI_ShowAllDisplayCmds ()
{
   ...
   if (cmds % 3 == 1)
   {
      for (i = 0; i < rows; i++)
      {
         ...
         if (strlen (DispCmds[i].name) < 11)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03X) %s", DispCmds[i + rows + 1].index,
                   DispCmds[i + rows + 1].name);
         if (strlen (DispCmds[i + rows + 1].name) < 15)
            WPU_DUMP ("\t");
         if (strlen (DispCmds[i + rows + 1].name) < 11)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03X) %s\n", DispCmds[i + 2 * rows + 1].index,
                   DispCmds[i + 2 * rows + 1].name);
      }
   ...
   else if (cmds % 3 == 2)
   {
      for (i = 0; i < rows; i++)
      {
         ...
         if (strlen (DispCmds[i + rows + 1].name) < 11)
            WPU_DUMP ("\t");
         WPU_DUMP ("(%03X) %s\n", DispCmds[i + 2 * rows + 2].index,
                   DispCmds[i + 2 * rows + 2].name);
      }
      WPU_DUMP ("(%03X) %s\t", DispCmds[i].index, DispCmds[i].name);
      if (strlen (DispCmds[i].name) < 11)
         WPU_DUMP ("\t");
...
void CLI_DisplayCmd (WP_CHAR * StrPrm)
{
   ...
   if (res < 1)
   {
      ...
      WPU_DUMP ("\n");
      WPU_DUMP
         ("<display_cmd_index>: Hexadecimal value of a WDDI handle or a WP_Display command\n");
...
void CLI_ListQnodes ()
{
   ...
   status =
      WP_Display (wpid_context, WP_DISPLAY_QNODE, WP_DISPLAY_RAW_REGISTERS,
                  NULL);
}
...
void CLI_WP_QNodeStatus (WP_CHAR * StrPrm)
{
   ...
   if (res < 1)
   {
      ...
      WPU_DUMP ("Parameters: \n");
      WPU_DUMP
         ("<qnode_handle>: Qnode Handle in Hexadecimal format, See Qnode display menu option 2, \"List all Qnode handles\"\n\n");
      return;
   }
 ...
 *
 ****************************************************************************/
void WT_ExampleDebugAreaCallback (WP_U32 wpid, WP_U32 size_allocated,
                                  WP_U32 * host_ptr)
{
...
void CLI_DebugAreDumpPartial (WP_CHAR * StrPrm)
{
   ...
   status =
      WP_ControlRegister (WP_DEBUG_CALLBACK_DEBUG_AREA_LOG,
                          &WT_ExampleDebugAreaCallback);
...
void CLI_gen_helpfile (void)
{
   ...
   WPU_DUMP
      ("\n*********************************************************\n");
   WPU_DUMP
      ("I. Parameter input: Parameter inputs to menu options are read together with the user's command input.\n\n");
   WPU_DUMP
      ("   Ex: Example Menu \n   1. WinUtil Cmd: md (Memory Display)\n   2. WP_SysCmd: Get TRS Allocation\n");
   WPU_DUMP
      ("   \n   Command: \n   \"1 0x10000000 :4\" will execute \n   \"md 0x10000000 :4\"\n\n");
   WPU_DUMP
      ("II. WinUtil Cmds can access the WinUtil help files with the \"-help\" parameter\n");
   WPU_DUMP
      ("   Ex: To view \"md\" help file from Example Menu given above, enter command: \"1 -help\",\n\n");
   WPU_DUMP
      ("III. Commands that require parameters can display the parameter format if no parameters provided\n");
   WPU_DUMP
      ("   Ex: To view the parameters for \"Get TRS Allocation\" from Example Menu given above,\n");
...
Y_MenuEntry V_Menu_FatalError[] = {
   {FATAL_MENU_SIZE, TRUE, "Fatal Error menu: Bus Error Functions",
    {V_Menu_Debug}},
#ifdef linux
...
#endif
   {K_Leaf, TRUE, "Print registers DTCR1 - DTCR4", {CLI_DisplayDTCR}},
   {K_Leaf, TRUE_M, "Print Bus ECC registers", {CLI_Bus_ECCcheck}},
   {K_Leaf, TRUE, "Print TA PC current value", {CLI_DisplayTA_PCs}},
#ifdef WP_HW_WINPATH3
   {K_Leaf, TRUE_M, "Print AA PCT configuration",
    {CLI_DisplaySA_PCT_Table}},
#else
   {K_Leaf, TRUE_M, "Print SA PCT configuration",
    {CLI_DisplaySA_PCT_Table}},
#endif
   {K_Leaf, TRUE,
    "wfc reg <all>  (WinUtil: WMM Check Corrupted FIFO by HW)",
    {CLI_DisplayFifo_CorruptedNumbers}},
   ...
   {K_Leaf, TRUE_M, "Versions and Build times",
    {CLI_All_VersionBuildTime}},
   {K_Leaf, HELP, "help", {CLI_gen_helpfile}}
};
...
Y_MenuEntry V_Menu_MemAndLocks[] = {
   ...
   {K_Leaf, TRUE,
    "Get table from TA base  <baseIndex> <entrySize> <entryIndex>",
    {CLI_WinUtilDisplayTA_Table}},
   {K_Leaf, TRUE_M, "tab  (WinUtil: Print TA bases)",
    {CLI_WinUtilTA_Bases}},
   {K_Menu, TRUE, "Debug base display menu", {V_Menu_DebugBaseDump}},
   {K_Menu, TRUE, "Qnode display menu", {V_Menu_Qnode}},
   {K_Leaf, TRUE_M, "bmap (WinUtil: Print Bus Mapping Table)",
    {CLI_WinUtilBmap}},
   {K_Leaf, TRUE, "Print Memory Available", {CLI_PrintMemoryAvailable}},
   {K_Leaf, TRUE, "Enable WDDI Memory Allocation Log",
    {CLI_WDDI_Allocation}},
   {K_Leaf, TRUE_M, "Disable WDDI Memory Allocation Log",
    {CLI_WDDI_AllocationEND}},
   {K_Leaf, HELP, "help", {CLI_gen_helpfile}}
};
...
Y_MenuEntry V_Menu_DisplayLogAndVersions[] = {
   ...
   {K_Leaf, TRUE_M, "WinUtil Version, build time",
...
Y_MenuEntry V_Menu_TraceSpecific[] = {
   ...
   {K_Leaf, TRUE, "tb init         (WinUtil: Enable trace)",
    {CLI_WinUtilTraceStart}},
   {K_Leaf, TRUE,
    "tb init -serial (WinUtil: Enable trace on specific serial)",
    {CLI_WinUtilTraceEnable_Serial}},
   {K_Leaf, TRUE, "List Trace Serials",
    {CLI_WinUtilDisplayAllTraceSerials}},
   {K_Leaf, TRUE, "tb status       (WinUtil: Print Trace options status)",
    {CLI_WinUtilTraceOptionsStatus}},
   {K_Leaf, TRUE, "tb dump         (WinUtil: Trace Dump)",
    {CLI_WinUtilTraceDump}},
#ifdef linux
...
Y_MenuEntry V_Menu_WinUtilCmd[] = {
   ...
   {K_Leaf, TRUE_M, "sysu   (WinUtil: Display Bus Utilization)",
    {CLI_WinUtilWingineBusUtilization}},
#endif
   {K_Leaf, TRUE, "dmap   (WinUtil: Print Device Mapping Table)",
    {CLI_WinUtilDmap}},
   {K_Leaf, TRUE, "bmap   (WinUtil: Print Bus Mapping Table)",
    {CLI_WinUtilBmap}},
   {K_Leaf, TRUE, "smap   (WinUtil: Print Serial Mapping Table)",
    {CLI_WinUtilSA_TRSLimits}},
   {K_Leaf, TRUE_M, "hbm    (WinUtil: Bus Error Check)",
    {CLI_WinUtilBusErrCheck}},
   {K_Leaf, TRUE, "map    (WinUtil: Print Register Mappings)",
    {CLI_WinUtilMemoryMap}},
   {K_Leaf, TRUE, "mc     (WinUtil: Memory Check)",
    {CLI_WinUtilMemoryCheck}},
   {K_Leaf, TRUE, "md     (WinUtil: Memory Display)",
    {CLI_WinUtilMemoryDisplay}},
   {K_Leaf, TRUE, "mf     (WinUtil: Memory Fill)",
    {CLI_WinUtilMemoryFill}},
   {K_Leaf, TRUE_M, "mm     (WinUtil: Memory Modify)",
    {CLI_WinUtilMemoryModify}},
   {K_Leaf, TRUE, "tab    (WinUtil: Print TA base registers)",
    {CLI_WinUtilTA_Bases}},
   {K_Leaf, TRUE, "taom   (WinUtil: Print TA order monitors)",
    {CLI_WinUtilOrderMonitorDisplay}},
   {K_Leaf, TRUE, "taot   (WinUtil: Print TA order registers)",
    {CLI_WinUtilTAOrderRegisters}},
   {K_Leaf, TRUE, "trs    (WinUtil: Print TRS registers)",
    {CLI_WinUtilTRSDump}},
   {K_Leaf, TRUE, "tast   (WinUtil: Print Current TRS status)",
    {CLI_WinUtilCurrentTRSStatus}},
   {K_Leaf, TRUE_M, "trsa   (WinUtil: Print TRS Serial Allocation)",
    {CLI_WinUtilTRS_SerialAllocation}},
   {K_Leaf, TRUE, "fmum   (WinUtil: Check FMU address errors)",
    {CLI_WinUtilFmum}},
#ifdef WP_HW_WINPATH3
   {K_Leaf, TRUE, "freq   (WinUtil: Change Bus frequency)",
    {CLI_WinUtilFreq}},
#endif
   {K_Leaf, TRUE, "core   (WinUtil: Dump memory partition to file)",
    {CLI_WinUtilCoreDump}},
   {K_Leaf, TRUE_M, "ver    (WinUtil: Print Version and Buildtimes)",
    {CLI_WinUtilDebugVersionInfo}},
   {K_Menu, TRUE, "wfc menu (WinUtil: WMM FIFO specific menu)",
    {V_Menu_WMMFIFOSpecific}},
   {K_Menu, TRUE, "tb menu  (WinUtil: Trace specific menu)",
    {V_Menu_TraceSpecific}},
...
Y_MenuEntry V_Menu_Debug[] = {
   ...
   {K_Menu, TRUE, "Display,Log & Versions menu",
    {V_Menu_DisplayLogAndVersions}},
