...
void App_CheckStats(WP_status *test_result)
{
   ...
   *test_result = WP_OK;
   ...
   if (flow_stats.forward_packet != APP_MAX_PKTS_NUM){
      *test_result = (-1);
      printf("IMA trans bridge egress 0 forward_packet check fail!(%4d)\n", ii);
   }
   ...
   if (flow_stats.forward_packet != 0){
      *test_result = (-1);
      printf("IMA trans bridge egress 1 forward_packet check fail!(%4d)\n", ii);
   }
   ...
   if (pwe3_flow_stats.forwarded_packets != APP_MAX_PKTS_NUM){
      *test_result = (-1);
      printf("IMA PWE3 ingress flow agg forwarded_packets check fail!(%4d)\n", ii);
   }
