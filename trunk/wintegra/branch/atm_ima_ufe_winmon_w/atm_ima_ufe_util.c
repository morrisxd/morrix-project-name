...
extern int tx_safe;
void display_events_noima (void);
...
#define APP_DATA_LENGTH 80
   ...
   while (!tx_safe)
   {
      printf ("i'm waiting for tx safe\n");
      WP_Delay (100000);
...
void App_EventTxIndicatePolled (WP_tag tag, WP_U32 data, WP_U32 info)
{
   add_task (irq_task_list, WP_EVENT_POLLED_TX_INDICATE, tag, NULL);
}
