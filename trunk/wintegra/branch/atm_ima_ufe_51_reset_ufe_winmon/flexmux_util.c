...
{
	printf ("WTI_UFE_FRAMER_PEEK_RESPONSE_CALLBACK called\n");
...
#define WTI_FlexmuxCheckStatus(m,s,l)	WTI_FlexmuxCheckStatusX(m,s,l,__FILE__)
void WTI_FlexmuxCheckStatusX (WP_CHAR * Message, WP_U8 status, WP_U32 line, char *f)
{
   extern char *OMIINO_FRAMER_ErrorCodeToTxt(WP_U32 ErrorCode);
   if (status)
   {
      printf("Test Abort: function %s in file(%s)Line:%d returned Error code %s\n",
             Message, f, line, OMIINO_FRAMER_ErrorCodeToTxt((WP_U32) status));
      exit(-1);
   }
   else
   {
      ...
      for (j = 0x88000028; j <= 0x88000e28; j += 0x200)
		{
         WPX_FRMR_DEV_DIAG_Poke(0, 0, j, 0x00000001);
			printf ("WPX_FRMR_DEV_DIAG_Poke: addr(%x)\n", j);
		}
...
//#endif
   ...
   else if(type == WPX_UFE_FRAMER_WUFE_SDH_TYPE_VC4_4C)
   {
	   WT_ufe_line_sdh_params *sdh_line_params = (WT_ufe_line_sdh_params*) ufe_line_params;
	   line_end_point.u.SDH.stm4 = sdh_line_params->stm4;
	   line_end_point.u.SDH.stm1 = sdh_line_params->stm1;
...
// printf ("pdh port(%d) line timing(%d),force to(0)\n", client_port_id, line_timing);
   /* Set the PDH Tx line timing  */
   status = WPX_FRMR_SOCKET_PDH_SetClkRec(0, flexmux_id, client_port_id, /*line_timing*/0);
