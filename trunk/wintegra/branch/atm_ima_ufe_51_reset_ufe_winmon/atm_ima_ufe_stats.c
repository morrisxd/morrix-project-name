...
void m_showGrpStatus (void)
{
	...
	int i = 0;
		...
		printf ("=================================================\n");
	        ...
	        printf("group_state[0].fe_state           =%s\n", ima_fe_s(group_state[i].fe_state));
...
void App_ShowStats(void)
{
	...
	memset (&s_hs_enet, 0, sizeof(WP_stats_enet));
			...
			case 'd':
				{
					...
					memset (&emphy_stats, 0, sizeof(WP_stats_emphy));
					printf("-------------------------------\n");
					printf(" Emphy statisics port          \n",ii);
					printf("-------------------------------\n");
					printf ("pos_err(%d)\n", emphy_stats.pos_err);
					printf("rx_unsupported_device(%d)\n", emphy_stats.rx_unsupported_device);
					printf("tx_unsupported_device(%d)\n", emphy_stats.tx_unsupported_device);
					printf("rx_err_parity(%d)\n", emphy_stats.rx_err_parity);
					printf("rx_err_sop_eop(%d)\n", emphy_stats.rx_err_sop_eop);
				}
			break;
			case '1':
				...
				printf("-------------------------------\n");
				App_ShowEnetDeviceStats(h_device_enet1, &s_hs_enet_x[0]);
				printf("-------------------------------\n");
				printf(" ENET2 statisics               \n");
				printf("-------------------------------\n");
				App_ShowEnetDeviceStats(h_device_enet2, &s_hs_enet_x[1]);
	printf ("+++++\n");

	printf(" rx_bytes[1]               : %d\n", (WP_U32)s_hs_enet_x[0].rx_bytes);
	printf(" rx_packets[1]             : %d\n",(WP_U32)s_hs_enet_x[0].rx_packets);
	printf(" tx_bytes[1]               : %d\n", (WP_U32)s_hs_enet_x[0].tx_bytes);
	printf(" tx_packets[1]             : %d\n",(WP_U32)s_hs_enet_x[0].tx_packets);
	printf(" rx_bytes[2]               : %d\n", (WP_U32)s_hs_enet_x[1].rx_bytes);
	printf(" rx_packets[2]             : %d\n",(WP_U32)s_hs_enet_x[1].rx_packets);
	printf(" tx_bytes[2]               : %d\n", (WP_U32)s_hs_enet_x[1].tx_bytes);
	printf(" tx_packets[2]             : %d\n",(WP_U32)s_hs_enet_x[1].tx_packets);
				if(ch== '1')  
					break;
			case '2':
				printf("-------------------------------\n");
...
void App_ShowBridgePortStats(WP_handle bport_handle, WP_bport_stats bport_stats)
{
	...
	if (bport_stats.discard_dfc_filter[0] )
	...
	if (bport_stats.discard_pce_filter[0] )
...
void App_ShowPwe3FlowAggStats(WP_handle flow_agg_handle, WP_iw_pwe3_stats flow_stats)
{
	...
	if (flow_stats.forwarded_packets)
...
char * ima_fe_s (int n)
{
	...
	case WP_IMA_FE_GSM_START_UP:
		sprintf (imass, "%s", "WP_IMA_FE_GSM_START_UP");
		break;
	case WP_IMA_FE_GSM_START_UP_ACK:
		sprintf (imass, "%s", "WP_IMA_FE_GSM_START_UP_ACK");
		break;
	case WP_IMA_FE_GSM_CONFIG_ABORTED_M:
		sprintf (imass, "%s", "WP_IMA_FE_GSM_CONFIG_ABORTED");
		break;
	case WP_IMA_FE_GSM_INSUFFICIENT:
		sprintf (imass, "%s", "WP_IMA_FE_GSM_INSUFFICIENT");
		break;
	case WP_IMA_FE_GSM_BLOCKED:
		sprintf (imass, "%s", "WP_IMA_FE_GSM_BLOCKED");
		break;
	case WP_IMA_FE_GSM_OPERATIONAL:
		sprintf (imass, "%s", "WP_IMA_FE_GSM_OPERATIONAL");
		break;
	case WP_IMA_NO_ICP_RECEIVED:
		sprintf (imass, "%s", "WP_IMA_NO_ICP_RECEIVED");
		break;
	default:
		sprintf (imass, "%s", "WP_IMA_FE_GSM_UNKOWN_STATE");
		break;
