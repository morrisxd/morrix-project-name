 ...
 *  Version     : 1
 *  Date        : 23 Dec 2011
 *	Accompanies : PMC-2113031 Issue 1 Atm IMA Ap. Note
 *****************************************************************************/
...
extern void App_TerminateOnErrorX (WP_handle handle, WP_CHAR *s, char *file, int line);
...
#if MORRIS_DISABLE_AGG_WHEN_RECV
printf ("App_DuReceive: disable flowAgg\n");
	WP_IwFlowAggregationDisable (h_bridge1_default_agg);
	WP_IwFlowAggregationDisable (h_flow_agg_tdm[1]);
#endif

	status = WP_HostReceive(h_rx, &data_unit);
	if (WP_OK != status) 
	{
		printf ("WP_HostReceive: error=(%s)\n", WP_error_name[WP_ERROR(status)]);
		return;
	}
...
{
	printf ("App_EventRxIndicate called\n");
	fflush((FILE*)1);
 ...
 * Return val   : None
 *****************************************************************************/
void App_TerminateOnErrorX (WP_handle handle, WP_CHAR *s, char *file, int line)
{
	if (WPI_SimulateInterrupts(), WP_ERROR_P(handle)) {
		printf("Test Abort %s %s 0x%x, file(%s)line(%d)\n", s, WP_error_name[WP_ERROR(handle)], handle, file, line);
		/* Release WDDI */
		WP_DriverRelease();
		exit(1);
	}
	else
	{
		...
		{
		   if (handle != WP_ERR_IMA_UNKNOWN_EVENT) {
			printf("returned from %s is %#8.8x,file(%s)line(%d)\n", s,handle, file, line);
		   }
		}
	}
}
...
void App_Debug(WP_boolean debug)
{
	...
	status = WP_IwFlowModify(h_iw_sys_bridge1, h_bridge1_dfcflow1[0], WP_IW_MODIFY_WRITE_THROUGH, 
