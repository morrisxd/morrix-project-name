...
void App_ufeSystem (void)
{
   ...
   WP_U32 property, i, num_of_lines, temp;
   ...
   // Configure and create 252(e1) UFE lines for T1 SDH mode
...
\*-------------------------------------------------------------*/
      status = WUFE_LineCreate (&ufe4_app_system.line_handle[i],
                                ...
                                WUFE_STRUCT_LINE_SDH, &g_line_cfg);
      ...
      ufe_line_params.transfer_type = g_line_cfg->transfer_type;
      ufe_line_params.stm4 = g_line_cfg->stm4;
      ufe_line_params.stm1 = g_line_cfg->stm1;
      ufe_line_params.stm0 = g_line_cfg->stm0;
      ufe_line_params.tug2 = g_line_cfg->tug2;
      ufe_line_params.tu = g_line_cfg->tu;
      ufe_line_params.clock_rec_line_params = &line_cr_params;
      ufe_line_params.clock_rec_line_params->tx_clk_rec_enable = 0;

      if (g_line_cfg->transfer_type == WUFE_SDH_TYPE_T1)
         conn_type = WT_FLEXMUX_CONNECTION_TYPE_T1_FRAMED;
      else
         conn_type = WT_FLEXMUX_CONNECTION_TYPE_E1_FRAMED;

/*-------------------------------------------------------------*\
      WTI_FlexmuxConnectionCreate() 
\*-------------------------------------------------------------*/
      ...
      if ((g_line_cfg->tu % NUM_TU) == 0)
      {
         g_line_cfg->tu = 0;
         ++g_line_cfg->tug2;

         if ((g_line_cfg->tug2 % 7) == 0)
         {
            g_line_cfg->tug2 = 0;
            ++g_line_cfg->stm0;

            if ((g_line_cfg->stm0 % 3) == 0)
            {
               g_line_cfg->stm0 = 0;
               ++g_line_cfg->stm1;
            }
