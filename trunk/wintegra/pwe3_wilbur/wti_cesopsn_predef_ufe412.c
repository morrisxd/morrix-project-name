...
static void WT_UfePhyHdlcConfig(WUFE_phy *phy_cfg,
                                 WP_U32 n_slots,
                                 WP_U8 *slots_arr)
{
   ...
   /* configure HDLC specific parameters */
   ...
   phy_cfg->type.hdlc.tx_no_flag_between_frames = WUFE_PHY_HDLC_TX_FLAGS_BETWEEN_FRAMES;
   phy_cfg->type.hdlc.tx_min_flag_num = 1;
}

void WT_UfePhyHdlcCreate(WT_ufe *ufe,
                         WP_U32 line_index,
                         WP_U32 phy_index,
                         WP_U32 n_slots,
                         WP_U8 *slots_arr,
                         ...
                         WP_U32 direction)
{
   ...
   WT_UfePhyHdlcConfig(&phy_cfg, n_slots, slots_arr);
   if (ufe->hdlc_crc == WT_CRC_32)
   {
...
void CLI_F_SonetSdhE1UnframedNoCas_DCC(char *StrPrm)
{
   ...
   WT_UfeLinePWCreate(&test_setup, 1);
...
#if WTI_TEST_RAISECOM_DCC

	WP_U32 dcc_lines = WTI_DCC_LINE;
	WP_U32 line_index = 0;
	WP_U32 phy_index = 0;
	WP_U32 dcc_mode = 0;
	WUFE_status ufe_status;
	...
	WUFE_line_dcc dcc_line_config[1] =
	{
	   {
	      /*struct_id*/WUFE_STRUCT_LINE_DCC,
	      /*transfer_type*/WUFE_DCC_TYPE_MS,
	      /*line_port_id*/0
	   }
	};

WP_device_emphy_hdlc_ppp hdlc_dev_config[1] =
{
   {
   ...
   WP_iw_sys_directmap iw_system_config;
   WP_handle ingress_iw_system;

   slots[0] = 0;

   memset(&iw_system_config, 0, sizeof(WP_iw_sys_directmap));
   iw_system_config.max_flows = 8;
   iw_system_config.buffer_gap = 64;

   ingress_iw_system = WP_IwSystemCreate(0,
                                             WP_IW_DIRECTMAP_MODE,
                                             &iw_system_config);
   WTI_TerminateOnError(ingress_iw_system, "WP_IwSystemCreate INGRESS", __LINE__);

   for (i=0; i<dcc_lines; i++)
   {
      line_index= 336 +i;
      phy_index = 1344+i;

      dcc_line_config->line_port_id= i/2;

      if (i%2 == 0)
      {
         dcc_mode = WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12;
         dcc_line_config->transfer_type = WUFE_DCC_TYPE_MS;
      }
      else
      {
         dcc_mode = WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3;
         dcc_line_config->transfer_type = WUFE_DCC_TYPE_RS;
      }

      /*Create DCC Line*/
      ufe_status = WUFE_LineCreate(&(the_system->ufe).line_handle[line_index],
                                   the_system->ufe.ufe_id,
                                   WUFE_STRUCT_LINE_DCC,
                                   dcc_line_config);
      WT_UfeTerminateOnError(ufe_status, "DCC WUFE_LineCreate failed", line_index, __LINE__);

//      WT_UfeLineSocketLpbkSetup(&(the_system->ufe), line_index, WUFE_LINE_LOOPBACK_TYPE_SYSTEM);

      ufe_status = WUFE_LineEnable((the_system->ufe).line_handle[line_index], WUFE_FULL_DUPLEX);
      WT_UfeTerminateOnError(ufe_status, "DCC WUFE_LineCreate failed", line_index, __LINE__);

      WTI_FlexmuxDccEnable(0, dcc_line_config->line_port_id, dcc_mode);

//   printf("Enabling DCC Port 0\n");
//   WTI_FlexmuxDccEnable(0, 0, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//   WTI_FlexmuxDccEnable(0, 0, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);
//    printf("Enabling DCC Port 1\n");
//    WTI_FlexmuxDccEnable(0, 1, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//    WTI_FlexmuxDccEnable(0, 1, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);
//   printf("Enabling DCC Port 2\n");
//   WTI_FlexmuxDccEnable(0, 2, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//   WTI_FlexmuxDccEnable(0, 2, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);
//    printf("Enabling DCC Port 3\n");
//    WTI_FlexmuxDccEnable(0, 3, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//    WTI_FlexmuxDccEnable(0, 3, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);
//   printf("Enabling DCC Port 4\n");
//   WTI_FlexmuxDccEnable(0, 4, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//   WTI_FlexmuxDccEnable(0, 4, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);
//    printf("Enabling DCC Port 5\n");
//    WTI_FlexmuxDccEnable(0, 5, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//    WTI_FlexmuxDccEnable(0, 5, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);
//   printf("Enabling DCC Port 6\n");
//   WTI_FlexmuxDccEnable(0, 6, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//   WTI_FlexmuxDccEnable(0, 6, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);
//    printf("Enabling DCC Port 7\n");
//    WTI_FlexmuxDccEnable(0, 7, WPX_UFE_FRAMER_DCC_MS_LINE_D4_D12);
//    WTI_FlexmuxDccEnable(0, 7, WPX_UFE_FRAMER_DCC_RS_SECTION_D1_D3);


      /* Create HDLC PHY over this line */
      WT_UfePhyHdlcCreate(&(the_system->ufe), line_index, phy_index,
                           n_slots, slots, spe, WUFE_FULL_DUPLEX);
      
      /* Create HDLC Device*/
      the_system->hdlc_dev_handle[i] =
         WP_DeviceCreate(the_system->trans_port,
                         WP_PHY(WUFE_PHY_INDEX_GET(the_system->ufe.phy_handle[phy_index])),
                         WP_DEVICE_HDLC,
                         &hdlc_dev_config[0]);
		WTI_TerminateOnError(the_system->hdlc_dev_handle[i], "WP_DeviceCreate() enet device",__LINE__);
   }
...

{
   ...
   WP_pkt_shaping_wfq hdlc_ch_shaping_wfq[1] =
      {
         {
            /* weight */   1,
            /* fraction */ 0
      ...
      {
         {
            ...
            /* tx_shaping_params */   &hdlc_ch_shaping_wfq[0],
            /* rx_maxsdu */           1792,
            /* tx_cw */               WP_CW_ID_A,
            /* tx_tq */               0
         }
   ...
   for(i =0; i < dcc_lines; i++)
   {
      printf("Channel create phy_index is %d\n",i);

		hdlc_ch_config[0].iwmode = WP_PKTCH_IWM_DISABLE;
		the_system->tx_host_hdlc_ch_handle[i] = WP_ChannelCreate(i, the_system->hdlc_dev_handle[i],
                                the_system->qnode[WTI_QNODE_HOST].handle,
                                WP_CH_TX, WP_HDLC, &hdlc_ch_config[0]);
	  WTI_TerminateOnError(the_system->tx_host_hdlc_ch_handle[i], "WP_ChannelCreate() RX",__LINE__);
	  
		hdlc_ch_config[0].iwmode = WP_PKTCH_IWM_ENABLE;
      handle = WP_ChannelCreate(i, the_system->hdlc_dev_handle[i],
                                the_system->qnode[WTI_QNODE_HOST].handle,
                                WP_CH_RX, WP_HDLC, &hdlc_ch_config[0]);
	  WTI_TerminateOnError(handle, "WP_ChannelCreate() RX",__LINE__);
      the_system->rx_hdlc_ch_handle[i] = handle;

      
      handle = WP_ChannelCreate(i, the_system->hdlc_dev_handle[i],
                                the_system->qnode[WTI_QNODE_IW_PSN2TDM].handle, WP_CH_TX,
                                WP_HDLC, &hdlc_ch_config[0]);
	  WTI_TerminateOnError(handle, "WP_ChannelCreate() TX",__LINE__);
      the_system->tx_hdlc_ch_handle[i] = handle;


	#if (1)
	{
	/*Ingress*/
		WP_iw_agg_pwe3 flow_agg_config[1];
		WP_U32 mpls_label;
		WP_rx_binding_directmap rx_binding_switch_config;
		WP_tx_binding  tx_binding_config;
		WP_pce_rule_classification cfg_ptr[1];
		WP_pce_result_flow_agg flow_agg;
		WP_handle pce_flow_handle;
		WP_flow_mpls mpls_flow_cfg[1];
		WP_iw_agg_routing flow_agg_route_config[1];
		
		
		memset(flow_agg_config, 0, sizeof(WP_iw_agg_pwe3));
		flow_agg_config->tag = 0;
		flow_agg_config->txfunc = the_system->tx_enet_channel_DCC;
		flow_agg_config->rfcs = 0;
		flow_agg_config->intmode = WP_DISABLE;
		flow_agg_config->interruptqueue = WP_IRQT3;
		flow_agg_config->statmode = WP_ENABLE;
		flow_agg_config->timestamp_mode = WP_IW_TIME_STAMP_DISABLE;
		flow_agg_config->ov_pool_mode = WP_IW_OV_POOL_DISABLE;
		flow_agg_config->fbp_drop_threshold = 0;
		flow_agg_config->mtu = 2000;
		flow_agg_config->flow_agg_type = WP_IW_AGG_PRIMARY;
		flow_agg_config->encapsulation_mode = WP_PWE3_HDLC;
		flow_agg_config->direction = WP_PWE3_INGRESS; /* egress updated later */
		flow_agg_config->sn_mode = 0;
		flow_agg_config->control_word_mode = 0;
		flow_agg_config->header_replace_mode = WP_PWE3_REPLACE_NOTHING;
		flow_agg_config->header = 0;
		flow_agg_config->switchover_threshold = 100;
		flow_agg_config->l3_length_update = WP_DISABLE;
		flow_agg_config->l3_header_offset = 0;

		memcpy(flow_agg_config->prefix_header, &mpls_enet_header, WTI_ETHERNET_HEADER_SIZE);

		mpls_label = WTI_MPLS_LABEL + ((i + 1344)<< 12);
   
		memcpy(&flow_agg_config->prefix_header[WTI_ETHERNET_HEADER_SIZE], &mpls_label, 4);

		flow_agg_config->prefix_length = WTI_ETHERNET_HEADER_SIZE + WTI_MPLS_LABEL_SIZE/* + WTI_CONTROL_WORD_LEN*/;

		the_system->rx_dcc_fa_handle[i] = WP_IwFlowAggregationCreate(the_system->wpid,
		                                            WP_IW_PWE3_MODE,
		                                            flow_agg_config);

		WTI_TerminateOnError(the_system->rx_dcc_fa_handle[i], "WP_IwSystemCreate PSN-->TDM", __LINE__);

/*recreate FA*/
		memset(flow_agg_route_config, 0, sizeof(WP_iw_agg_routing));

		memcpy(flow_agg_route_config->prefix_header, &mpls_enet_header, WTI_ETHERNET_HEADER_SIZE);

		mpls_label = WTI_MPLS_LABEL + ((i + 1344)<< 12);
   
		memcpy(&flow_agg_route_config->prefix_header[WTI_ETHERNET_HEADER_SIZE], &mpls_label, 4);

		flow_agg_route_config->prefix_length = WTI_ETHERNET_HEADER_SIZE + WTI_MPLS_LABEL_SIZE/* + WTI_CONTROL_WORD_LEN*/;
		flow_agg_route_config->txfunc = the_system->tx_enet_channel_DCC;
		flow_agg_route_config->mtu = 2000;
		flow_agg_route_config->statmode = WP_ENABLE;
      	flow_agg_route_config->l2_header_insert_mode = WP_IW_L2H_INSERT_ENABLE;

		the_system->rx_dcc_fa_handle[i] = WP_IwFlowAggregationCreate(the_system->wpid,
		                                            WP_IW_ROUTING_MODE,
		                                            flow_agg_route_config);

		WTI_TerminateOnError(the_system->rx_dcc_fa_handle[i], "WP_IwSystemCreate PSN-->TDM", __LINE__);

/*recreate FA*/

		memset(&rx_binding_switch_config, 0, sizeof(WP_rx_binding_directmap));
		rx_binding_switch_config.default_aggregation = the_system->rx_dcc_fa_handle[i];
		rx_binding_switch_config.mru = 2000;

		status = WP_IwRxBindingCreate(the_system->rx_hdlc_ch_handle[i],
                              ingress_iw_system,
                             the_system->qnode[WTI_QNODE_IW_TDM2PSN].handle,
                             &rx_binding_switch_config);
		WTI_TerminateOnError(status, "WP_IwRxBindingCreate INGRESS", __LINE__);

/*Egress*/

		memset(&tx_binding_config, 0, sizeof(WP_tx_binding));

		tx_binding_config.dci_mode = 1;
		tx_binding_config.maxt = 2000;
		status = WP_IwTxBindingCreate(the_system->tx_hdlc_ch_handle[i],
		                             WP_IW_TX_BINDING,
		                             &tx_binding_config);

		WTI_TerminateOnError(status, "WP_IwTxBindingCreate EGRESS TX", __LINE__);

		flow_agg_config->direction = WP_PWE3_EGRESS;
		flow_agg_config->extraction_length = flow_agg_config->prefix_length;
		flow_agg_config->txfunc = the_system->tx_hdlc_ch_handle[i];

		the_system->tx_dcc_fa_handle[i] = WP_IwFlowAggregationCreate(the_system->wpid,
		                                            WP_IW_PWE3_MODE,
		                                            flow_agg_config);
		WTI_TerminateOnError(the_system->tx_dcc_fa_handle[i], "WP_IwSystemCreate PSN-->TDM", __LINE__);


		flow_agg.flow_aggregation = the_system->tx_dcc_fa_handle[i];

		memset(mpls_flow_cfg, 0, sizeof(WP_flow_mpls));

		mpls_flow_cfg->aggregation = the_system->tx_dcc_fa_handle[i];
		mpls_flow_cfg->wred_entry = 0;
		mpls_flow_cfg->mpls_label = (WTI_MPLS_LABEL >> 12) + (i + 11);
		mpls_flow_cfg->deny_mode = 0;

		pce_flow_handle = WP_IwFlowAdd(the_system->psn2tdm_iw_system,
                                                      WP_FLOW_MPLS,
                                                      mpls_flow_cfg);

		WTI_TerminateOnError(pce_flow_handle,
		...
		status = WP_IwSystemBuild(ingress_iw_system);
		WTI_TerminateOnError(status, "WP_IwSystemBuild INGRESS", __LINE__);
   ...
   for (i=0; i<dcc_lines; i++)
   {
      ...
      WT_UfePhyEnable(&(the_system->ufe), phy_index);

      status =  WP_DeviceEnable(the_system->hdlc_dev_handle[i],WP_DIRECTION_DUPLEX);
	  WTI_TerminateOnError(status, "WP_DeviceEnable",__LINE__);
   }
   
   for (i=0; i<dcc_lines; i++)
   {
      status =  WP_ChannelEnable(the_system->tx_hdlc_ch_handle[i]);
	  WTI_TerminateOnError(status, "WP_ChannelEnable",__LINE__);

      status =  WP_ChannelEnable(the_system->tx_host_hdlc_ch_handle[i]);
	  WTI_TerminateOnError(status, "WP_ChannelEnable",__LINE__);
   }
...
{
	vc_3_4 = WPX_UFE_FRAMER_WUFE_SDH_TYPE_VC4;
    WP_U32 phy_mode = WT_FLEXMUX_UFE_412_4_PORTS_OC3;
   ...
   /*
 {
   ...
   */
   ...
   char ch;
   WP_U32 line_port_id = 0, line_active=1;;
