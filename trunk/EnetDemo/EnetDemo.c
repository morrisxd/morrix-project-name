...
WP_handle tx_gbe_hier_channel[NUM_OF_L1_GROUP][NUM_OF_L2_GROUP][NUM_OF_HIER_ENET_FAS_PER_L2];
...
void WPE_Send_HostData (WP_handle tx_channel, WP_data_type data_type, WP_U8 dst_mac[]);
...
/* Interrupt queue configuration used with WP_SysInit */
	WP_int_queue_table int_queue_tables = { {	/*size, rate */
...
// To Do: put NONE everywhere for non crucial masters
	WP_context context = {
		...
		{
		 { /* BUS_PARAM                */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* BUS_PACKET               */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
		 { /* BUS_INTERNAL             */ WP_BUS_INTERNAL,
		  APP_BANK_INTERNAL},
		 { /* BUS_HOST                 */ WP_BUS_HOST,
		  APP_BANK_HOST},
		 { /* INTQUEUE_DEBUG           */ WP_BUS_HOST,
		  APP_BANK_HOST},
		 { /* TIMESTAMP                */ WP_BUS_HOST,
		  APP_BANK_HOST},
		 { /* CAS                      */ WP_BUS_NONE, 0},
		 ...
		 { /* CW                       */ WP_BUS_INTERNAL,
		  APP_BANK_INTERNAL},
		 { /* ASU_TXQUEUE              */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* STATISTICS               */ WP_BUS_HOST,
		  APP_BANK_HOST},
		 { /* ATM_ADDLOOKUP            */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* ATM_CPT_EBMT_EXTHEADER   */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* ETH HDLC CPT             */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* FBP BD                   */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* FBP DATA                 */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
		 { /* L2_BKGND                 */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
		 { /* BRIDGE_LEARNING_DFC_PFMT */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* IMA_MLPPP_STATE          */ WP_BUS_INTERNAL,
		  APP_BANK_INTERNAL},
		 { /* IMA_MLPPP_BUFFER         */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* HASH_IW_LPM_EMC          */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
		 { /* IW_MFC_RULES             */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* FIWT_QAT                 */ WP_BUS_PARAM,
		  APP_BANK_PARAM},
		 { /* MFCPT_DIFFSERV_UPPT_DFCT */ WP_BUS_INTERNAL,
		  APP_BANK_INTERNAL},
		 { /* STATISTICS2              */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
		 { /* STATISTICS3              */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
		 { /* UNASSIGNED6              */ WP_BUS_NONE, 0},
		 { /* PKT_GLOBAL_TCPT          */ WP_BUS_INTERNAL,
		  APP_BANK_INTERNAL},
		 { /* AAL2_QSR                 */ WP_BUS_NONE, 0},
		 { /* UNASSIGNED9              */ WP_BUS_NONE, 0},
		 { /* UNASSIGNED10             */ WP_BUS_NONE, 0},
		 /* ********************* VIRTUAL BUS ASSIGNMENT ******************** */
		 { /* MCH_DATA                 */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
		 { /* OAM_FM_QUERY             */ WP_BUS_NONE, 0},
		 { /* HASH_ON_HEAP             */ WP_BUS_NONE, 0},
		 { /* MFC_INTERNAL             */ WP_BUS_PACKET,
		  APP_BANK_PACKET},
...
void WPE_CreateIWQnode ()
{
	...
	WP_qnode_iwq qn_iw_config = {
		...
		/* num_buffers */
		MAXT_SIZE * (NUM_OF_HIER_ENET_TX_CHANNELS + 1 + 1) + (NUM_OF_HIER_ENET_RX_CHANNELS + 1 + 1),
		/* adjunct_pool */ 0
	};
...
void WPE_CreateHostQnode ()
{
	...
	WP_pool_buffer_data buffers_host_config = { /* n_buffers */ 10 * NUM_OF_ALL_L2HOST_CHANNELS,	// n_rings * ring_length
		/* offset */ 64,
		/* size */ 2048,
		/* pad */ 0,
		/* bus */ WP_BUS_HOST,
...
void WPE_CreateHostIwRxChannel ()
{
	...
	WP_ch_iw_rx iw_host_ch_config = {
		...
		/* pqlevel */ 0,
		/* tx_binding_type */ WP_IW_TX_BINDING,	// from point of view of IW, here is TX channel.
...
void WPE_CreateHostTermFlowAgg ()
{
	...
	WP_iw_agg_bridging agg_host_config = {
		...
		/*mtu; */ MTU_SIZE,
		/*prefix_length */ 0,
		/*prefix_header[32] */ {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					0x00, 0x00,
...
void WPE_CreateFastEnetPortDevice ()
{
	...
	enet_port_config.pkt_limits.max_tx_channels = NUM_OF_FAST_ENET_TX_CHANNELS;
	enet_port_config.pkt_limits.max_rx_channels = NUM_OF_FAST_ENET_RX_CHANNELS;
...
void WPE_CreateFastEnetRxTxBinding ()
{
	...
	for (i = 0; i < NUM_OF_FAST_ENET_TX_CHANNELS; i++)
	{
		status = WP_IwTxBindingCreate (tx_gbe_channel[i], WP_IW_TX_BINDING, &tx_binding_enet_config);
...
void WPE_CreateFastEnetRxTxChannel ()
{
	...
	for (i = 0; i < NUM_OF_FAST_ENET_TX_CHANNELS/* 1 */; i++)
	{
		...
		tx_gbe_channel[i] = WP_ChannelCreate (0x11, dev_enet, iw_qnode, WP_CH_TX, WP_ENET, &enet_ch_config);
		terminate_on_error (tx_gbe_channel[i], "WP_Channel_Create() Fast TX");
	}
...
void WPE_CLI (void)
{
	...
	while ((InputBuf[0] != 'q') && (InputBuf[0] != 'k'))
	{
		...
		case '2':
			...
			break;
		case '3':
			printf ("************************* HierarchicalEnet FlowAgg[0] Stats ********************** \n");
