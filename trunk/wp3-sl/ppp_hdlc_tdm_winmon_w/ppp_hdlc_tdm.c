 ...
 *  Description:
 *
 * This application contains 4 different TDM Datapath configurations:
 * 1.HDLC_OVER_IP
 * 2.HDLC_OVER_MPLS
 * 3.PPP_OVER_IP
 * 4.PPP_OVER_MPLS
...
#include "ppp_hdlc_tdm.h"
#include "ppp_hdlc_tdm_util.c"
#include "ppp_hdlc_tdm_stats.c"
...
void App_InitConfig(void)
{
	...
	tag_bport_enet1 = 700;
	tag_bport_enet2 = 701;
	for(ii = 0;ii < APP_MAX_PORT_TDM ;ii++)
	{
	...
	for(ii = 0;ii < APP_MAX_FLOW_AGG_TDM ;ii++)
	{
...
* Description  : create the ENET and TDM ports and devices
*
* Input  params: none
* Output params: 
* Return val   : none
...
void App_DeviceAndPortCreate()
{
   ...
   /* Create TDM ports and devices */
	for(ii = 0;ii < APP_MAX_DEV_TDM	;ii++)
	{
...
* Description  : creates host, ENET, and TDM(hdlc) channels. PQblock and associated ENET 2 channels are created in this function
*
*
* Input  params: none
* Output params: 
...
void App_ChannelAndPQBlockCreate(void)
{
   ...
   h_enet1_rx = WP_ChannelCreate(tag_enet1_rx, h_device_enet1,
	...
	for(block_index=0;block_index<PQBLOCK_SIZE; block_index++)
	{
	   ...
	   pqblock_handle[block_index] = WP_IwPQBlockCreate(h_port_tdm[block_index],WP_HDLC);
	   App_TerminateOnError(pqblock_handle[block_index], "WP_PQBlockCreate ");

	   /* Each PQ block will have n number of channel here n is max up to APP_MAX_TDM_PORTS*/
	   ch_hdlc_cfg.tx_pqblock = pqblock_handle[block_index];
	   ch_hdlc_cfg.iwmode = WP_PKTCH_IWM_ENABLE;

	   for(ch_index= 0;ch_index < APP_MAX_TDM_PORTS;ch_index++)
	   {
	      ...
	      h_tdm_tx[block_index][ch_index] = WP_ChannelCreate(0, h_dev_tdm[block_index], h_qnode_iwq_tdm,
	                                                         WP_CH_TX, WP_HDLC, &ch_hdlc_cfg);
	      App_TerminateOnError(h_tdm_tx[block_index][ch_index], "WP_ChannelCreate()  Tx");

	      /* tx binding for tdm channel */
	      status =	  WP_IwTxBindingCreate(h_tdm_tx[block_index][ch_index], WP_IW_TX_BINDING, &tx_binding_cfg);
...
* Description  :   Enet, TDM ports  and channels are enabled in this function
*
*
* Input  params: none
* Output params: 
...
void App_EnableSystem(void)
{
	...
	for(block_index=0;block_index<PQBLOCK_SIZE; block_index++)
	{
	   ...
	   {
		   status = WP_ChannelEnable(h_tdm_tx[block_index][ii]);
...
*	 forwards the packet to one of the TDM ports.
*   In this example TDM[ii] is taken. TDM[ ii] was picked based on FOR LOOP index, it could have been any TDM.
*	 For this flow priority mapping, priority remarking and policing is done. Priority remarking is based on DSCP.
*
*	 TDM is used based on for loop index. It could have been any TDM.
*
*	 Note that the flow aggregation that forwards the packet on TDM ( through PQ Block) strips off the Ethernet
*	 header and prefixes a 4 byte HDLC header to the packet before sending it out on the TDM (hdlc_prefix_header)
...
void App_CreateEnetHdlcBridgeForIP()
{
   ...
   for(ii = 0;ii < PQBLOCK_SIZE;ii++)
   {
      ...
      iw_agg_bridging_cfg.txfunc = pqblock_handle[ii]; /*PQ Block forward the packet to associated TDM channel*/
      ...
      h_flow_agg_tdm[ii] = WP_IwFlowAggregationCreate( APP_MASTER_WPID,
                                                       WP_IW_VLAN_AWARE_BRIDGING_MODE,
                                                       &iw_agg_bridging_cfg);
      App_TerminateOnError(h_flow_agg_tdm[ii]," WP_IwFlowAggregationCreate() TDM ");
      memcpy(&iw_agg_tdm_param[ii], &iw_agg_bridging_cfg, sizeof(WP_iw_agg_bridging));
   }
   ...
   App_TerminateOnError(policer_handle," WP_IwPolicerCreate() bridge1 ");
	...
	/*Creating (PQBLOCK_SIZE*APP_MAX_TDM_PORTS) DFC flow rule. Rule DST MAC and VLAN is same but DSCP value is, FOR loop index   */
   ...
   for(ii = 0; ii<PQBLOCK_SIZE; ii++)
   {
      ...
      for(jj = 0; jj<APP_MAX_TDM_PORTS; jj++)
      {
         ...
         priority_action.field_type = WP_FLOW_PRIORITY_DSCP;
			...
			forwarding_action.flow_aggregation = h_flow_agg_tdm[ii]; /* Note here TDM FA is chosen according to FOR LOOP index*/
...
*
*    The PPP SWITCHING default flow aggregation (1 per TDM port) adds a dummy 18 byte ethernet header( including VLAN) and
...
void App_CreateHdlcEnetBridgeForIP(void)
{
   ...
   memset(&flow_class_rule, 0, sizeof(WP_flow_class_rule));
   memset(&dfc_flow_info, 0, sizeof(WP_dfc_flow_info));
   /* NOTE for this test we are using TDM#0 to send out and receive the packet, so we use the bridge port for TDM 0 */
   dfc_flow_info.input_port = h_bridge2_bport_tdm[0];  /* Note that Input_port is one of the fields in the filter */
   dfc_flow_info.output_port = 0;
   ...
   App_TerminateOnError(h_bridge2_dfcflow_control," WP_IwFlowAdd() DFC Bridge2");
	...
	/* Add DFC flow */
	/*Creating (PQBLOCK_SIZE*APP_MAX_TDM_PORTS) DFC flow rule.  */
	...
	for(ii = 0; ii<PQBLOCK_SIZE; ii++)
	{
	   for(jj = 0; jj<APP_MAX_TDM_PORTS; jj++)
	   {
	      /* NOTE for this test we are using TDM#ii to send out and receive the packet, */
	      dfc_flow_info.input_port = h_bridge2_bport_tdm[ii];   /* Note that Input_port is one of the fields in the filter , port need to choosen carefully*/
	      dfc_flow_info.output_port = 0;
	      /*NOTE: Please set dummy_prefix_header according else DFC will fail*/
	      ...
	      App_TerminateOnError(h_bridge2_dfcflow[ii][jj]," WP_IwFlowAdd() DFC Bridge1 rule1 ");
	   }
	}
...
*	 to one of the TDM ports (TDM #ii in this example).
*	 TDM[ ii] was picked based on FOR LOOP index, it could have been any TDM. 
*	 For this flow priority mapping, priority remarking and policing is done. Priority remarking is based on DSCP.
*
*	 Note that the flow aggregation that forwards the packet on TDM[ii], strips off the Ethernet header and MPLS header, 
*	 then prefixes a 4 byte HDLC header to the packet before sending it out on the TDM (hdlc_prefix_header)
...
void App_CreateEnetHdlcBridgeForMPLS()
{
   ...
   /* EACH TDM port is represented as bridge port in Bridge1 too so a packet going from ENET to TDM has the ENET bridge port
    * as input bridge port and TDM bridge port as the output bridge port */
   for(ii = 0;ii < APP_MAX_PORT_TDM ;ii++)
   {
      bridge_port_cfg.tag = tag_bridge1_bport_tdm[ii];
      bridge_port_cfg.flow_agg = h_bridge1_default_agg;
   ...
   /*Creating (PQBLOCK_SIZE*APP_MAX_TDM_PORTS) DFC flow rule. Rule for MPLS label is added */
   /*All rule can be configured separately and accordingly Flow agg. can be choosen*/
   for(ii = 0; ii<PQBLOCK_SIZE; ii++)
   {
      ...
      for(jj = 0; jj<APP_MAX_TDM_PORTS; jj++)
      {
         ...
         action.num_of_actions = 3;
         action.action_type = action_type;
         forwarding_action.flow_aggregation = h_flow_agg_tdm[ii]; /* Note here TDM FA is choosen acoording to FOR LOOP index*/
         /*It can be configured to different FA (TDM) as per requirement */
...
*    So 0x0F000800 is received back into the PPP Switching system as 0x0F000800 since no PPP header is present (0xFF03)
*
*    The PPP SWITCHING default flow aggregation (1 per TDM port) adds a dummy 18 byte ethernet header( including VLAN) and forwards the packet to Bridge2
*	  Dummy ethernet header is configured VLAN id as follows
*			0x0a for traffic
*			0x0b for control 
*	  NOTE: VLAN id should add properly based on which bridge2 filter will forward the packet.
...
*	 packet to one of the TDM ports .
*	 TDM is used based on FOR LOOP index. it could have been any TDM. 
*	 For this flow priority mapping, priority remarking and policing is done. Priority remarking is based on DSCP.
*
*	 Note that the flow aggregation that forwards the packet on TDM ( through PQ Block)
*		strips off the Ethernet header and prefixes a 4 byte PPP header to the packet before sending it out on the TDM (ppp_prefix_header)
...
void App_CreateEnetPPPBridgeForIP()
{
   ...
   /* EACH TDM port is represented as bridge port in Bridge1 too so a packet going from ENET to TDM has the ENET bridge
    * port as input bridge port and TDM bridge port as the output bridge port */
   for(ii = 0;ii < APP_MAX_PORT_TDM ;ii++)
   {
      bridge_port_cfg.tag = tag_bridge1_bport_tdm[ii];
      bridge_port_cfg.flow_agg = h_bridge1_default_agg;
...
*    (1) If VLAN id 0x0b. This packet is sent to a flow agg named h_flow_agg_control and host terminated
*    (2) If VLAN id 0x0c. This packet is sent to a flow agg named h_flow_agg_lcp and host terminated
*    (3) If VLAN id 0x0a This packet is sent to a flow agg named h_flow_agg_enet2 and sent out of ENET2 
*		 For this flow priority mapping priority remarking and policing is done. Priority remarking is based on VLAN.
*
*	flow agg h_flow_agg_control will removed dummy ethernet mac header and send rest of the packet
...
void App_CreatePPPEnetBridgeForIP()
{
   ...
   for(ii = 0; ii<PQBLOCK_SIZE; ii++)
   {
      for(jj = 0; jj<APP_MAX_TDM_PORTS; jj++)
      {
         ...
         dfc_flow_info.input_port = h_bridge2_bport_tdm[ii];/* Note that Input_port is one of the fields in the filter , port need to chosen carefully*/
         dfc_flow_info.output_port = 0;
         /*NOTE: Please set dummy_prefix_header according else DFC will fail*/
         /* currently using same rule for all input... we can have different rule for each input*/
         sprintf(rule_string,"n;n;n;n;n;n;n;n;n;n;0xa;n;n;n;n;n"); /*rule for traffic packet */
...
*
 Bridge1: 
*	 Host sends a packet on ENET1 (port is in internal loopback mode)
*	 The packet is received into Bridge1. A DFC classifier rule based on MPLS label is applied that forwards the PACKET to one of the TDM ports .
*	 TDM is used based on FOR LOOP index. it could have been any TDM. 
*	 For this flow priority mapping, priority remarking and policing is done. Priority remarking is based on DSCP.
*
*	 Note that the flow aggregation that forwards the packet on TDM ( through PQ Block)
...
void App_CreateEnetPPPBridgeForMPLS()
{
   ...
   policer_handle = WP_PolicerCreate(WP_WINPATH(0)/*wpid*/,WP_POLICER_V1,policer_config);
   App_TerminateOnError(policer_handle," WP_IwPolicerCreate() bridge1 ");
   /*Creating (PQBLOCK_SIZE*APP_MAX_TDM_PORTS) DFC flow rule. Rule is based on MPLS label   */
   /*All rule can be configured separately and accordingly Flow agg. can be chosen*/
   for(ii = 0; ii<PQBLOCK_SIZE; ii++)
   {
...
void App_CreatePPPEnetBridgeForMPLS()
{
   ...
   for(ii = 0; ii<PQBLOCK_SIZE; ii++)
   {
      for(jj = 0; jj<APP_MAX_TDM_PORTS; jj++)
      {
         ...
         dfc_flow_info.input_port = h_bridge2_bport_tdm[ii];
         dfc_flow_info.output_port = 0;
         /*NOTE: Please set dummy_prefix_header according else DFC will fail*/
         /* currently using same rule for all input... we can have different rule for each input*/
         sprintf(rule_string,"n;n;n;n;n;n;n;n;n;n;0xa;n;n;n;n;n"); /*rule for traffic packet */
...
WP_status App_ParameterSwitch(WP_CHAR switchto)
{
   ...
   {
      memcpy(iw_agg_tdm_param[jj].prefix_header, hdlc_prefix_header, hdlc_prefix_header_len);
      result = WP_IwFlowAggregationModify(h_flow_agg_tdm[jj], WP_IW_FLOW_AGG_MOD_PRX, &iw_agg_tdm_param[jj]);
