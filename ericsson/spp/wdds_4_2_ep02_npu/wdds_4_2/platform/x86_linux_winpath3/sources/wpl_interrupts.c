...
WP_U32 WPL_IntDisable(WP_U32 wpid, WPL_IntVector vector)
{
   ...
   case WPL_Eint3Ufe4:
   {
      ...
      // XALYO
      {
          ...
          WPL_INTRPT_REGISTER_GET(intrpt_mask, WPL_INTRPT_MASK_EXTERNAL_L);
          intrpt_mask &= ~(1 << WPL_EINT3_INTRPT);
          WPL_INTRPT_REGISTER_SET(WPL_INTRPT_MASK_EXTERNAL_L, intrpt_mask);
      }
...
WP_U32 WPL_IntEnable(WP_U32 wpid, WPL_IntVector vector)
{
   ...
   case WPL_SgmiiAn:
   {
      ...
      // XALYO - Enable SGMII AN interrupt
      WPL_INTRPT_REGISTER_GET(intrpt_mask, WPL_INTRPT_MASK_EXTERNAL_H);
      intrpt_mask |= 1<<(WPL_SIGNAL_RT_SGMII_AN - WPL_INTERRUPT_HIGH_REG_OFFSET);
      WPL_INTRPT_REGISTER_SET(WPL_INTRPT_MASK_EXTERNAL_H, intrpt_mask);
   ...
   {
      sigaction(WPL_SIGNAL_RT_OV, NULL, &siginfo);
      // XALYO - must be Wintegra copy/paste error. If WPL_overrun_callback_handler is not called
      //         the overrun condition is never cleared and we are flooded by interrupts

      //siginfo.sa_sigaction = (void (*)(int, siginfo_t *, void *))WPL_service_callback_handler;
      siginfo.sa_sigaction = (void (*)(int, siginfo_t *, void *))WPL_overrun_callback_handler;
...
void WPL_service_callback_handler(WP_S32 signum, siginfo_t *info, void *ptr)
{
  //WP_U32 intrpt_mask;
   ...
   if (wddi_callback_ptrs[WPL_IntServiceQueues] != NULL)
   {
      ...
      #if 0
      // XALYO - Enable overrun interrupts
      WPL_INTRPT_REGISTER_GET(intrpt_mask, WPL_INTRPT_MASK_EXTERNAL_H);
      if((intrpt_mask & 0x0003c000) == 0) {
                intrpt_mask |= 0x0003c000;
          WPL_INTRPT_REGISTER_SET(WPL_INTRPT_MASK_EXTERNAL_H, intrpt_mask);
...
void WPL_overrun_callback_handler(WP_S32 signum, siginfo_t *info, void *ptr)
{
  //   WP_U32 intrpt_mask;
   ...
   if (wddi_callback_ptrs[WPL_IntOverrunQueues] != NULL)
   {
      ...
      #endif
      //printf("WPL_overrun_callback_handler intrpt_mask = 0x%x",intrpt_mask);
   }
}
