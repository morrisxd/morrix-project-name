...
static void WPLI_WinipcUserIntrHandler(WP_S32 signum, siginfo_t* info, void *ptr);

static void WPLI_WinipcReqIntrHandler(WP_S32 signum, siginfo_t* info, void *ptr);
...
WP_U32 WPL_WinipcHandlerRegister(WPL_sig_handler_type pi_cb, WP_S32 i_signal)
{
   ...
   switch (i_signal)
   {
      case WPL_NTR_SIGNAL_REQ:
         ...
         siginfo.sa_sigaction = WPLI_WinipcReqIntrHandler;
         break;
      case WPL_NTR_SIGNAL_USR:
         signal_handler[1] = pi_cb;
         siginfo.sa_sigaction = WPLI_WinipcUserIntrHandler;
         break;
      default:
...
static WP_U32 WPLI_WinipcDeviceOpen(void)
{
   if ((wpl_ntr_device = open("/dev/wp_winipc", O_RDWR)) < 0)
   {
