...
static long test_once(LINX * linx, LINX_SPID server, int use_linx_api)
{
	...
	if (use_linx_api) {
		...
		len = sendto(sd, sig, sizeof(LINX_SIGSELECT), 0,
			     (const struct sockaddr *)&to, socklen);
...
attach_test(LINX * linx, const char *path,
	    int cnt, LINX_SPID server, int use_linx_api)
{
	...
	for (i = 0; i < cnt; i++) {
		int t = test_once(linx, server, use_linx_api);
		if (t > max)
			max = t;
		if (t < min)
			min = t;
