...
int npu_handle_linx_msg(void)
{
	...
	while(rsvnum--) 
	{
		...
		if (linx_receive_w_tmo(linx_info->linx, &sig, 0,
				       sigsel_any) == -1) 
		{
			WDDI_WARNING("linx_receive() %s", strerror(errno));
	    ...
	    {	
			...
			{
				WDDI_WARNING("linx_sender() %s", strerror(errno));
			}
	        ...
	        switch (sig->sig_no) 
            {
			    case NPU_ECHO_SIG:
				{
					...
					{
							WDDI_WARNING("linx_attach() failed %s",strerror(errno));
					}						
                     #endif
				...
				case VPORT_PORT_OPEN_SIG:
				{
					...
					if(vp_type == VP_TYPE_FR)
					{
						...
						WDDI_INFO("server: received open vport sig vp_type = %s  port_src = %d : S%dP%dFR%dD%d\n","VP_TYPE_FR",port_src,card,trunk,group,dlci);						
					}
					else
					{
						WDDI_INFO("server: received open vport sig vp_type = %s ipaddr_src = 0x%x port_src = %d \n","VP_TYPE_UDP",ipaddr_src,htons(port_src));
					...
					{
					     WDDI_WARNING("np_creat_fr_vport invalid vp_type %d \n",vp_type);
						 break;
					}
				...
				case VPORT_PORT_CLOSE_SIG:
				{
					...
					if(status != STATUS_OK)
					{
                				WDDI_WARNING("npu_clear_vport_info() status error");
						//break;
					}		
				...
				case SPP_SEND_SIG:
				{
					...
					if(vp_type == VP_TYPE_FR)
					{
						...
						u32 dlci;
						np_fraddr_unpack(port_src,&card,&trunk,&group,&dlci);
						WDDI_PRINT("server: received  send sig vp_type = %s length = 0x%x port_src = %d : S%dP%dFR%dD%d\n","VP_TYPE_FR",length,(port_src),card,trunk,group,dlci);
						
					}
					else
					{
						WDDI_PRINT("server: received  send sig vp_type = %s length = 0x%x port_src = %d port_dst = %d ipaddr_src = 0x%x ipaddr_dst = 0x%x\n","VP_TYPE_UDP",length,htons(port_src),htons(port_dst),ipaddr_src,ipaddr_dst);
					...
					if(np_waittime == NPU_WAIT_FINISHED)
					{
						if(vp_type == VP_TYPE_UDP)
						{
							...
							{	
								...
								{
									WDDI_WARNING("server: send UDP SPP_SEND_SIG vport unused !vport = %d \n",(u16)htons(port_src));
						...
						else if(vp_type == VP_TYPE_FR)
						{
							...
							if(VportInfo != NULL)
							{
								 ...
								 else
								 {
									 ...
									 u32 dlci;
									 np_fraddr_unpack(port_src,&card,&trunk,&group,&dlci);							 
									 WDDI_WARNING("server: send FR SPP_SEND_SIG vport unused !vport = %d : S%dP%dFR%dD%d\n",(u16)(port_src),card,trunk,group,dlci); 					 
								 }
					...
					if(NPU_SET_REQUEST_TYPE == sig->npu_request.cmdType)
					{
                        switch(sig->npu_request.cmd)
                        {
                            case NPU_SET_RECONFIG:
							{
                                ...
                                {
                                    WDDI_WARNING("reconfig mode is FR!\n");
                                ...
                                {
                                    WDDI_WARNING("reconfig mode is IP!\n");						
                                }
                               u32 i;
							   u32 routeFlag = 0;
							   for(i = 0;i< SPP_MAX_ROUTE_NUM;i++)
							   {
								...
								else
								{
                                    ...
                                    ret = 0;
									WDDI_WARNING("Reconfig parameters are the same!!", strerror(errno));
								}
                            ...
                            case NPU_DELETE_UPLINK_PATH_RULE:
							{
								...
								{
								    WDDI_WARNING("NPU_DELETE_UPLINK_PATH_RULE  DeleteMode ERROR %d",ul_rule.DeleteMode);
			    ...
			    case ATTACH_CLIENT_SIG:
				{
					...
					{
						WDDI_WARNING("Server: Contact with client lost. client =0x%x\n",client);					
				...
				{
					WDDI_WARNING("Server: Contact with NPUMON lost. client =0x%x\n",client);
					linx_free_buf(linx_info->linx,&sig);
...
int npu_clear_vport_info(u32 port_src,u8 vp_type)
{
	...
	int status;
    sprintf(item.key,"%d",port_src);
	WDDI_INFO("clear item.key = %s \n",item.key);
	//if((found_item = hsearch(item,FIND)) == NULL)
	if(0 == hsearch_r(item,FIND,&found_item,ht))//fail, first time to add 
	{
		WDDI_WARNING("When clear vport,hash not found port_src: %d\n",port_src);
		return -1;
	}
	p_VportInfo = (STRU_VPORT_INFO *)found_item->data;
	if(p_VportInfo->UsedFlag == UNUSED)
	{
		WDDI_WARNING("When clear vport,vport unused: %d\n",port_src);
		return -1;
	}
	...
	if(vp_type == VP_TYPE_UDP)
	{
		...
		for(Dport = 0;Dport<65535;Dport++)
		{
			...
			{
				if(htons(port_src) >= htons(Dport) && htons(port_src) <= (htons(Dport) + initial_cfg->PortPoolSize-1))
				{
	...
	{
		WDDI_WARNING("npu_clear_vport_info failed vport have been set to unused !port = %d \n",port_src);
		//return -1;
	}
	if(vp_type == VP_TYPE_FR)
	{
...
STRU_VPORT_INFO * npu_get_vport_info(u32 port_src)
{
	...
	{
	     #if ENEA_RELEASE
		  WDDI_INFO("When get vport,hash not found port_src: %ld\n",port_src);
		 #else
		  WDDI_WARNING("When get vport,hash not found port_src: %ld\n",port_src);		 
		 #endif
	    return NULL;
	}
	else
	{   STRU_VPORT_INFO *p_VportInfo;
		...
		else
		{
		    if(((STRU_VPORT_INFO *)found_item->data)->vp_type == VP_TYPE_FR)
		    {
				...
				WDDI_WARNING("npu_get_vport_info failed vport unused ! port = 0x%x : S%dP%dFR%dD%d\n",port_src,
											  card,trunk,group,dlci);	  
		    }
			else
			{
			    #if ENEA_RELEASE
				WDDI_INFO("npu_get_vport_info failed vport unused ! port = %d\n",port_src);	  
				#else
...
int npu_send_udp_packet(u8* p, u32 len,u32 ipaddr_src,u32 ipaddr_dst,u32 port_src, u32 port_dst)
{
	 ...
	 u8 *pu8UdpData = NULL;
	 char ArraySendBuf[NPU_UDP_ETHPACK_LEN_MAX];
	 u32 u32i;
     u32 tmp_flag = 0; 
    ...
    if (tmp_flag)
    {
	 ...
	 if(ret < 0)Fprintf("npu_dispatch_packet_to_client ret = %d",ret);
                 ...
                 else
                 {
                    ...
                    WDDI_INFO("tap send packet ipaddr_dst = 0x%x ipaddr_src =0x%x\n",ipaddr_dst,ipaddr_src);
