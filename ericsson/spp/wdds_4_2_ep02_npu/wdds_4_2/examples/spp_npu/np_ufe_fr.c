...
WP_port_upi_packet_ext upi_packet_config[1] = 
{
   
   {
                                                                     ...
                                                                     /*WP_U32 packet_characteristics; */ WP_UPI_POS_CRC_ENABLE,
                                                                     //WP_UPI_POS_CRC_DISABLE
         /*WP_U32 slow_phy_tx_bitrate;   */ 50000000, 
         /*WP_port_pkt_limits *pkt_limits; */ pkt_limits 
} };
static WP_port_upi_iw_ext emphy_iw_config[1] = { {1, 0 } 
};
   ...
   WP_port_upi port_config_ufe[1] = 
{
   
   {
         ...
         /* tx_characteristics */ 
         WP_UPI_MASTER | WP_UPI_SINGLE | WP_UPI_PRT_EVEN | /*WHY ? */ 
         WP_UPI_FIXED | WP_UPI_PRTCHK_ON | WP_UPI_EMPHY2_13 |
         WP_UPI_BUS_WIDTH_16, 
         /* rx_characteristics */ 
         WP_UPI_SINGLE | WP_UPI_MASTER | WP_UPI_PRT_EVEN | WP_UPI_ROBIN
         | WP_UPI_PRTCHK_OFF | WP_UPI_BUS_WIDTH_16, 
         /* atm_ext */ 0, 
...
{
   {
         ...
         /*rx_queue; */ 0, 
         /*tx_tq_types[4]; Array of WP_packet_scheduling_type */
      {
      0}, 
...
WUFE_handle ufe_line_handle[N_UFE_MAX_LINES_T1] =
{
0};

WUFE_handle ufe_phy_handle[N_UFE_MAX_LINES_T1][32] =
{
   {
0}};                            /*max slot in one line is 32 */

WUFE_handle ufe_device_handle[N_UFE_MAX_LINES_T1][32] =
{
   {
0}};                            /*max slot in one line is 32 */

u32 E1MapTable[N_UFE_MAX_LINES_T1] =
{
0};
...
#define RECONFIG_TRUNK_MAX (SPP_MAX_TRUNK_NUM*2)
   WP_U8 reconfigStm1_createE1_flag[RECONFIG_TRUNK_MAX] =
{
0};
...
void WTUFE_config_phys (WP_U32 line_index, WP_U32 groupId,
                         WP_U32 slot_selection,
                         WP_U32 line_index_base
                         /*,WP_handle *phy_handle,WP_handle *device_handle */
                         );
WP_CHAR input[20];

#define WT_UFE_HANDLE_INVALID 0xffffffff;
   STRU_STM1_ALARM_INFO np_ufe4_ararm_info =
{
0};
static void WT_UfeTerminateOnError (WP_handle handle, WP_CHAR * s,
                                      WP_U32 id, WP_U32 LineNum) 
{
   if (handle != WUFE_OK)
...
void np_initial_fr_qnode (WP_U32 ufe_port) 
{
   ...
   fr_buffer_pool =
      WP_PoolCreate (WP_WINPATH (0), WP_pool_frbuffer,
                     &buffer_pool_config);
   terminate_on_error (fr_buffer_pool,
   ...
   qnode_fr_host =
      WP_QNodeCreate (WP_WINPATH (0), WP_QNODE_FR_HOSTQ, &qn_fr_host);
   ...
   iw_fr_buffer_pool =
      WP_PoolCreate (WP_WINPATH (0), WP_pool_iwbuffer,
                     &buffer_pool_config);
   terminate_on_error (iw_fr_buffer_pool,
   ...
   qnode_fr_iw =
      WP_QNodeCreate (WP_WINPATH (0), WP_QNODE_IWQ, &qnode_iw_config);
   terminate_on_error (qnode_fr_iw,
   ...
   fr_iw_host_device =
      WP_DeviceCreate (fr_iw_host_port, 0, WP_DEVICE_FR,
                       &fr_device_config[0]);
   
#endif   /*  */
#if 1
      fr_iw_host_device =
      WP_DeviceCreate (fr_iw_host_port, 0, WP_DEVICE_IW_HOST, NULL);
      ...
      
   {
         ...
         upi_ufe_port =
         WP_PortCreate (WP_WINPATH (0), WP_PORT_UPI1, &port_config_ufe[0]);
      ...
      
   {
      ...
      terminate_on_error (status, "WP_SysClockCreate() UPI2");
         ...
         upi_ufe_port =
         WP_PortCreate (WP_WINPATH (0), WP_PORT_UPI2, &port_config_ufe[0]);
...
void fr_system_init (WP_U32 ufe_port) 
{
   ...
   ufe_events.fm_events.event_reg =
      WUFE_EVENT_FM & ((~WUFE_EVENT_FM_TX_DDR_UNDERRUN) &
...
WP_U32 np_fraddr_pack (WP_U32 card, WP_U32 trunk, WP_U32 tsgroup,
                         WP_U32 dlci) 
{
   fr_addr_t a =
   {
...
void np_fraddr_unpack (WP_U32 frport, WP_U32 * card, WP_U32 * trunk,
                        WP_U32 * tsgroup, WP_U32 * dlci) 
{
   fr_addr_t * a;
   assert (sizeof (fr_addr_t) == 4);
   a = (fr_addr_t *) & frport;
...
int np_creat_fr_vport (WP_U32 vfport) 
{
      ...
      
   {
      ...
      {
         trunk =
            VportInfo->ch_info.trunk - 1 + stm1Id * N_UFE_MAX_LINES_E1 / 2;
      }
      ...
      {
         trunk =
            VportInfo->ch_info.trunk - 1 + stm1Id * N_UFE_MAX_LINES_T1 / 2;
      }
         ...
         
      {
         ...
         VportInfo->ch_info.device_handle =
            ufe_device_handle[trunk][group];
      }
      ...
      {
         WDDI_WARNING
            ("Error: np_creat_fr_vport::ufe_device_handle[%d][%d] is not create yet! \n",
             trunk, group);
         return -1;
      }
      WDDI_INFO
         ("Info: np_creat_fr_vport, stm1Id:%d, trunk:%d, group:%d\n",
          stm1Id, trunk, group);
      
         /*if fr host channel not created,then creat it */ 
         if ((VportInfo->ch_info.rx_host_handle == 0)
             && (VportInfo->ch_info.tx_host_handle == 0))
         ...
         
      {
            ...
            
         {
            ...
            WDDI_WARNING
               ("Warn: np_creat_fr_vport::np_enable_fr_host_channel, vport:%d\n",
                vfport);
         }
      }
   ...
   {
      WDDI_WARNING
         ("Warn: np_creat_fr_vport::npu_get_initial_cfg_info fail!\n");
      return -1;
   }
   return 0;
}
int np_close_fr_vport (WP_U32 vfport) 
{
      ...
      
   {
      ...
      {
         WDDI_WARNING
            ("Warn: np_close_fr_vport, ufe_device_handle hasn't created ,vport = %d \n",
             vfport);
         return -1;
      }
      ...
      {                         /*maybe we should disable/delete iw_channel first,because it will use
         ...
         if ((VportInfo->ch_info.rx_iw_handle != 0)
              && (VportInfo->ch_info.tx_iw_handle != 0))
         ...
         
      {
         ...
         initial_cfg->stru_stm1_cfg_info[stm1Id].trunk_info[trunk].
            group_info[group].groupValid = NPU_INVALID;
      }
   ...
   {
      WDDI_WARNING
         ("Warn: np_close_fr_vport::npu_get_vport_info fail! vfport:0x%x\n",
          vfport);
      return -1;
   }
   return 0;
...
void npu_send_fr_packet (WP_U8 * p, WP_U32 len, WP_U32 vfport) 
{
      ...
      
   {
      ...
      np_fr_host_send (ch_info->tx_host_handle, len, (WP_CHAR *) p,
                        ch_info->dlci, vfport);
   }
   ...
   {
      WDDI_WARNING
         ("Warn: npu_send_fr_packet::npu_get_vport_info fail! vport = %d \n ",
          vfport);
   }
}
void np_fr_host_send (WP_handle tx_host_handle, WP_S32 DataLength,
                        WP_CHAR * pTxData, WP_U32 dlci, WP_U32 port) 
{
   WP_status status;
   ...
   tx_buf.buffer_class = 0;
      ...
      WP_HT_IW_FSP_FLOW_AGG_FIELD (agg_enet_1) | WP_HT_IW_CRC_FIELD (0);
   ...
   data_unit.status = 0;
      ...
      WDDI_INFO
      ("Info: np_fr_host_send, FR-packet, dlci:%d, vport:0x%0x\n", dlci,
       port);
   curr_buf = data_unit.segment;
   WDDI_STRING (curr_buf->data, curr_buf->data_size);
   npu_mon_capture_packet ((char *) data_ptr, data_unit.data_size, port,
                            1);
      ...
      
   {
      ...
      {
         WDDI_WARNING
            ("np_fr_host_send fail,status=%d,tx_host_handle=0x%x\n",
      ...
      {
         WDDI_WARNING
            ("np_fr_host_send fail,WP_PoolFree,status=%d,tx_host_handle=0x%x\n",
             status, tx_host_handle);
         WP_PoolFree (curr_buf->pool_handle, curr_buf->data);
      }
   }
}
int np_fr_host_receive (WP_handle rx_host_handle, WP_S32 * DataLength,
                           WP_CHAR * pTxData) 
{
   WP_status status;
   WP_data_unit du =
      ...
      
   {
      if (strcmp
           ("WP_ERR_HST_NO_DATA_TO_GET",
            WP_error_name[WP_ERROR (status)]) != 0)
...
void np_disable_fr_host_channel (FR_CHANNEL_INFO * ch_info) 
{
   ...
   WP_boolean ch_disabled_state = WP_FALSE;
      ...
      return 0;
   (void) WP_ChannelStatus (ch_info->rx_host_handle,
                             WP_CH_STATUS_DISABLED, &ch_disabled_state);
      ...
      
   {
         ...
         
      {
         terminate_on_error (status,
                              "WP_ChannelDisable() rx_fr_host_handle");
      }
   ...
   {
      WDDI_INFO
         ("Warn: np_disable_fr_host_channel, ch_info->tx_host_handle:0\n");
      return 0;
   }
   (void) WP_ChannelStatus (ch_info->tx_host_handle,
                              WP_CH_STATUS_DISABLED, &ch_disabled_state);
   if (WP_FALSE == ch_disabled_state)
      
   {
      WDDI_INFO
         ("np_disable_fr_host_channel vfort = 0x%x (%d): S%dP%dFR%dD%d \n",
          ch_info->vfport, htonl (ch_info->vfport), card, trunk, group,
          dlci);
      i = 0;
         ...
         
      {
         ...
         {
            terminate_on_error (status,
                                 "WP_ChannelDisable() tx_fr_host_handle");
         }
         (void) WP_ChannelStatus (ch_info->tx_host_handle,
                                    WP_CH_STATUS_DISABLED,
                                    &ch_disabled_state);
...
void np_enable_fr_host_channel (FR_CHANNEL_INFO * ch_info) 
{
   ...
   np_fraddr_unpack (ch_info->vfport, &card, &trunk, &group, &dlci);
   WDDI_INFO
      ("np_enable_fr_host_channel vfort = 0x%x (%d): S%dP%dFR%dD%d \n",
      ...
      
   {
      ...
      {
         terminate_on_error (status,
                              "WP_ChannelEnable() rx_fr_host_handle");
      }
   }
   if (ch_info->tx_host_handle == 0)
      return;
      ...
      
   {
      ...
      {
         terminate_on_error (status,
                              "WP_ChannelEnable() tx_fr_host_handle");
      }
   }
}
void np_disable_fr_iw_channel (FR_CHANNEL_INFO * ch_info) 
{
   ...
   {
      WDDI_INFO
         ("Warn: np_disable_fr_iw_channel::np_get_system_info fail!\n");
      return;
   }
   
      /*Disable FR host rx and tx channels */ 
      (void) WP_ChannelStatus (ch_info->rx_iw_handle,
                               WP_CH_STATUS_DISABLED, &ch_disabled_state);
   if (WP_FALSE == ch_disabled_state)
      
   {
      WDDI_INFO
         ("Info: np_disable_fr_iw_channel, vfort:0x%x (%d): S%dP%dFR%dD%d\n",
          ch_info->vfport, htonl (ch_info->vfport), card, trunk, group,
          dlci);
      memset (&rx_binding_config, 0, sizeof (WP_rx_binding_fr_switching));
      rx_binding_config.default_aggregation =
         ch_info->agg_fr_rx_host_flow;
      status =
         WP_IwRxBindingModify (ch_info->rx_iw_handle,
                               np_system->system_fr_handle, qnode_fr_iw,
         ...
         
      {
         ...
         {
            terminate_on_error (status,
                                 "WP_ChannelDisable() rx_iw_handle");
         }
         (void) WP_ChannelStatus (ch_info->rx_iw_handle,
                                    WP_CH_STATUS_DISABLED,
                                    &ch_disabled_state);
   ...
   if (ch_info->tx_iw_handle == 0)
      return;
   (void) WP_ChannelStatus (ch_info->tx_iw_handle, WP_CH_STATUS_DISABLED,
                             &ch_disabled_state);
   if (WP_FALSE == ch_disabled_state)
      
   {
      ...
      status =
         WP_IwTxBindingModify (ch_info->tx_iw_handle,
                               WP_IW_TX_BIND_MOD_MAXT |
                               WP_IW_TX_BIND_MOD_DCI, WP_IW_TX_BINDING,
                               &tx_binding);
         ...
         
      {
         ...
         {
            terminate_on_error (status,
                                 "WP_ChannelDisable() tx_iw_handle");
         }
         (void) WP_ChannelStatus (ch_info->tx_iw_handle,
                                    WP_CH_STATUS_DISABLED,
                                    &ch_disabled_state);
         WP_Delay (50);
         i++;
...
void np_enable_fr_iw_channel (FR_CHANNEL_INFO * ch_info) 
{
   ...
   np_fraddr_unpack (ch_info->vfport, &card, &trunk, &group, &dlci);
   WDDI_INFO
      ("np_enable_fr_iw_channel vfort = 0x%x (%d): S%dP%dFR%dD%d \n",
       ch_info->vfport, htonl (ch_info->vfport), card, trunk, group, dlci);
   WP_boolean ch_disabled_state = WP_FALSE;
   WP_rx_binding_fr_switching rx_binding_config;
   NP_SYSTEM_INFO * np_system;
   ...
   if (ch_info->rx_iw_handle == 0)
      return;
   (void) WP_ChannelStatus (ch_info->rx_iw_handle, WP_CH_STATUS_DISABLED,
                             &ch_disabled_state);
   if (WP_TRUE == ch_disabled_state)
      
   {
         ...
         /*Enable FR host rx and tx channels */ 
         memset (&rx_binding_config, 0,
                 sizeof (WP_rx_binding_fr_switching));
      rx_binding_config.default_aggregation =
         ch_info->agg_fr_rx_host_flow;
      ...
      {
         terminate_on_error (status,
                              "WP_ChannelEnable() np_enable_fr_iw_rx channel");
      }
   }
   if (ch_info->tx_iw_handle == 0)
      return;
      ...
      
   {
      ...
      {
         terminate_on_error (status,
                              "WP_ChannelEnable() np_enable_fr_iw_tx channel");
      }
   }
}
...
WP_iw_agg_directmap tx2brg_agg_cfg[1] = 
{
   
   {
         ...
         /* prefix_header[68]; */
      {
      0x00, 0x22, 0x33, 0x44, 0x55, 0x66, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x04, 0x81, 0x00, 0x00, 0x00, 0x08, 0x00, 0x45, 0x00, 0xFF,
            0xFF, 0x00, 0x00, 0x00, 0x00, 0x40, 0x11, 0xFF, 0xFF, 0x00,
      ...
      , 
         ...
         /* cong_threshold_param */ NULL, 
                                                               ...
                                                               //WP_IW_LENGTH_UPDATE_L3,//WP_IW_LENGTH_UPDATE_DISABLED,
         /* length_offset */ 18 
   }
};
...
void np_creat_fr_iw_channel (FR_CHANNEL_INFO * ch_info) 
{
...
#if START_SCHEDULER
   {
      ...
      fr_shaping_param.cir =
         initial_cfg->stru_stm1_cfg_info[ch_info->card -
                                         1].trunk_info[ch_info->trunk -
                                                       1].
         group_info[ch_info->group].PVCQos[ch_info->dlci].cir;
      ...
      fr_shaping_param.cbs =
         initial_cfg->stru_stm1_cfg_info[ch_info->card -
                                         1].trunk_info[ch_info->trunk -
                                                       1].
         group_info[ch_info->group].PVCQos[ch_info->dlci].bc;
      fr_shaping_param.ebs =
         initial_cfg->stru_stm1_cfg_info[ch_info->card -
                                         1].trunk_info[ch_info->trunk -
                                                       1].
         group_info[ch_info->group].PVCQos[ch_info->dlci].be;
   ...
   ch_info->tx_iw_handle =
      WP_ChannelCreate (0, ch_info->device_handle, qnode_fr_iw, WP_CH_TX,
                        WP_FRAME_RELAY, &fr_ch_config);
   terminate_on_error (ch_info->tx_iw_handle,
                        "WP_ChannelCreate() tx_iw_handle");
   fr_ch_config.rx_dlci = ch_info->dlci;
   ch_info->rx_iw_handle =
      WP_ChannelCreate (0, ch_info->device_handle, qnode_fr_iw, WP_CH_RX,
                        WP_FRAME_RELAY, &fr_ch_config);
   terminate_on_error (ch_info->rx_iw_handle,
                        "WP_ChannelCreate() rx_iw_handle");
   ...
   fr_iw_tx_agg_config.auto_protocol_conversion_mode =
      WP_IW_PROTOCOL_CONV_DISABLE;
   fr_iw_tx_agg_config.protocol_pdu_update_mode =
   ...
   {
      ch_info->agg_fr_rx_host_flow =
         WP_IwFlowAggregationCreate (0, WP_IW_FR_SWITCHING_MODE,
                                     &fr_iw_tx_agg_config);
      terminate_on_error (ch_info->agg_fr_rx_host_flow,
                           "WP_IwFlowAggregationCreate ch_info->agg_fr_rx_host_flow");
      WDDI_INFO
         ("Info: np_creat_fr_iw_channel, created agg_fr_rx_host_flow:0x%x\n",
          ch_info->agg_fr_rx_host_flow);
   }
   WP_rx_binding_fr_switching rx_binding_config;
   memset (&rx_binding_config, 0, sizeof (WP_rx_binding_fr_switching));
   ...
   rx_binding_config.mru = N_FR_MAX_SDU;
   status =
      WP_IwRxBindingCreate (ch_info->rx_iw_handle,
                            np_system->system_fr_handle, qnode_fr_iw,
                            &rx_binding_config);
   terminate_on_error (status,
   ...
   {
      np_system->agg_FR_PrioRx0_host_handle =
         WP_IwFlowAggregationCreate (0, WP_IW_FR_SWITCHING_MODE,
                                     &fr_iw_tx_agg_config);
      terminate_on_error (np_system->agg_FR_PrioRx0_host_handle,
                           "WP_IwFlowAggregationCreate agg_FR_PrioRx0_host_handle");
      status =
         WP_ChannelEnable (np_system->Channel_FR_PrioRx0_host_handle);
      terminate_on_error (status,
                           "WP_ChannelEnable() Channel_FR_PrioRx0_host_handle");
      
#if 0
         u32 FlowIndex =
         WPI_HANDLE_INDEX (np_system->agg_FR_PrioRx0_host_handle);
      Fprintf
         ("np_system->agg_FR_PrioRx0_host_handle FlowIndex = 0x%x \n",
          FlowIndex);
      u32 tag = 0;
      int status;
      ...
      {
         Fprintf
            ("np_system->agg_FR_PrioRx0_host_handle get fail = 0x%x \n",
             FlowIndex);
      }
      ...
      (void) WP_PriorityMsgFlowSet (0, WP_PRIORIYT_MSG_FR_TYPE,
                                     &flowTable);
   ...
   flow_handle =
      WP_IwFlowAdd (np_system->system_fr_handle, WP_FLOW_FR_SWITCHING,
                    &flow_fr_sw);
   ...
   status =
      WP_IwTxBindingCreate (ch_info->tx_iw_handle, WP_IW_TX_BINDING,
                            &tx_binding_config);
...
void np_creat_fr_host_channel (FR_CHANNEL_INFO * ch_info) 
{
   ...
   fr_shaping_param.cir =
      initial_cfg->stru_stm1_cfg_info[ch_info->card -
                                      1].trunk_info[ch_info->trunk -
                                                    1].group_info[ch_info->
                                                                  group].
      PVCQos[ch_info->dlci].cir;
   if (WT_MIN_CH_RATE > fr_shaping_param.cir)
   ...
   ch_config_iw_tx_fr->iw_system = iw_sys_bridging;
      ...
      ch_info->tx_host_handle =
      WP_ChannelCreate (0, ch_info->device_handle, qnode_fr_host, WP_CH_TX,
                        WP_FRAME_RELAY, &fr_ch_config);
   terminate_on_error (ch_info->tx_host_handle,
                        "WP_ChannelCreate() tx_host_handle");
      ...
      ch_info->rx_host_handle =
      WP_ChannelCreate (0, fr_iw_host_device, qnode_fr_iw, WP_CH_RX,
                        WP_IW_HOST, &ch_config_iw_fr[0]);
   terminate_on_error (ch_info->rx_host_handle,
                         "WP_ChannelCreate() rx_host_handle");
   ...
   {
      np_system->Channel_FR_PrioRx0_host_handle =
         WP_ChannelCreate (0, fr_iw_host_device, qnode_fr_iw, WP_CH_RX,
                           WP_IW_HOST, &ch_config_iw_fr_priority[0]);
      terminate_on_error (np_system->Channel_FR_PrioRx0_host_handle,
...
void WT_SerdesInterrupt (WP_U32 wpid, WP_U32 signal_info) 
{
   ...
   fflush (stdout);
      ...
      // read and clear
      tmp =
      *(volatile unsigned int *) (unsigned long) (0x1e0082ec +
                                                  WPL_RIF_VIRTUAL (0, 0));
   *(volatile unsigned int *) (unsigned long) (0x1e0082ec +
                                                WPL_RIF_VIRTUAL (0, 0)) =
      tmp;
   int_reg = ntohl (tmp);
   tmp =
      *(volatile unsigned int *) (unsigned long) (0x1e0082cc +
                                                  WPL_RIF_VIRTUAL (0, 0));
   sta_reg = ntohl (tmp);
   for (i = 0; i < 3; i++)
   {
...
void WT_SgmiiAnInterrupt (WP_U32 wpid, WP_U32 signal_info) 
{
   ...
   fflush (stdout);
   tmp =
      *(volatile unsigned int *) (unsigned long) (0x1e016234 +
                                                  WPL_RIF_VIRTUAL (0, 0));
   tadir = ntohl (tmp);
   if (tadir & 1)
   {
   ...
   tmp =
      *(volatile unsigned int *) (unsigned long) (0x1e018234 +
                                                  WPL_RIF_VIRTUAL (0, 0));
   tadir = ntohl (tmp);
   if (tadir & 1)
   {
   ...
   tmp =
      *(volatile unsigned int *) (unsigned long) (0x1e016274 +
                                                  WPL_RIF_VIRTUAL (0, 0));
   tadir = ntohl (tmp);
   if (tadir & 1)
   {
...
static void WTUFE_config_system (WP_U32 ufe_lb_mode, WP_U32 ufe_port) 
{
   WUFE_init_config wufe_config =
   {
   0};
   WUFE_system wufe_system_config =
   {
   0};
   WUFE_status status;
      ...
      
   {
      ...
      wufe_system_config.system_clock_rec_enable =
...
#if 0
      WPL_IntConnect (WP_WINPATH (0), WPL_SerdesInterrupt, 0,
                      WT_SerdesInterrupt);
   ...
   WPL_IntEnable (WP_WINPATH (0), WPL_SgmiiAn);
      ...
      *(volatile unsigned int *) (unsigned long) (0x1e0082f0 +
                                                  WPL_RIF_VIRTUAL (0, 0)) =
      0;
   *(volatile unsigned int *) (unsigned long) (0x1e0082f4 +
                                                WPL_RIF_VIRTUAL (0, 0)) =
      htonl (0x80000777);
...
#if ENEA_RELEASE
      *(volatile unsigned int *) (unsigned long) (0x1e0082c0 +
                                                  WPL_RIF_VIRTUAL (0, 0)) =
      htonl (0x05618000);
   *(volatile unsigned int *) (unsigned long) (0x1e0082c4 +
                                                WPL_RIF_VIRTUAL (0, 0)) =
      htonl (0x05618000);
   *(volatile unsigned int *) (unsigned long) (0x1e0082c8 +
                                                WPL_RIF_VIRTUAL (0, 0)) =
      htonl (0x05618000);
   *(volatile unsigned int *) (unsigned long) (0x1e0082bc +
                                                WPL_RIF_VIRTUAL (0, 0)) =
      htonl (0x05618000);
   
#endif   /*  */
      status = WUFE_UfeFramerHwInterruptEnable (0);
   WT_UfeTerminateOnError (status, "WUFE_UfeFramerHwInterruptEnable", 0,
                            __LINE__);
   ...
   WT_UfeTerminateOnError (status, "WUFE_UfeCoreHwInterruptDisable", 0,
                            __LINE__);
   status = WUFE_SystemInterruptDisable (0);
   WT_UfeTerminateOnError (status, "WUFE_SystemInterruptDisable ", 0,
                            __LINE__);
   ...
   WTI_FlexmuxInit (ufe_port, 0, physical_configuration, flexmux_mode,
                      vc_3_4, &flex_global_params);
...
} static void WTUFE_config_lines (WP_U32 line_lb_mode) 
{
      ...
      
   {
      ...
      if (initial_cfg->stru_stm1_cfg_info[0].stm1Valid == NPU_VALID
          && initial_cfg->stru_stm1_cfg_info[1].stm1Valid == NPU_VALID)
      
   {
      
         /*make sure mode must be same */ 
         if (initial_cfg->stru_stm1_cfg_info[0].E1T1type !=
             initial_cfg->stru_stm1_cfg_info[1].E1T1type)
         
      {
         WDDI_ERR
            ("can't support STM1-1 STM-2 configured to different mode \n");
      }
      ...
      
   {
      ...
      {
         if ((line_index % N_UFE_MAX_LINES_SPE_T1) >=
         ...
         
      {
         ...
         if (OPMODE_E1_FALSE ==
              initial_cfg->stru_stm1_cfg_info[stm1Id].trunk_info[trunkId].
              Opt_Mode)
            ...
            
         {
            ...
            if (initial_cfg->stru_stm1_cfg_info[stm1Id].
                 trunk_info[trunkId].trunkValid)
               
            {
               ...
               {
                  if (initial_cfg->stru_stm1_cfg_info[stm1Id].
                       trunk_info[trunkId].group_info[i].groupValid)
                     
                  {
                     if (initial_cfg->stru_stm1_cfg_info[stm1Id].
                          trunk_info[trunkId].group_info[i].
                          slot_selection & 0x00010000)
         ...
         if (initial_cfg->stru_stm1_cfg_info[stm1Id].trunk_info[trunkId].
               trunkValid)
            
         {
            if (initial_cfg->stru_stm1_cfg_info[stm1Id].
                 trunk_info[trunkId].clock_mode == NPU_E1T1_CLOCK_MASTER)
            ...
            Fprintf
               ("line_index = %d framing_mode = %d cas_enable = %d clock_mode = %d \n",
                line_index, framing_mode, cas_enable, clock_mode);
            WT_Np_UfeLineConfigureAndCreateE1 (line_index, framing_mode,
                                                cas_enable, line_lb_mode,
                                                clock_mode);
         ...
         {
            Fprintf
               ("Create Default E1s for these un-configured E1s: line_index = %d framing_mode = %d cas_enable = %d clock_mode = %d \n",
                line_index, WUFE_FRAMED, cas_enable, WUFE_CAS_DISABLE);
            WT_Np_UfeLineConfigureAndCreateE1 (line_index, WUFE_FRAMED,
                                                WUFE_CAS_DISABLE,
                                                line_lb_mode,
      ...
      
   {
         ...
         
      {
            ...
            
         {
            ...
            WT_Np_UfeLineConfigureAndCreateT1 (line_index, framing_mode,
                                                cas_enable, line_lb_mode,
                                                clock_mode);
            reconfigStm1_createE1_flag[line_index] = 1;
         }
                           ...
                           ) 
{
   ...
   if (NPU_E1T1_CRC32 ==
        initial_cfg->stru_stm1_cfg_info[stm1Id].trunk_info[line_index -
                                                           line_index_base].
        group_info[groupId].crc_type)
   ...
   for (j = 0, nslots = 0, slots = slot_selection; slots > 0;
          ++j, slots >>= 1)
   ...
   memcpy (&phy_cfg.tx_slot_group, &slot_group, sizeof (slot_group));
      ...
      status =
      WUFE_PhyCreate (&ufe_phy_handle[line_index][groupId],
                      ufe_line_handle[line_index], &phy_cfg,
                      WUFE_FULL_DUPLEX);
   
      //terminate_on_error(status, "WUFE_PhyCreate()");
      WT_UfeTerminateOnError (status, "WUFE_PhyCreate", line_index,
                              __LINE__);
   status =
      WUFE_PhyEnable (ufe_phy_handle[line_index][groupId],
                      WUFE_FULL_DUPLEX);
...
#endif   /*  */
      fr_device_config[0].tx_maxsdu =
      initial_cfg->stru_stm1_cfg_info[stm1Id].trunk_info[line_index -
                                                         line_index_base].
      group_info[groupId].max_frame_size;
   fr_device_config[0].rx_maxsdu =
      initial_cfg->stru_stm1_cfg_info[stm1Id].trunk_info[line_index -
                                                         line_index_base].
      group_info[groupId].max_frame_size;
   ufe_device_handle[line_index][groupId] =
      WP_DeviceCreate (upi_ufe_port,
                       WP_PHY (WUFE_PHY_INDEX_GET
                                (ufe_phy_handle[line_index][groupId])),
                       WP_DEVICE_FR, &fr_device_config[0]);
   terminate_on_error (ufe_device_handle[line_index][groupId],
                        "WP_DeviceCreate ufe device()");
   status =
      WP_DeviceEnable (ufe_device_handle[line_index][groupId],
                       WP_DIRECTION_DUPLEX);
...
} } WP_status WTUFE_Reconfig_phys (WP_U32 trunkid, WP_U32 groupid,
                                       WP_U32 slot_selection,
                                       WP_U32 crc_type) 
{
   ...
   usr_phy_cfg.tx_slot_group.hmvip_id = 0;
      ...
      Fprintf ("slot_group.hmvip_id= %d,slot_group.n_slots= %d\n",
               usr_phy_cfg.rx_slot_group.hmvip_id,
               usr_phy_cfg.rx_slot_group.n_slots);
   for (i = 0; i < 32; i++)
      
   {
      Fprintf ("slot_group.slot_arr[%d]= %d\n", i,
                usr_phy_cfg.rx_slot_group.slot_arr[i]);
   }
   ...
   {
      status =
         WUFE_PhyReconfigure (ufe_phy_handle[trunkid][groupid],
                              &usr_phy_cfg);
...
WP_status WTUFE_Reconfig_Stm1 (WP_U32 stm1id, WP_U32 line_index_base,
                                 STRU_STM1_RECFG * reconfig_info,
                                 STRU_SET_INITAIL_CONFIG * p2) 
{
      ...
      
   {
         ...
         
      {
         initial_cfg->stru_stm1_cfg_info[stm1id].trunk_info[trunkid -
                                                             line_index_base].
            group_info[groupid].groupValid = NPU_VALID;
         initial_cfg->stru_stm1_cfg_info[stm1id].trunk_info[trunkid -
                                                             line_index_base].
            group_info[groupid].crc_type = crc_type;
         initial_cfg->stru_stm1_cfg_info[stm1id].trunk_info[trunkid -
                                                             line_index_base].
            group_info[groupid].max_frame_size = max_frame_size;
         initial_cfg->stru_stm1_cfg_info[stm1id].trunk_info[trunkid -
                                                             line_index_base].
            group_info[groupid].slot_selection = slot_selection;
            ...
            
         {
...
#if 1
                  status =
                  WUFE_PhyDisable (ufe_phy_handle[trunkid][groupid],
                                   WUFE_FULL_DUPLEX);
               if (WUFE_OK != status)
            ...
            (void) WP_DeviceDisable (ufe_device_handle[trunkid][groupid],
                                      WP_DIRECTION_DUPLEX);
            if (ufe_phy_handle[trunkid][groupid] != 0)
               
            {
               status =
                  WTUFE_Reconfig_phys (trunkid, groupid, slot_selection,
                                       crc_type);
               if (WUFE_OK != status)
            ...
            status =
               WP_DeviceModify (ufe_device_handle[trunkid][groupid],
            ...
            {
               status =
                  WUFE_PhyEnable (ufe_phy_handle[trunkid][groupid],
                                  WUFE_FULL_DUPLEX);
               if (WUFE_OK != status)
            ...
            (void) WP_DeviceEnable (ufe_device_handle[trunkid][groupid],
                                     WP_DIRECTION_DUPLEX);
         }
            ...
            
         {
            ...
            {
               Fprintf
                  ("Error: WTUFE_Reconfig_Stm1, trunk ID is invalid\n");
               return -1;
            }
               ...
               
            {
                  ...
                  /*creat new line */ 
                  if (p2->stru_stm1_cfg_info[stm1id].E1T1type ==
                      NPU_MODE_E1)
                  
               {
                  
#if 0                           //bugs
                     if ((trunkid % N_UFE_MAX_LINES_SPE_T1) >=
                         N_UFE_MAX_LINES_SPE_E1)
...
#endif   /*  */
                     if (OPMODE_E1_FALSE ==
                         p2->stru_stm1_cfg_info[stm1id].
                         trunk_info[trunkid - line_index_base].Opt_Mode)
                  ...
                  {
                     framing_mode = WUFE_FRAMED;
                     if (p2->stru_stm1_cfg_info[stm1id].
                          trunk_info[trunkid - line_index_base].trunkValid)
                  ...
                  if (p2->stru_stm1_cfg_info[stm1id].
                        trunk_info[trunkid - line_index_base].clock_mode ==
                  ...
                  Fprintf
                     ("WTUFE_Reconfig_Stm1::trunkid = %d framing_mode = %d cas_enable = %d clock_mode = %d\n",
                      trunkid, framing_mode, cas_enable, clock_mode);
                  WT_Np_UfeLineConfigureAndCreateE1 (trunkid,
                                                      framing_mode,
                                                      cas_enable,
                                                      UFE_NO_LOOPBK,
                                                      clock_mode);
               ...
               {
                  if (OPMODE_T1_D4 ==
                       p2->stru_stm1_cfg_info[stm1id].trunk_info[trunkid -
                                                                 line_index_base].
                       Opt_Mode)
                  ...
                  WT_Np_UfeLineConfigureAndCreateT1 (trunkid,
                                                      framing_mode,
                                                      cas_enable,
                                                      UFE_NO_LOOPBK,
                                                      clock_mode);
            ...
            initial_cfg->stru_stm1_cfg_info[stm1id].trunk_info[trunkid -
               ...
               p2->stru_stm1_cfg_info[stm1id].trunk_info[trunkid -
                                                         line_index_base].
               clock_mode;
            initial_cfg->stru_stm1_cfg_info[stm1id].E1T1type =
               p2->stru_stm1_cfg_info[stm1id].E1T1type;
            
               /*creat new phys */ 
               WTUFE_config_phys (trunkid, groupid, slot_selection,
                                  line_index_base);
         }
      }
   }
...
WP_status WTUFE_Reconfig_Stm1_inLoop (WP_U32 stm1id, WP_U32 trunkid,
                                         WP_U32 groupid,
                                         WP_U32 line_index_base,
                                         STRU_STM1_RECFG * reconfig_info,
                                         STRU_SET_INITAIL_CONFIG * p2) 
{
      ...
      
   {
         ...
         
      {
         ...
         WDDI_INFO
            ("WTUFE_Reconfig_Stm1_inLoop, device modify ufe_device_handle[%d][%d] \n",
             trunkid, groupid);
         (void) WP_DeviceEnable (ufe_device_handle[trunkid][groupid],
                                  WP_DIRECTION_DUPLEX);
      }
         ...
         
      {
         ...
         {
            Fprintf
               ("Error: WTUFE_Reconfig_Stm1_inLoop, trunk ID is invalid\n");
            return -1;
         }
         if (reconfigStm1_createE1_flag[trunkid] == 0)
            
         {
               ...
               
            {
               ...
               Fprintf
                  ("WTUFE_Reconfig_Stm1_inLoop::trunkid = %d framing_mode = %d cas_enable = %d clock_mode = %d\n",
                   trunkid, framing_mode, cas_enable, clock_mode);
               WT_Np_UfeLineConfigureAndCreateE1 (trunkid, framing_mode,
                                                   cas_enable,
                                                   UFE_NO_LOOPBK,
                                                   clock_mode);
            }
               ...
               
            {
               ...
               WT_Np_UfeLineConfigureAndCreateT1 (trunkid, framing_mode,
                                                   cas_enable,
                                                   UFE_NO_LOOPBK,
                                                   clock_mode);
         ...
         initial_cfg->stru_stm1_cfg_info[stm1id].E1T1type =
            ...
            /*creat new phys */ 
            WDDI_INFO
            ("WTUFE_Reconfig_Stm1_inLoop::WTUFE_config_phys,trunkid[%d], groupid[%d] \n",
             trunkid, groupid);
         WTUFE_config_phys (trunkid, groupid, slot_selection,
                             line_index_base);
      }
...
WP_status WTUFE_Reconfig_Fr_Channel (WP_U32 card, WP_U32 trunk,
                                         WP_U32 tsgroup, WP_U32 dlci,
                                         STRU_SET_INITAIL_CONFIG * p2) 
{
   STRU_VPORT_INFO * VportInfo;
      ...
      
   {
      ...
      {
         Fprintf
            ("WTUFE_Reconfig_Fr_Channel vport not used! vport = 0x%x\n",
             port_src);
         return 0;
      }
      ch_info = &(VportInfo->ch_info);
      ...
      {
         WP_pkt_shaping_cir_eir fr_shaping_param;
         memset ((void *) &fr_shaping_param, 0,
                  sizeof (fr_shaping_param));
         fr_shaping_param.cir =
            p2->stru_stm1_cfg_info[ch_info->card -
                                   1].trunk_info[ch_info->trunk -
                                                 1].group_info[ch_info->
                                                               group].
            PVCQos[ch_info->dlci].cir;
         ...
         {
            Fprintf
               ("Reconfig fr_iw_channel fr_shaping_param.cir rate %d is low than %d set is to ",
                fr_shaping_param.cir, WT_MIN_CH_RATE);
            fr_shaping_param.cir = WT_MIN_CH_RATE;
            Fprintf ("%d\n", fr_shaping_param.cir);
         }
         ...
         {
            Fprintf
               ("Reconfig fr_iw_channel fr_shaping_param.cir rate %d is big than %d set it to ",
                fr_shaping_param.cir, WT_MAX_CH_RATE);
            fr_shaping_param.cir = WT_MAX_CH_RATE;
            Fprintf ("%d\n", fr_shaping_param.cir);
         }
         ...
         fr_shaping_param.cbs =
            p2->stru_stm1_cfg_info[ch_info->card -
                                   1].trunk_info[ch_info->trunk -
                                                 1].group_info[ch_info->
                                                               group].
            PVCQos[ch_info->dlci].bc;
         fr_shaping_param.ebs =
            p2->stru_stm1_cfg_info[ch_info->card -
                                   1].trunk_info[ch_info->trunk -
                                                 1].group_info[ch_info->
                                                               group].
            PVCQos[ch_info->dlci].be;
         ...
         
      {
         ...
         status =
            WP_ChannelModify (ch_info->tx_iw_handle,
                              WP_CH_MOD_FR_TX_TRAFFIC, &fr_ch_config);
...
WP_status WTUFE_Reconfig_Stm1_All (STRU_SET_INITAIL_CONFIG * p1,
                                     STRU_SET_INITAIL_CONFIG * p2) 
{
      ...
      
   {
         ...
         
      {
         ...
         initial_cfg->stru_stm1_cfg_info[i].E1T1type =
            p2->stru_stm1_cfg_info[i].E1T1type;
...
#if 1
            for (trunkid = line_index_base; trunkid < line_index_counts;
                 trunkid++)
            
         {
            if (NPU_VALID ==
                 p2->stru_stm1_cfg_info[i].trunk_info[trunkid -
                                                      line_index_base].
                 trunkValid)
               
            {
               initial_cfg->stru_stm1_cfg_info[i].trunk_info[trunkid -
                                                              line_index_base].
                  trunkValid = NPU_VALID;
                  ...
                  
               {
                     ...
                     
                  {
                     ...
                     {
                        if (0 !=
                             (p2->stru_stm1_cfg_info[i].
                              trunk_info[trunkid -
                                         line_index_base].
                              group_info[groupid].PVCQos[k].cir))
                           
                        {
                           
                              /*only changed channel do reconfiguration */ 
                              if ((p1->stru_stm1_cfg_info[i].
                                   trunk_info[trunkid -
                                              line_index_base].
                                   group_info[groupid].PVCQos[k].cir ==
                                   p2->stru_stm1_cfg_info[i].
                                   trunk_info[trunkid -
                                              line_index_base].
                                   group_info[groupid].PVCQos[k].
                                   cir)  &&(p1->stru_stm1_cfg_info[i].
                                             trunk_info[trunkid -
                                                        line_index_base].
                                             group_info[groupid].PVCQos[k].
                                             bc ==
                     ...
                     initial_cfg->stru_stm1_cfg_info[i].
                        trunk_info[trunkid -
                                   line_index_base].group_info[groupid].
                        groupValid = NPU_VALID;
                     if (((p1->stru_stm1_cfg_info[i].
                            trunk_info[trunkid -
                                       line_index_base].
                            group_info[groupid].crc_type) ==
                           (p2->stru_stm1_cfg_info[i].
                            trunk_info[trunkid -
                                       line_index_base].
                            group_info[groupid].crc_type))  &&((p1->
                                                                 stru_stm1_cfg_info
                                                                 [i].
                                                                 trunk_info
                                                                 [trunkid -
                          ...
                          &&((p1->stru_stm1_cfg_info[i].
                              trunk_info[trunkid -
                                         line_index_base].
                              group_info[groupid].slot_selection) ==
                             (p2->stru_stm1_cfg_info[i].
                              trunk_info[trunkid -
                                         line_index_base].
                              group_info[groupid].slot_selection)))
                     ...
                     {
                        reconfig_info.group_info[groupid].groupValid =
                           NPU_VALID;
                        reconfig_info.group_info[groupid].crc_type =
                           p2->stru_stm1_cfg_info[i].trunk_info[trunkid -
                                                                line_index_base].
                           group_info[groupid].crc_type;
                        reconfig_info.group_info[groupid].max_frame_size =
                           p2->stru_stm1_cfg_info[i].trunk_info[trunkid -
                                                                line_index_base].
                           group_info[groupid].max_frame_size;
                        reconfig_info.group_info[groupid].slot_selection =
                           ...
                           group_info[groupid].slot_selection;
                        Fprintf
                           ("###reconfig_info slot_selection is 0x%x\n",
                            reconfig_info.group_info[groupid].
                            slot_selection);
                        Fprintf
                           ("stm1id = %d,trunkid = %d,line_index_base = %d,groupid = %d \n",
                            i, trunkid, line_index_base, groupid);
                        
#if 1
                           status =
                           WTUFE_Reconfig_Stm1_inLoop (i, trunkid, groupid,
                                                       line_index_base,
                                                       &reconfig_info, p2);
                        if (WUFE_OK != status)
                     ...
                     
                  {
                     initial_cfg->stru_stm1_cfg_info[i].
                        trunk_info[trunkid -
                                   line_index_base].group_info[groupid].
                        groupValid = NPU_INVALID;
                  }
               }
               
#if 0
                  status =
                  WTUFE_Reconfig_Stm1 (i, line_index_base, &reconfig_info,
                                       p2);
               if (WUFE_OK != status)
                  ...
                  
               {
                  ...
                  npu_ufe_reconfig_clock_mode (trunkid,
                                                p2->stru_stm1_cfg_info[i].
                                                E1T1type, clock_mode);
               }
            }
      ...
      {
         if (0 != initial_cfg->stm1_num
              && NPU_VALID == initial_cfg->stru_stm1_cfg_info[i].stm1Valid)
            ...
            
         {
               ...
               
            {
                  ...
                  
               {
                  ...
                  {
                     status =
                        WUFE_PhyDelete (ufe_phy_handle[trunkid][groupid]);
                     if (status == WUFE_OK)
                        break;
               ...
               {
                  (void)
                     WP_DeviceDisable (ufe_device_handle[trunkid][groupid],
                                       WP_DIRECTION_DUPLEX);
                  (void)
                     WP_DeviceDelete (ufe_device_handle[trunkid][groupid]);
               }
         } } continue;
...
void np_display_ufe_system (void) 
{
   ...
   WPX_FRMR_STATUS_DEV_VERSION_GetHardwareInfo (0,
...
{
   WP_handle status;
   if (lineIndex > N_UFE_MAX_LINES_T1 - 1)
      return;
      ...
      
   {
      ...
      {
         client_port =
            WTI_FlexmuxClientPortIndexGet (E1MapTable[lineIndex],
                                           WUFE_SDH_TYPE_E1);
      ...
      {
         status =
            WPX_FRMR_SONET_SDH_DIAG_LpbkDisable (0, 0, &line_params);
         terminate_on_error (status,
                              "WPX_FRMR_SONET_SDH_DIAG_LpbkDisable ");
         status =
            WPX_FRMR_SONET_SDH_PORT_DIAG_LpbkEnableShallowLine (0, 0,
                                                                client_port);
         ...
         Fprintf
            ("line %d WPX_FRMR_SONET_SDH_PORT_DIAG_LpbkEnableShallowLine !\n",
             lineIndex);
      }
         ...
         
      {
         ...
         status =
            WPX_FRMR_SONET_SDH_PORT_DIAG_LpbkDisableShallowLine (0, 0,
                                                                 client_port);
         terminate_on_error (status,
         ...
         
      {
         ...
         terminate_on_error (status,
                              "WPX_FRMR_SONET_SDH_PORT_DIAG_LpbkDisableShallowLine ");
         status =
            WPX_FRMR_SOCKET_PDH_DIAG_LpbkEnableDeepLine (0, 0,
                                                         client_port);
         terminate_on_error (status,
                              "WPX_FRMR_SOCKET_PDH_DIAG_LpbkEnableDeepLine ");
         loopbkflag[lineIndex]++;
         Fprintf
            ("line %d WPX_FRMR_SOCKET_PDH_DIAG_LpbkEnableDeepLine !\n",
             lineIndex);
      }
...
void np_frIwSystemCreate (NP_SYSTEM_INFO * np_system) 
{
   ...
   np_system->system_fr_handle =
      WP_IwSystemCreate (WP_WINPATH (0), WP_IW_FR_SWITCHING_MODE,
                         &fr_sw_system_cfg);
   terminate_on_error (np_system->system_fr_handle,
...
} void displayUfeEvents (void) 
{
   ...
   if (event & 0x400)
      Fprintf ("\tBS LOCK : TX EMPHY PLL LOST CLOCK\n");
   if (event & 0x200)
      Fprintf ("\tBS LOCK : RX EMPHY PLL LOST CLOCK\n");
   if (event & 0x080)
      Fprintf ("\tBS LOCK : DDR1 PLL LOST CLOCK\n");
   if (event & 0x040)
      Fprintf ("\tBS LOCK : DDR0 PLL LOST CLOCK\n");
   if (event & 0x020)
      Fprintf ("\tBS LOCK : DIFF PLL LOST CLOCK\n");
   if (event & 0x010)
      Fprintf ("\tBS LOCK : ADAPT PLL LOST CLOCK\n");
   if (event & 0x008)
      Fprintf ("\tBS LOCK : EXT PLL LOST CLOCK\n");
   if (event & 0x004)
      Fprintf ("\tBS LOCK : FRMR PLL LOST CLOCK\n");
   if (event & 0x003)
      Fprintf ("\tBS LOCK : DDR PLL LOST CLOCK\n");
   if (event & 0x002)
      Fprintf ("\tBS LOCK : SYSCLK PLL LOST CLOCK\n");
...
{
   Fprintf ("\033[%dA", up);
} void NP_DisplayWingineUtilization (WP_U32 * wg_util,
                                        WP_U32 total_util) 
{
      ...
      
   {
      ...
      Fprintf
         ("     WG1:%02d%%  WG2:%02d%%  WG3:%02d%%  WG4:%02d%%     \n",
          wg_util[WP_WINGINE1], wg_util[WP_WINGINE2],
          wg_util[WP_WINGINE3], wg_util[WP_WINGINE4]);
      Fprintf ("                                            \n");
...
void Np_ShowMemoryAvailable (STRU_MEMORY_STATICS * get_memory_statics) 
{
   ...
   WP_U32 bytes;
   STRU_SET_INITAIL_CONFIG * initial_cfg = npu_get_initial_cfg_info ();
   get_memory_statics->InterMemTotalByte =
      initial_cfg->InterMemTotalByte;
   get_memory_statics->PacketMemTotalByte =
      initial_cfg->PacketMemTotalByte;
   get_memory_statics->ParamMemTotalByte = initial_cfg->ParamMemTotalByte;
   
      /* get the available memeory of the packet bus */ 
      status =
      WP_MemoryAvailable (WP_WINPATH (0), (u32) WP_BUS_PACKET,
   ...
   get_memory_statics->PacketMemLeftByte = bytes;
      ...
      status =
      WP_MemoryAvailable (WP_WINPATH (0), (u32) WP_BUS_PARAM,
   ...
   get_memory_statics->ParamMemLeftByte = bytes;
      ...
      status =
      WP_MemoryAvailable (WP_WINPATH (0), (u32) WP_BUS_INTERNAL,
...
#define HWno
void WT_Np_UfeLineConfigureAndCreateE1 (WP_U32 line_index,
                                        WP_U16 framing_mode,
                                        WP_U8 cas_enable,
                                        WP_U32 line_lb_mode,
                                        WP_U32 clock_mode) 
{
   ...
   WT_Np_UfeLineCreate (line_index, &line_cfg, &line_params,
                         line_lb_mode);
}
void WT_Np_UfeLineConfigureAndCreateT1 (WP_U32 line_index,
                                          WP_U16 framing_mode,
                                          WP_U8 cas_enable,
                                          WP_U32 line_lb_mode,
                                          WP_U32 clock_mode) 
...
WP_S32 WTI_Np_FlexmuxClientPortIndexGet (WP_U32 ufe_line_index,
...
WP_S32 WTI_Np_FlexmuxLinePortIndexGet (WP_U32 ufe_line_index,
                                         WP_U32 flexmux_id) 
{
   WP_U32 result = -1;
                            ...
                            WP_U32 line_lb_mode) 
{
   ...
   case (WUFE_SDH_TYPE_E1):
      ...
      {
         if (line_sdh_params->framed == WUFE_FRAMED
              && line_sdh_params->cas_enable == WUFE_CAS_ENABLE)
         ...
         else if (line_sdh_params->framed == WUFE_FRAMED
                  && line_sdh_params->cas_enable == WUFE_CAS_DISABLE)
   ...
   (void) WTI_FlexmuxConnectionCreate (0, line_port, client_port,
                                        con_type,
   ...
   {
      status =
         WUFE_LineLoopbackConfigure (ufe_line_handle[line_index],
...
void np_display_all_ufe4_e1t1_status () 
{
      ...
      
   {
         ...
         
      {
         ...
         {
            if (initial_cfg->stru_stm1_cfg_info[stm1].trunk_info[i].
                 trunkValid == NPU_VALID)
...
void np_display_ufe4_e1t1_status (WP_U32 LinePort, WP_U32 lineIndex) 
{
   ...
   if (LinePort > SPP_MAX_STM1_PORT_NUM)
      return -1;
   STRU_STM1_ALARM_INFO * alamInfo = np_get_ufe4_aralm_info ();
   STRU_STM1_E1T1_ALARM_INFO * AlarmE1T1 =
      &alamInfo->E1T1_alarm[LinePort][lineIndex];
   if (AlarmE1T1->LOS || AlarmE1T1->LOF || AlarmE1T1->AIS
         || AlarmE1T1->AIS_CI || AlarmE1T1->RAI || AlarmE1T1->RAI_CI)
...
void np_set_ufe4_aralm_info (WP_U32 LinePort, WP_U32 AlarmCategory,
                               WP_U32 IsAsserted) 
{
   if (LinePort > SPP_MAX_STM1_PORT_NUM)
      return;
   if (AlarmCategory > WPX_UFE_FRAMER_SDH_TU_LOP)
      return;
   STRU_STM1_ALARM_INFO * alamInfo = np_get_ufe4_aralm_info ();
...
void
np_set_ufe4_E1T1_aralm_info (WPX_UFE_FRAMER_COMMON_SDH_SONET_ENDPOINT_TYPE
                             * pLineEndpoint, WP_U32 AlarmCategory,
                             WP_U32 IsAsserted) 
{
   WP_U8 stm1 = pLineEndpoint->u.SDH.stm1;
   WP_U8 tug3 = pLineEndpoint->u.SDH.stm0;
   WP_U8 tug2 = pLineEndpoint->u.SDH.tug2;
   WP_U8 tug12 = pLineEndpoint->u.SDH.tu;
   WP_U16 lineIndex;
   STRU_SET_INITAIL_CONFIG * initial_cfg = npu_get_initial_cfg_info ();
   
      //Fprintf(" np_set_ufe4_E1T1_aralm_info stm1 =%d  lineIndex = %d IsAsserted = %d tug3=%d tug2 %d tug12=%d \n",stm1,lineIndex,IsAsserted,tug3,tug2,tug12);
#if 1
      //if(stm1 == 4 && IsAsserted == 0)
      if (stm1 != 0)
...
#endif   /*  */
      if (stm1 > (SPP_MAX_STM1_PORT_NUM - 1))
      
   {
      
         //Fprintf("stm1 %d is not valid \n",stm1);
         return;
   }
   if (initial_cfg->stru_stm1_cfg_info[stm1].E1T1type == NPU_MODE_E1)
      
   {
      ...
      if (tug12 > 3)
         return;
      lineIndex = tug12 + 3 * tug2 + 21 * tug3;
      
         //Fprintf("stm1 = %d tug3 = %d tug2 =%d tug12 = %d lineIndex = %d \n",stm1,tug3,tug2,tug12,lineIndex);
         /*Lucent TU12 + TU2 *3 +  TU3*21 */ 
   }
      ...
      
   {
      ...
      if (tug12 > 4)
         return;
      lineIndex = tug12 + 4 * tug2 + 28 * tug3;
      
         //Fprintf("stm1 = %d tug3 = %d tug2 =%d tug12 = %d lineIndex = %d \n",stm1,tug3,tug2,tug12,lineIndex);
         /*Lucent TU12 + TU2 *4 +  TU3*28 */ 
   }
   STRU_STM1_ALARM_INFO * alamInfo = np_get_ufe4_aralm_info ();
   STRU_STM1_E1T1_ALARM_INFO * AlarmE1T1 =
      &alamInfo->E1T1_alarm[stm1][lineIndex];
   if (AlarmCategory == WPX_UFE_FRAMER_SDH_LP_PLM)
...
void np_set_ufe4_E1T1_aralm_info_fromClientPort (WP_U32 ClientPort,
                                                   WP_U32 AlarmCategory,
                                                   WP_U32 IsAsserted) 
{
   WP_U16 lineIndex;
   WP_U16 stm1;
...
int np_check_ufe4_stm1_status (WP_U32 LinePort) 
{
   STRU_STM1_ALARM_INFO * alamInfo = np_get_ufe4_aralm_info ();
   STRU_STM1_SDH_ALARM * AlarmSDH = &alamInfo->SDH_alarm[LinePort];
   if (AlarmSDH->SDH_OOF)      //LOF miss when initial
   {
...
void np_config_all_phys (void) 
{
      ...
      
   {
         ...
         
      {
         ...
         {
            if (NPU_VALID ==
                 initial_cfg->stru_stm1_cfg_info[i].trunk_info[j -
                                                               line_index_base].
                 trunkValid)
                     ...
                     
                  {
                        ...
                        
                     {
                        slot_selection =
                           initial_cfg->stru_stm1_cfg_info[i].
                           trunk_info[j -
                                      line_index_base].group_info[k].
                           slot_selection;
                        WTUFE_config_phys (j, k, slot_selection,
                                            line_index_base);
                        Fprintf
                           ("UFE phy on stm1 %d trunk %d ,group %d is Created slot_selection = 0x%x \n",
            ...
            else                // for these un-configured E1s, we need create the defaut object to clear the LOP alarms on the peer transmission device;
            {
                  ...
                  //Create UFE phy for un-configured E1:
                  WTUFE_config_phys (j, 1, 0xfffffffe, line_index_base);
               Fprintf
                  ("Create UFE phy for un-configured E1:  UFE phy on stm1 %d trunk %d ,group %d is Created slot_selection = 0x%x \n",
                   i, j + 1, 1, 0xfffffffe);
            }
         }
      }
...
void np_DeviceDelete (WP_U32 trunkid, WP_U32 groupid) 
{
      ...
      
   {
      ...
      {
         Fprintf
            ("np_DeviceDelete WP_DeviceDisable failed! status = %d\n",
             status);
      }
...
WP_status np_Ufe_Phy_Delete (WP_U32 trunkid, WP_U32 groupid) 
{
      ...
      
   {
      ...
      {
         Fprintf
            ("np_Ufe_Phy_Delete WUFE_PhyDisable failed! status = %d\n",
             status);
      }
      for (j = 0; j < 100000; j++)
...
WP_status np_Channel_is_None (WP_U32 stm1Id, WP_U32 trunkid,
                                WP_U32 groupid) 
{
   WP_U32 vfport, dlci;
   STRU_VPORT_INFO * VportInfo;
      ...
      
   {
      ...
      {
         if ((VportInfo->ch_info.rx_host_handle != 0)
              || (VportInfo->ch_info.rx_iw_handle != 0)
              || (VportInfo->ch_info.tx_iw_handle != 0))
...
} void npu_ufe_reconfig_clock_mode (WP_U32 line_index, WP_U32 trans_type,
                                      WP_U32 clock_mode) 
{
   WP_S32 client_port;
   WP_S32 status;
   ...
   Fprintf ("npu_ufe_reconfig_clock_mode clock_mode=%d\n", clock_mode);
      ...
      status =
      WPX_FRMR_SOCKET_PDH_SetClkRec (0, 0, client_port, clock_mode);
   WTI_FlexmuxCheckStatus
      ("npu_ufe_reconfig_clock_mode WPX_FRMR_SOCKET_PDH_SetClkRec", status,
       __LINE__);
   return;
}
