...
WP_handle DportGrpagg_handle[65535] = {0};
...
int np_dl_route_table_add(STRU_SET_DOWNLINK_PATH_RULE * rule)
{
	...
	if(DportGrpflow_handle[rule->DportStartValue] == 0)
	{
		...
		sprintf (enet_rule_string, "n;n;n;n;n;n;[%d-%d];UDP;",htons(rule->DportStartValue),htons(rule->DportStartValue)+initial_cfg->PortPoolSize-1); 
		...
		DportGrpagg_handle[rule->DportStartValue] = enet_iw_flow_aggregation;
	...
	flow_info.agg_dl_dportGrp_handle= DportGrpagg_handle[rule->DportStartValue];
...
int np_dl_link_table_set(STRU_SET_FR_DOWNLINK_PATH_LINK_TABLE * link_table)
{
	...
	WDDI_INFO("np_dl_link_table_set vfort = 0x%x \n",vfort);
...
int np_dl_route_table_delete(WP_U16 DportStartValue,WP_U16 bvci,WP_U32 OptMode)
{
	...
	*/
    ...
    if(OptMode == DEL_DL_RULE_TALBE_BASED_ON_DPORT_BVCI)
    {
	    ...
	    WDDI_INFO("np_dl_route_table_delete1 DportStartValue = 0x%x bvci = %d \n",DportStartValue,bvci);
	...
	else if(OptMode == DEL_DL_RULE_TALBE_BASED_ON_DPORT)
	{
	    ...
	    {	    
		    ...
		    if(flow_info_ptr == NULL)
		    {
				WDDI_WARNING("can not find folw to be deleted.DportStartValue = %d \n",DportStartValue);
				//return ERR_DOWNLINX_FLOW_DELETE;
		    }
			else if(flow_info_ptr->UsedFlag == UNUSED)
			{
				WDDI_WARNING("this dl route has been been deleted ,OptMode = DEL_DL_RULE_TALBE_BASED_ON_DPORT\n");
				//return ERR_DOWNLINX_FLOW_DELETE;
			}
			else
			{
				WDDI_INFO("np_dl_route_table_delete2 DportStartValue = 0x%x bvci = %d \n",DportStartValue,bvci);			
	...
	else if(OptMode == DEL_DL_RULE_TALBE_BASED_ON_BVCI)
	{
	    ...
	    while(temp!=NULL)
	    {
		    ...
		    if(flow_info_ptr == NULL)
		    {
				WDDI_WARNING("can not find folw to be deleted.bvci= %d \n",bvci);
				//return ERR_DOWNLINX_FLOW_DELETE;
		    }
			else if(flow_info_ptr->UsedFlag == UNUSED)
			{
				WDDI_WARNING("this dl route has been been deleted ,OptMode = DEL_DL_RULE_TALBE_BASED_ON_BVCI\n");
				//return ERR_DOWNLINX_FLOW_DELETE;
			}			
			else
			{
				WDDI_INFO("np_dl_route_table_delete3 DportStartValue = 0x%x bvci = %d \n",DportStartValue,bvci);			
				WP_DlLookupEntryRemove(0,flow_info_ptr->agg_dl_dportGrp_handle,bvci);
				flow_info_ptr->UsedFlag = UNUSED;
				npu_set_downlink_info(flow_info_ptr);
			}			
	...
	{
        WDDI_ERR("np_dl_route_table_delete OptMode= %d \n",OptMode);	    
        return -1;
	}
...
int np_updateBVCI_flow_add(STRU_SET_UPDATE_BVCI_PATH_RULE *rule)
{
	...
	if(VportInfo == NULL)
	{
	    if(initial_cfg->OpterateMode == NPU_MODE_FR)
	    {
			WDDI_WARNING(" np_updateBVCI_flow_add can't get fr vport info vport = %d S%dP%dFR%dD%d\n ",rule->u_port.vfport,card,trunk,group,dlci);	    
	    }
		else
		{
			WDDI_WARNING(" np_updateBVCI_flow_add can't get fr vport info vport = %d \n ",rule->u_port.vfport);
		}
		return ERR_BVCI_FLOW_ADD;
	}
    else if(VportInfo->UsedFlag == UNUSED)
    {
		WDDI_WARNING(" np_updateBVCI_flow_add  vport closed! = %d \n ",rule->u_port.vfport);
		return ERR_BVCI_FLOW_ADD;
	}
	if(initial_cfg->OpterateMode == NPU_MODE_FR)
	{
...

{
	...
	if(VportInfo == NULL)
	{
		WDDI_WARNING(" np_updateBVCI_flow_delete can't get fr vport info vport = %d \n ",vfport);
		return ERR_BVCI_FLOW_DELETE;
	}else if(VportInfo->UsedFlag == UNUSED)
	{
		WDDI_WARNING(" np_updateBVCI_flow_delete can't get fr vport info vport = %d unused !\n ",vfport);	
		return ERR_BVCI_FLOW_DELETE;
	}
...
int np_ul_route_table_add(STRU_SET_UPLINK_PATH_RULE *uplink_rule)
{
	...
	if(NRI>256)
	{
	    WDDI_WARNING("np_ul_route_table_add NIR is invalid = %d\n",NRI); 
	...
	if(VportInfo ==  NULL)
	{
	    WDDI_WARNING("np_ul_route_table_add npu_get_vport_info failed vfport = %d\n",uplink_rule->u_port.vfport); 
	    return ERR_UPLINX_FLOW_ADD;
	}
	else if(VportInfo->UsedFlag == UNUSED)
	{
	    WDDI_WARNING("np_ul_route_table_add vport closed! vfport = %d\n",uplink_rule->u_port.vfport); 
	    return ERR_UPLINX_FLOW_ADD;
	}
	if(initial_cfg->OpterateMode == NPU_MODE_FR)
	{
...
int np_ul_route_table_delete(WP_U32 vfport,WP_U32 NRIValue)
{
    ...
    {
        WDDI_WARNING("this ul route has been been deleted vfport = 0x%x,NRIValue =  %d\n",vfport,NRIValue);
		return ERR_UPLINX_FLOW_DELETE;
	}
    FR_CHANNEL_INFO * ch_info;
	...
	if(VportInfo ==  NULL)
	{
	    WDDI_WARNING("np_ul_route_table_delete npu_get_vport_info failed vfport = %d\n",p_flow_info->vport); 
	    return ERR_UPLINX_FLOW_ADD;
	}
	else if(VportInfo->UsedFlag == UNUSED)
	{
		WDDI_WARNING("np_ul_route_table_delete npu_get_vport_info fail ,vport closed vport= 0x%x",vfport);
		return ERR_DOWNLINX_FLOW_ADD;
	}	
    if(initial_cfg->OpterateMode == NPU_MODE_FR)
    {
	    ...
	    WDDI_INFO("np_ul_route_table_delete vfport = 0x%x NRIValue = %d \n",vfport,NRIValue);
    }
	else /*IP<-IP>*/
	{
		...
		p_flow_info->UsedFlag = UNUSED;
	    npu_set_uplink_info(p_flow_info);    
	    WDDI_INFO("np_ul_route_table_delete portSrc = 0x%x NRIValue = %d \n",vfport,NRIValue);
...
int np_ul_link_table_set(STRU_SET_UPLINK_PATH_LINK_TABLE *link_table)
{
	...
	/*set arp for IPEP dst*/
	{
		...
		{
			WDDI_WARNING("np_ul_link_table_set npu_get_arp_info fail ipdst= 0x%x",IpDst);
			//return ERR_UPLINX_FLOW_ADD;
			LoacalArpfail = 1;
	...
	else
	{
		...
		if(local_rt.valid_num == 0)
		{
			WDDI_WARNING("np_ul_link_table_set npu_get_arp_info fail & not PEER link configured!");
			return ERR_UPLINX_FLOW_ADD;
		}
		WP_IPEndPointEntryInsert(0,htons(link_table_tmp.PortSrc), &local_rt);
		WDDI_INFO("np_ul_link_table_set npu_get_arp_info PortSrc = 0x%x linknum=%d\n",link_table_tmp.PortSrc,link_table_tmp.IpEpAcitveNum);
...
int np_ul_release_vport_resource(u32 vport)
{
	...
	if(VportInfo == NULL)
	{
		WDDI_WARNING("np_ul_release_resource npu_get_vport_info fail vport= 0x%x",vport);
		return ERR_UL_RELEASE_RESOURCE;
	}
	else if(VportInfo->UsedFlag == UNUSED)
	{
		WDDI_WARNING("np_ul_release_resource npu_get_vport_info fail ,vport closed vport= 0x%x",vport);
		return ERR_UL_RELEASE_RESOURCE;
	}
...
int np_dl_release_vport_resource(u32 vport)
{
	...
	if(VportInfo == NULL)
	{
		WDDI_WARNING("np_dl_release_resource npu_get_vport_info fail vport= 0x%x",vport);
		return ERR_DL_RELEASE_RESOURCE;
	}
	else if(VportInfo->UsedFlag == UNUSED)
	{
		WDDI_WARNING("np_dl_release_resource npu_get_vport_info fail ,vport closed vport= 0x%x",vport);
		return ERR_DL_RELEASE_RESOURCE;
	}
...
void np_dl_release_FMC_resource(u16 DportStartValue)
{
	...
	if(DportGrpagg_handle[DportStartValue] != 0)
	{
		status = WP_IwFlowAggregationDelete(DportGrpagg_handle[DportStartValue]);
		terminate_on_error(status, "WP_IwFlowAggregationDelete - DportGrpflow_handle[DportStartValue]");  
		DportGrpagg_handle[DportStartValue] = 0;
	}
}
...
int np_ul_link_table_delete(WP_U32 portsrc)
{
	...
	if(link_table == NULL)
	{
		WDDI_WARNING("np_ul_link_table_delete ul link table is not existed ,key PortSrc = 0x%x",portsrc);
		return ERR_DOWNLINX_FLOW_ADD;
	}
	if(link_table->IpEpAcitveNum == 0)
	{
...
void np_reconfig_vlan_in_iwPort(void)
{
    ...
    Fprintf("np_reconfig->VlanTag is %d\n",np_reconfig->VlanTag);
	...
	if(the_system->Bport_mac_filter_handle != 0)
	{
	    ...
	    if(WP_OK != status)
	    {
	       WDDI_WARNING("np_reconfig_vlan_in_iwPort WP_IwPortModify the_system->Bport_mac_filter_handle fail");
	    }
	}
    if(the_system->agg_mac_filter_handle != 0)
    {
	   ...
	   if(WP_OK != status)
	   {
	      WDDI_WARNING("np_reconfig_vlan_in_iwPort WP_IwFlowAggregationModify the_system->agg_mac_filter_handle fail");
	   }
    }
#if 1
	if(iw_enet_3 != 0)
	{
	    ...
	    if(WP_OK != status)
	    {
	       WDDI_WARNING("np_reconfig_vlan_in_iwPort WP_IwPortModify iw_enet_3 fail");
	    }
	}
    ...
    if(ipdefrag_agg != 0)
    {
	   ...
	   if(WP_OK != status)
	   {
	      WDDI_WARNING("np_reconfig_vlan_in_iwPort WP_IwFlowAggregationModify ipdefrag_agg fail");
	   }
    }
#endif	
       tx_agg_enet->iw_port = iw_enet_3; 
	   ...
	   if(WP_OK != status)
	   {
	      WDDI_WARNING("np_reconfig_vlan_in_iwPort WP_IwFlowAggregationModify the_system->agg_after_defrag fail");
	   }
}
